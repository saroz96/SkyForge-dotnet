    ...
    public class TransactionItem
    {
        ...
        public decimal? Quantity
        {
            ...
            set
            {
                if (value.HasValue)
                {
                    ...
                    _quantity = wsUnit == 0 ? value.Value : wsUnit * value.Value;
                }
                else
                {
                    _quantity = null;
        ...
        public decimal? Bonus
        {
            ...
            set
            {
                if (value.HasValue)
                {
                    var wsUnit = WSUnit ?? 1;
                    // Prevent division by zero
                    _bonus = wsUnit == 0 ? value.Value : wsUnit * value.Value;
                }
                else
                {
                    _bonus = null;
        ...
        public decimal? PuPrice
        {
            ...
            set
            {
                ...
                {
                    var wsUnit = WSUnit ?? 1;
                    // Prevent division by zero - this is the critical fix
                    if (wsUnit == 0)
                    {
                        _puPrice = value.Value; // Can't divide by zero, just set the value
                    }
                    else
                    {
                        _puPrice = value.Value / wsUnit;
