...
//                 // -----------------------------------------------------------------
//                 // FIX: Pick the fiscal year that CONTAINS today — same behaviour
//                 // as the authenticated /api/retailer/statement initial-data flow.
//                 // This avoids picking a future "IsActive = true" fiscal year and
//                 // rendering a From-date like 2027-07-17.
//                 // -----------------------------------------------------------------
...
//                      .FirstOrDefault(f => f.StartDate.HasValue && f.EndDate.HasValue
//                                        && f.StartDate.Value.Date <= today
//                                        && f.EndDate.Value.Date >= today)
//                  ?? fiscalYears.FirstOrDefault(f => f.IsActive)
//                  ?? fiscalYears.FirstOrDefault();
//                 // Use AD dates (the StatementService always expects AD)
//                 DateTime fromDate = fiscalYear?.StartDate ?? today.AddMonths(-1);
//                 DateTime toDate = today;

//                 // Safety net for bad / inverted data
//                 if (fromDate > toDate)
//                 {
...
//             Account account, Company company, StatementDataDTO data,
//             DateTime fromDate, DateTime toDate)
//         {
//             string fromDateNepali = data.Statement.FirstOrDefault()?.NepaliDate
//                 ?? fromDate.ToString("yyyy-MM-dd");
//             string toDateNepali = data.Statement.LastOrDefault()?.NepaliDate
