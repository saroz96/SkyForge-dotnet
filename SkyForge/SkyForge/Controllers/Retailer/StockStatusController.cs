            ...
            [FromQuery] string? toDate = null)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(fromDate) && !string.IsNullOrEmpty(toDate))
                {
                    ...
                    {
                        _logger.LogInformation("Using provided fromDate: {FromDate}, toDate: {ToDate}", fromDate, toDate);
                    }
                    else
                    {
                        fromDateTime = DateTime.UtcNow.Date;
                        toDateTime = DateTime.UtcNow.Date;
                        _logger.LogWarning("Invalid date format, using current date");
                    }
                }
                else
                {
                    // Default to current fiscal year dates
                    ...
                    if (currentFiscalYear != null)
                    {
                        if (isNepaliFormat)
                        {
                            // For Nepali format, parse the string date or use current date
                            if (!string.IsNullOrEmpty(currentFiscalYear.StartDateNepali) && DateTime.TryParse(currentFiscalYear.StartDateNepali, out fromDateTime))
                            {
                                // Successfully parsed
                            }
                            else
                            {
                                fromDateTime = DateTime.UtcNow.Date.AddMonths(-1);
                            }
                            toDateTime = DateTime.UtcNow.Date;
                        }
                        else
                        {
                            // For English format, use the DateTime property
                            fromDateTime = currentFiscalYear.StartDate ?? DateTime.UtcNow.Date.AddMonths(-1);
                            toDateTime = DateTime.UtcNow.Date;
                        }
                    }
                    else
