    ...
    public class ItemsController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateItem(Guid id, [FromBody] UpdateItemDTO updateItemDto)
        {
            try
            {
                ...
                var updatedItem = await _itemService.UpdateItemAsync(id, updateItemDto, companyIdGuid, currentFiscalYearIdGuid);
        ...
        public async Task<IActionResult> GetItemById(Guid id)
        {
            try
            {
                ...
                var item = await _context.Items
                    ...
                    .Include(i => i.ItemCompositions)
                        .ThenInclude(ic => ic.Composition)
                    .Include(i => i.StockEntries) // Include stock entries
                    .Include(i => i.InitialOpeningStock)
                        .ThenInclude(ios => ios!.InitialFiscalYear)
                    .Include(i => i.OpeningStocksByFiscalYear)
                        .ThenInclude(os => os.FiscalYear)
                ...
                // Get opening stock from ItemOpeningStockByFiscalYear for current fiscal year
                ...
                // 10. ✅ CORRECTED: Check stock availability from StockEntry table
                // Check if there are any stock entries with quantity > 0 for this item and company
                var hasStockEntries = await _context.StockEntries
                    .AnyAsync(se => se.ItemId == id && se.CompanyId == companyIdGuid && se.Quantity > 0);

                // Combined stock availability check - TRUE if ANY stock exists
                var hasStockAvailable = hasStockEntries;

                _logger.LogInformation($"Stock availability check for item {id}: " +
                    $"StockEntries={hasStockEntries} " +
                    $"Total={hasStockAvailable}");

                // 11. Process stock entries with null checks
                var stockEntries = (item.StockEntries ?? new List<StockEntry>())
                    .Select(entry => new
                    {
                        ...
                        entry.CreatedAt,
                        entry.UpdatedAt,
                        BarcodeData = $"{companyName}|{item.UniqueNumber}|{entry.Mrp}|{entry.BatchNumber ?? ""}|{entry.ExpiryDate.ToString("yyyy-MM-dd") ?? ""}"
                    })
                    .ToList();

                // 12. Get barcode preferences
                ...
                // 15. Prepare compositions - Check for null
                ...
                // 16. Prepare opening stocks by fiscal year - Check for null
                ...
                // 17. Prepare the response with detailed stock info
                var responseData = new
                {
                    success = true,
                    data = new
                    {
                        ...
                        hasStockAvailable = hasStockAvailable, // ✅ This is the key field
                        stockInfo = new
                        {
                            openingStock,
                            openingStockValue,
                ...
                _logger.LogInformation($"Has Stock Available: {hasStockAvailable}");
        ...
        public async Task<IActionResult> SearchItems([FromQuery] DateTime? asOfDate = null)
        {
            try
            {
                ...
                _logger.LogInformation($"asOfDate parameter: {asOfDate}");
                ...
                var includeInactive = bool.TryParse(Request.Query["includeInactive"].FirstOrDefault(), out bool includeInactiveParam) ? includeInactiveParam : false;
                // 3. Validate required claims exist
                if (string.IsNullOrEmpty(userId) || !Guid.TryParse(userId, out Guid userIdGuid))
                {
                    _logger.LogError("Invalid or missing userId claim");
                ...
                if (!includeInactive)
                {
                    ...
                    _logger.LogInformation("Filtering active items only (inactive items excluded)");
                }
                else
                {
                    _logger.LogInformation("Include inactive items = true");
                }

                // In the backend SearchItems method, add this filter after building the query
                if (asOfDate.HasValue)
                {
                    // Filter items created on or before the asOfDate
                    query = query.Where(i => i.Date <= asOfDate.Value);
                    _logger.LogInformation($"Filtering items created on or before: {asOfDate.Value}");
                }
                ...
                if (vatStatus != "all")
                {
                    if (vatStatus == "false")
                    {
                        query = query.Where(i => i.VatStatus == "13");
                    }
                    else
                    {
                ...
                // 12. Use AD date for filtering (no BS date conversion needed)
                DateTime? effectiveDate = asOfDate?.Date;

                // If no date provided, use current date
                if (!effectiveDate.HasValue)
                {
                    effectiveDate = DateTime.UtcNow.Date;
                    _logger.LogDebug($"No date provided, using current date: {effectiveDate}");
                }
                else
                {
                    _logger.LogDebug($"Using AD date filter: {effectiveDate}");
                }

                // 13. Process items to calculate stock as of the given date
                var itemsWithStock = items.Select(item =>
                {
                    // Filter stock entries based on AD date - DIRECTLY use Date field
                    ...
                    {
                        // Filter by AD date - NO parsing needed, just compare dates
                        filteredStockEntries = filteredStockEntries
                            .Where(se => se.Date.Date <= effectiveDate.Value);

                        _logger.LogDebug($"Filtering stock entries by AD date <= {effectiveDate.Value}");
                ...
                var response = new
                {
                    ...
                    filterInfo = new
                    {
                        dateFormatUsed = "english",
                        asOfDate = effectiveDate?.ToString("yyyy-MM-dd"),
                        includeInactive = includeInactive
                    }
                };

                _logger.LogInformation($"Successfully searched items. Found {itemsWithStock.Count} items. Filter date = {effectiveDate?.ToString("yyyy-MM-dd")}");
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        [HttpGet("items/search/items-ledger")]
        public async Task<IActionResult> SearchItemsInItemsLedger([FromQuery] DateTime? asOfDate = null)
        {
            try
            {
                _logger.LogInformation("=== SearchItems Started ===");
        ...
        public async Task<IActionResult> GetLastSalesPrice(Guid itemId)
        {
            try
            {
                ...
                var lastSalesBillItem = await _context.SalesBillItems
                    ...
                    .OrderByDescending(sbi => sbi.SalesBill.Date)
                    .FirstOrDefaultAsync();
