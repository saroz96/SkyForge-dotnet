...
namespace SkyForge.Controllers.Retailer
{
    ...
    public class AccountImportController : ControllerBase
    {
        ...
        [HttpPost("accounts-import")]
        [Consumes("multipart/form-data")]
        public async Task<IActionResult> ImportAccounts([FromForm] AccountImportRequestDto request)
        {
            try
            {
                var userId = User.FindFirst("userId")?.Value ?? User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
                ...
                // Process the file
                var result = await ProcessAccountImport(request.ExcelFile, companyIdGuid, currentFiscalYear.Id, userIdGuid);
