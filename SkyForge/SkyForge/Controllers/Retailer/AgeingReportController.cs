...
namespace SkyForge.Controllers.Retailer
{
    ...
    public class AgeingReportController : ControllerBase
    {
        ...
        // GET: api/retailer/day-count-aging
        [HttpGet("day-count-aging")]
        public async Task<IActionResult> GetDayCountAging([FromQuery] Guid? accountId = null, [FromQuery] string? fromDate = null, [FromQuery] string? toDate = null)
        {
            try
            {
                _logger.LogInformation("=== GetDayCountAging Started ===");
                _logger.LogInformation("FromDate: {FromDate}, ToDate: {ToDate}", fromDate, toDate);
                ...
                DateTime fromDateTime = DateTime.UtcNow.Date;
                DateTime toDateTime = DateTime.UtcNow.Date;

                if (!string.IsNullOrEmpty(fromDate) && accountId.HasValue)
                {
                    if (isNepaliFormat)
                    {
                        if (DateTime.TryParse(fromDate, out fromDateTime))
                        {
                            _logger.LogInformation("Using provided Nepali fromDate: {FromDate}", fromDate);
                        }
                        else
                        {
                            fromDateTime = DateTime.UtcNow.Date;
                            _logger.LogWarning("Invalid Nepali fromDate format: {FromDate}, using current date", fromDate);
                        }
                    }
                    else
                    {
                        if (DateTime.TryParse(fromDate, out fromDateTime))
                        {
                            _logger.LogInformation("Using provided English fromDate: {FromDate}", fromDate);
                        }
                        else
                        {
                            fromDateTime = DateTime.UtcNow.Date;
                            _logger.LogWarning("Invalid English fromDate format: {FromDate}, using current date", fromDate);
                        }
                    }
                }

                if (!string.IsNullOrEmpty(toDate) && accountId.HasValue)
                {
                    if (isNepaliFormat)
                    {
                        if (DateTime.TryParse(toDate, out toDateTime))
                        {
                            _logger.LogInformation("Using provided Nepali toDate: {ToDate}", toDate);
                        }
                        else
                        {
                            toDateTime = DateTime.UtcNow.Date;
                            _logger.LogWarning("Invalid Nepali toDate format: {ToDate}, using current date", toDate);
                        }
                    }
                    else
                    {
                        if (DateTime.TryParse(toDate, out toDateTime))
                        {
                            _logger.LogInformation("Using provided English toDate: {ToDate}", toDate);
                        }
                        else
                        {
                            toDateTime = DateTime.UtcNow.Date;
                            _logger.LogWarning("Invalid English toDate format: {ToDate}, using current date", toDate);
                        }
                    }
                }
                ...
                // If no accountId provided, return accounts list (WITHOUT date range)
                if (!accountId.HasValue)
                {
                    return Ok(new
                    {
                        ...
                        data = new
                        {
                            ...
                            currentFiscalYear = new
                            {
                                ...
                                startDate = await _context.FiscalYears.Where(f => f.Id == fiscalYearIdGuid).Select(f => f.StartDate).FirstOrDefaultAsync(),
                                endDate = await _context.FiscalYears.Where(f => f.Id == fiscalYearIdGuid).Select(f => f.EndDate).FirstOrDefaultAsync(),
                                startDateNepali = await _context.FiscalYears.Where(f => f.Id == fiscalYearIdGuid).Select(f => f.StartDateNepali).FirstOrDefaultAsync(),
                                endDateNepali = await _context.FiscalYears.Where(f => f.Id == fiscalYearIdGuid).Select(f => f.EndDateNepali).FirstOrDefaultAsync()
                            },
                            currentCompanyName = companyName,
                            hasDateFilter = false
                        }
                ...
                // Get ageing report data from service with date range
                var ageingData = await _ageingReportService.GetDayCountAgingForAccountAsync(
                    companyIdGuid,
                    fiscalYearIdGuid,
                    accountId.Value,
