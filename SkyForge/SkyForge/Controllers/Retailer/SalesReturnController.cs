...
namespace SkyForge.Controllers.Retailer
{
    ...
    public class SalesReturnController : ControllerBase
    {
        ...
        // GET: api/retailer/last-sales-return-date
        [HttpGet("last-sales-return-date")]
        public async Task<IActionResult> GetLastSalesReturnDate()
        {
            try
            {
                _logger.LogInformation("=== GetLastSalesReturnDate Started ===");
                ...
                IOrderedQueryable<SalesReturn> orderedQuery;
                ...
                var lastSalesReturn = await orderedQuery
                ...
                if (lastSalesReturn.Date != null)
                    dateString = lastSalesReturn.Date.ToString("yyyy-MM-dd");

                if (lastSalesReturn.NepaliDate != null)
                    nepaliDateString = lastSalesReturn.NepaliDate;

                if (lastSalesReturn.TransactionDate != null)
                    transactionDateString = lastSalesReturn.TransactionDate.ToString("yyyy-MM-dd");

                if (lastSalesReturn.TransactionDateNepali != null)
                    transactionDateNepaliString = lastSalesReturn.TransactionDateNepali;

                _logger.LogInformation($"Last purchase date found: Date={dateString}, NepaliDate={nepaliDateString}, Bill={lastSalesReturn.BillNumber}, IsNepaliFormat={isNepaliFormat}");
                ...
                return Ok(new
                {
                    ...
                    data = new
                    {
                        ...
                        billNumber = lastSalesReturn.BillNumber,
        ...
        [HttpGet("salesReturn-vat-report")]
        public async Task<IActionResult> GetSalesReturnVatReport([FromQuery] string? fromDate = null, [FromQuery] string? toDate = null)
        {
            try
            {
                _logger.LogInformation("=== GetSalesReturnVatReport Started ===");
                ...
                {
                    var emptyResponse = new SalesReturnVatReportDTO
                    {
                        Company = company,
                ...
                // Build query for sales returns - ALWAYS use Date field (AD dates) for filtering
                ...
                var response = new SalesReturnVatReportDTO
                {
                    Company = company,
                    CurrentFiscalYear = currentFiscalYear,
                ...
                _logger.LogInformation($"Successfully fetched sales return VAT report with {salesReturnVatReport.Count} entries");

                return Ok(new { success = true, data = response });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error in GetSalesReturnVatReport");
                return StatusCode(500, new
                {
                    success = false,
                    error = "Internal server error while fetching sales return VAT report",
                    details = Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT") == "Development" ? ex.Message : null
                });
            }
        }
