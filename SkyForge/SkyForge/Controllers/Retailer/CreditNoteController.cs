            ...
            try
            {
                ...
                var result = await executionStrategy.ExecuteAsync(async () =>
                {
                    ...
                    try
                    {
                        ...
                        // Find the credit note by bill number and company with entries and accounts
                        var creditNote = await _context.CreditNotes
                        ...
                        if (cashEntries.Any())
                        {
                            try
                            {
                                await _cashCounterService.RemoveSessionFromCreditNoteAsync(creditNote.Id);
                                _logger.LogInformation($"Removed credit note {creditNote.BillNumber} from cash counter session");
                            }
                            catch (Exception ex)
                            {
                                _logger.LogError(ex, $"Error removing credit note {creditNote.BillNumber} from cash counter");
            ...
            try
            {
                ...
                var result = await executionStrategy.ExecuteAsync(async () =>
                {
                    ...
                    try
                    {
                        ...
                        // STEP 2: ADD REACTIVATED CREDIT NOTE TO CASH COUNTER if it has cash entries
                        var cashEntries = creditNote.CreditNoteEntries
                            .Where(cne => cne.Account.Name?.ToLower() == "cash in hand")
                            .ToList();
                        ...
                        if (cashEntries.Any())
                        {
                            try
                            {
                                ...
                                _logger.LogInformation($"Added reactivated credit note {creditNote.BillNumber} to cash counter session");
                            }
                            catch (Exception ex)
                            {
