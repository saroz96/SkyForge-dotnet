    ...
    public class AccountBalanceService : IAccountBalanceService
    {
        ...
        public async Task<AccountBalanceDataDTO> CalculateAccountBalanceAsync(Guid accountId, Guid companyId, Guid fiscalYearId)
        {
            try
            {
                ...
                // Get opening balance for the fiscal year from OpeningBalanceByFiscalYear
                ...
                if (fiscalYear == null)
                {
                    return new AccountBalanceDataDTO
                    {
                        Balance = openingBalanceAmount,
                        BalanceType = openingBalanceType,
                        RawBalance = openingBalanceType == "Dr" ? openingBalanceAmount : -openingBalanceAmount,
                        OpeningBalance = openingBalanceAmount,
                        OpeningBalanceType = openingBalanceType,
                        OpeningBalanceId = openingBalance?.Id,
                        OpeningBalanceDate = openingBalance?.Date,
                        OpeningBalanceNepaliDate = openingBalance?.NepaliDate
                    };
                }
        ...
        // Method 2: GetAccountBalanceAsync (More comprehensive version)
        public async Task<AccountBalanceResponseDTO> GetAccountBalanceAsync(Guid accountId, Guid companyId, Guid fiscalYearId)
        {
            try
            {
                ...
                // Fetch the account details with OpeningBalanceByFiscalYear included
                var account = await _context.Accounts
                    .Include(a => a.OpeningBalanceByFiscalYear)
                    .FirstOrDefaultAsync(a => a.Id == accountId &&
                                              a.CompanyId == companyId &&
                ...
                // Fetch opening balance for the specific fiscal year from OpeningBalanceByFiscalYear
                var openingBalanceRecord = await _context.OpeningBalanceByFiscalYear
                    .FirstOrDefaultAsync(ob => ob.AccountId == accountId &&
                                               ob.FiscalYearId == fiscalYearId &&
                                               ob.CompanyId == companyId);

                // Calculate opening balance (Dr = positive, Cr = negative)
                decimal openingBalance = 0;
                if (openingBalanceRecord != null)
                {
                    openingBalance = openingBalanceRecord.Type == "Dr"
                        ? openingBalanceRecord.Amount
                        : -openingBalanceRecord.Amount;
                }

                _logger.LogInformation("Opening balance for Account {AccountId}, FiscalYear {FiscalYearId}: {OpeningBalance} ({Type})",
                    accountId, fiscalYearId, openingBalanceRecord?.Amount ?? 0, openingBalanceRecord?.Type ?? "N/A");
                ...
                // Build the DTO with opening balance details
                var result = new AccountBalanceDataDTO
                {
                    Balance = absoluteBalance,
                    BalanceType = balanceType,
                    RawBalance = balance,
                    ...
                    OpeningBalance = openingBalanceRecord?.Amount ?? 0,
                    OpeningBalanceType = openingBalanceRecord?.Type ?? "Dr",

                    // Optional: Add more details if your DTO supports them
                    // OpeningBalanceId = openingBalanceRecord?.Id,
                    // OpeningBalanceDate = openingBalanceRecord?.Date,
                    // OpeningBalanceNepaliDate = openingBalanceRecord?.NepaliDate
                };
