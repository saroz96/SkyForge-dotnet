...
//                 // 1. Calculate and save CLOSING stock for SOURCE fiscal year (current fiscal year)
...
//                     // 2. Create OPENING stock for TARGET fiscal year (new fiscal year) based on source closing stock
...
//                 // 4. Create Opening Balance Transaction for TARGET fiscal year
...
//                 _logger.LogInformation("Fiscal year transfer completed successfully. Opening balance transaction ID: {TransactionId}",
...
//                     existingRecord.Date = transferDate;
...
//         /// <summary>
//         /// Create OPENING stock for TARGET fiscal year based on SOURCE fiscal year's closing stock
//         /// </summary>
...
//                 else
//                 {
//                     // Create new opening stock record for TARGET fiscal year
...
//                 // Check OpeningBalanceByFiscalYear table
...
//             // Save to OpeningBalanceByFiscalYear table for TARGET fiscal year
//             foreach (var balance in closingBalances.Where(b => b.DebitAmount > 0 || b.CreditAmount > 0))
//             {
...
//                 AccountGroupsId = defaultGroup?.Id ?? Guid.Empty,
//                 CompanyId = companyId,
//                 OpeningBalanceType = "Dr",
//                 IsActive = true,
//                 Date = DateTime.UtcNow,
//                 NepaliDate = DateTime.UtcNow,
//                 CreatedAt = DateTime.UtcNow,
//                 UpdatedAt = DateTime.UtcNow
//             };
...
//                 // 4. Add Stock in Hand account balance to closing balances
...
//                 // Update accounts summary with stock in hand
//                 summary.AccountsSummary = new AccountTransferSummaryDto
//                 {
//                     AccountsProcessed = closingBalances.Count(a => a.DebitAmount > 0 || a.CreditAmount > 0),
//                     TotalDebitBalance = closingBalances.Sum(a => a.DebitAmount),
...
//         /// <summary>
//         /// Add Stock in Hand account balance to the closing balances
//         /// </summary>
...
//             // Get or create Stock in Hand account
//             var stockInHandAccount = await GetOrCreateStockInHandAccountAsync(companyId);

//             if (stockInHandAccount == null)
//             {
//                 _logger.LogWarning("Stock in Hand account not found or could not be created");
//                 return;
//             }
...
//         /// <summary>
//         /// Get or create Stock in Hand account under Current Assets group
//         /// </summary>
//         private async Task<Account> GetOrCreateStockInHandAccountAsync(Guid companyId)
//         {
//             // First, try to find existing Stock in Hand account
...
//             // Get Current Assets account group
//             var currentAssetsGroup = await _context.AccountGroups
//                 .FirstOrDefaultAsync(g => g.CompanyId == companyId && 
//                                          g.Name == AccountGroup.AccountGroupTypes.CurrentAssets);

//             if (currentAssetsGroup == null)
//             {
//                 _logger.LogWarning("Current Assets account group not found for company {CompanyId}", companyId);
//                 return null;
//             }

//             // Create new Stock in Hand account
//             stockInHandAccount = new Account
//             {
//                 Id = Guid.NewGuid(),
//                 Name = "Stock in Hand",
//                 AccountGroupsId = currentAssetsGroup.Id,
//                 CompanyId = companyId,
//                 OpeningBalanceType = "Dr",
//                 IsActive = true,
...
//             _logger.LogInformation("Created new Stock in Hand account for company {CompanyId}", companyId);
    ...
    Guid companyId)
{
    ...
    try
    {
        ...
        DateTime transferDateNepaliDateTime;
        
        // Check if TransferDateNepali is a string (which it is based on the DTO)
        if (!string.IsNullOrEmpty(request.TransferDateNepali?.ToString()))
        {
            // Parse the string to DateTime (adjust format as needed)
            // Option 1: Simple parse (if format matches)
            transferDateNepaliDateTime = DateTime.Parse(request.TransferDateNepali.ToString());
            
            // Option 2: TryParse with specific format
            // if (!DateTime.TryParseExact(request.TransferDateNepali.ToString(), "yyyy-MM-dd", 
            //     CultureInfo.InvariantCulture, DateTimeStyles.None, out transferDateNepaliDateTime))
            // {
            //     transferDateNepaliDateTime = request.TransferDate; // fallback to TransferDate
            // }
        }
        else
        {
            // Default to TransferDate if Nepali date is not provided
            transferDateNepaliDateTime = request.TransferDate;
        }
        ...
        if (request.TransferItems)
        {
            var itemSummary = await CalculateAndSaveClosingStockForSourceFiscalYearAsync(
                ...
                companyId,
                request.TransferDate,
                transferDateNepaliDateTime);  // Use converted DateTime
    ...
    return response;
        ...
        private async Task<Account?> GetOrCreateStockInHandAccountAsync(Guid companyId)
        {
            ...
            stockInHandAccount = new Account
            {
                ...
                IsDefaultAccount = true,
                Date = DateTime.UtcNow,
                NepaliDate = DateTime.UtcNow.ToString(), // Convert DateTime to string
                CreatedAt = DateTime.UtcNow,
                UpdatedAt = DateTime.UtcNow
            };
            ...
            DateTime transferDateNepali)
        {
            ...
            foreach (var item in items)
            {
                ...
                if (existingClosingStocks.TryGetValue(item.Id, out var existingRecord))
                {
                    ...
                    existingRecord.SalesPrice = item.Price ?? avgPurchasePrice;
                    existingRecord.Date = transferDate;
                    existingRecord.NepaliDate = transferDateNepali; // Convert DateTime to string
                    existingRecord.UpdatedAt = DateTime.UtcNow;
                }
                else
                {
                    ...
                    var closingStock = new ItemClosingStockByFiscalYear
                    {
                        ...
                        SalesPrice = item.Price ?? avgPurchasePrice,
                        Date = transferDate,
                        NepaliDate = transferDateNepali, // Convert DateTime to string
                        CreatedAt = DateTime.UtcNow,
                        UpdatedAt = DateTime.UtcNow
                    };
                    _context.ItemClosingStockByFiscalYear.Add(closingStock);
            ...
            List<AccountBalanceSummaryDto> closingBalances)
        {
            ...
            var openingBalanceTransaction = new Transaction
            {
                ...
                BillNumber = billNumber,
                Date = transferDate,
                NepaliDate = transferDateNepali.ToString(), // Convert DateTime to string
                TransactionDateNepali = transferDateNepali.ToString(), // Convert DateTime to string
                TotalDebit = totalDebit,
                TotalCredit = totalCredit,
                Status = TransactionStatus.Active,
                IsActive = true,
