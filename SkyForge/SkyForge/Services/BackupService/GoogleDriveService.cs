...
namespace SkyForge.Services.BackupService
{
    public class GoogleDriveService
    {
        ...
        public GoogleDriveService(IWebHostEnvironment environment)
        {
            ...
            // Use file-based data store for tokens (works on VPS)
            var tokenPath = Path.Combine(_environment.ContentRootPath, TokenFolderPath);
            _dataStore = new FileDataStore(tokenPath, true);
        }
        ...
        public string GetCredentialsFilePath()
        {
            if (_environment.IsProduction())
            {
                var webCredsPath = Path.Combine(_environment.ContentRootPath, "credentials_web.json");
                if (File.Exists(webCredsPath))
                {
                    return webCredsPath;
                }
            }
            return Path.Combine(_environment.ContentRootPath, "credentials.json");
        }
        ...
        /// </summary>
        public async Task<bool> ExchangeCodeForTokensAsync(string userId, string code, string redirectUri)
        {
            try
            {
                var flow = new GoogleAuthorizationCodeFlow(new GoogleAuthorizationCodeFlow.Initializer
                ...
                {
                    Console.WriteLine($"Token saved for user: {userId}");
                    return true;
                }
        ...
        /// <summary>
        /// Get authenticated Drive service for a specific user using stored token
        /// </summary>
        public async Task<DriveService> GetDriveServiceAsync(string userId)
        {
            if (string.IsNullOrEmpty(userId))
            ...
            if (token == null)
            {
                throw new Exception($"No stored token found for user: {userId}. User needs to authenticate first.");
            }

            var credential = new UserCredential(flow, userId, token);
            
            // Check if token is expired using SystemClock
        ...
        /// <summary>
        /// Get or create backup folder for user
        /// </summary>
        public async Task<string> GetOrCreateBackupFolderAsync(string userId, string folderName = "Ams_Backups")
        {
            var service = await GetDriveServiceAsync(userId);
