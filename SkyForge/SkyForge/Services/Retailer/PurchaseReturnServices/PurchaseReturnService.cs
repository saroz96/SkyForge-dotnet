    ...
    public class PurchaseReturnService : IPurchaseReturnService
    {
        ...
        public async Task<PurchaseReturn> CreatePurchaseReturnAsync(CreatePurchaseReturnDTO dto, Guid userId, Guid companyId, Guid fiscalYearId)
        {
            ...
            try
            {
                ...
                var purchaseReturn = new PurchaseReturn
                {
                    ...
                    TotalCcAmount = dto.TotalCcAmount,
                ...
                if (dto.AccountId != Guid.Empty && totalPartyDebit > 0)
                {
                    var partyTransaction = new Transaction
                    {
                        ...
                        NonTaxableAmount = dto.NonVatPurchaseReturn ?? 0,
                        VatPercentage = dto.VatPercentage,
                ...
                if (dto.PaymentMode?.ToLower() == "cash" && dto.TotalAmount > 0)
                {
                    try
                    {
                        await _cashCounterService.UpdateSessionFromPurchaseReturnAsync(purchaseReturn.Id);
                    }
                    catch (Exception ex)
                    {
                        // Log but don't fail the purchase return creation
                        _logger.LogError(ex, $"Error adding purchase return {purchaseReturn.BillNumber} to cash counter");
                    }
                }
        ...
        public async Task<PurchaseReturnRegisterDataDTO> GetPurchaseReturnRegisterAsync(Guid companyId, Guid fiscalYearId, string? fromDate = null, string? toDate = null)
        {
            try
            {
                ...
                // Check if there are any purchase returns for this company and fiscal year
                var totalBillsCount = await _context.PurchaseReturns
                    .CountAsync(pb => pb.CompanyId == companyId);

                _logger.LogInformation("Total purchase returns for company {CompanyId} and fiscal year {FiscalYearId}: {Count}",
                ...
                if (purchaseReturns.Count == 0)
                {
                    ...
                    _logger.LogInformation("Sample of recent purchase returns (Date vs NepaliDate): {SampleBills}",
            ...
            {
                _logger.LogError(ex, "Error getting purchase return register for company {CompanyId}", companyId);
                throw;
            }
        }
        ...
        private PurchaseReturnResponseDTO MapToResponseDTO(PurchaseReturn purchaseReturn, string companyDateFormat)
        {
            ...
            return new PurchaseReturnResponseDTO
            {
                ...
                Items = purchaseReturn.Items.Select(i => new PurchaseReturnItemResponseDTO
                {
                    ...
                    CcPercentage = i.CcPercentage,        // ADD THIS - Fetch from DB
                    ItemCcAmount = i.ItemCcAmount,
                    MainUnitPuPrice = i.MainUnitPuPrice,  // ADD THIS
                    BillQty = i.BillQty,                  // ADD THIS
        ...
        public async Task<PurchaseReturn> UpdatePurchaseReturnAsync(Guid id, UpdatePurchaseReturnDTO dto, Guid companyId, Guid fiscalYearId, Guid userId)
        {
            ...
            try
            {
                ...
                // STEP 1: REMOVE OLD PURCHASE RETURN FROM CASH COUNTER (if payment mode was cash)
                if (existingBill.PaymentMode?.ToLower() == "cash" && existingBill.TotalAmount > 0)
                {
                    try
                    {
                        await _cashCounterService.RemoveSessionFromPurchaseReturnAsync(id);
                        _logger.LogInformation($"Removed purchase return {existingBill.BillNumber} from cash counter session");
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError(ex, $"Error removing purchase return {existingBill.BillNumber} from cash counter");
                ...
                // STEP 2: RESTORE STOCK for all existing items (reverse the previous return)
                ...
                // STEP 6: PROCESS NEW ITEMS - Create new items and reduce stock
                decimal overallDiscountPercentage = dto.DiscountPercentage ?? 0m;
                ...
                foreach (var itemDto in dto.Items)
                {
                    ...
                    var newItem = new PurchaseReturnItem
                    {
                        ...
                        CcPercentage = itemDto.CcPercentage ?? 0,
                        ItemCcAmount = itemDto.ItemCcAmount ?? 0,
                        Date = dto.Date,
                        TransactionDate = dto.TransactionDate,
                        NepaliDate = dto.NepaliDate,
                        TransactionDateNepali = dto.TransactionDateNepali
                ...
                if (dto.AccountId != Guid.Empty && totalPartyDebit > 0)
                {
                    var partyTransaction = new Transaction
                    {
                        ...
                        TotalCredit = 0,
                        TaxableAmount = dto.TaxableAmount,
                        NonTaxableAmount = dto.NonVatPurchaseReturn,
                        VatPercentage = dto.VatPercentage,
                        VatAmount = dto.VatAmount,
                        PaymentMode = paymentMode,
                        Date = dto.Date,
                ...
                if (paymentMode == PaymentMode.Cash && cashAccount != null && dto.TotalAmount > 0)
                {
                    var cashTransaction = new Transaction
                    {
                        ...
                        Type = TransactionType.PrRt,
                        PurchaseSalesReturnType = "Purchase Return",
                        TotalDebit = dto.TotalAmount.Value,
                        TotalCredit = 0,
                        PaymentMode = paymentMode,
                        Date = dto.Date,
                        TransactionDate = dto.TransactionDate,
                ...
                // STEP 8: ADD UPDATED PURCHASE RETURN TO CASH COUNTER (if payment mode is cash)
                if (dto.PaymentMode?.ToLower() == "cash" && dto.TotalAmount > 0)
                {
                    try
                    {
                        await _cashCounterService.UpdateSessionFromPurchaseReturnAsync(existingBill.Id);
                        _logger.LogInformation($"Added updated purchase return {existingBill.BillNumber} to cash counter session");
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError(ex, $"Error adding purchase return {existingBill.BillNumber} to cash counter");
                        // Continue with update even if cash counter fails
                    }
                }
            ...
            try
            {
                ...
                var response = new PurchaseReturnPrintDTO
                {
                    ...
                    Bill = new PurchaseReturnPrintBillDTO
                    {
                        ...
                        Date = purchaseReturnBill.Date,
                        TransactionDate = purchaseReturnBill.TransactionDate,
                        NepaliDate = purchaseReturnBill.NepaliDate,
                        TransactionDateNepali = purchaseReturnBill.TransactionDateNepali,
                        ...
                        TotalCcAmount = purchaseReturnBill.TotalCcAmount,
        ...
        // public async Task<PurchaseReturnVatReportDTO> GetPurchaseReturnVatReportAsync(Guid companyId, Guid fiscalYearId, string? fromDate, string? toDate)
        // {
        //     try
        //     {
        //         _logger.LogInformation("GetPurchaseReturnVatReportAsync called for Company: {CompanyId}, FiscalYear: {FiscalYearId}, FromDate: {FromDate}, ToDate: {ToDate}",
        //             companyId, fiscalYearId, fromDate, toDate);
        ...
        //             return new PurchaseReturnVatReportDTO
        //             {
        //                 Company = company,
        //                 CurrentFiscalYear = currentFiscalYear,
        ...
        //         // Check if there are any purchase returns for this company and fiscal year
        //         var totalReturnsCount = await _context.PurchaseReturns
        //             .CountAsync(pr => pr.CompanyId == companyId && pr.FiscalYearId == fiscalYearId);

        //         _logger.LogInformation("Total purchase returns for company {CompanyId} and fiscal year {FiscalYearId}: {Count}",
        //             companyId, fiscalYearId, totalReturnsCount);
        ...
        //         // Get purchase returns ordered by date and bill number
        //         var purchaseReturns = await query
        //             .OrderBy(pr => pr.Date)
        ...
        //     catch (Exception ex)
        //     {
        //         _logger.LogError(ex, "Error in GetPurchaseReturnVatReportAsync for Company: {CompanyId}", companyId);
        //         throw;
        //     }
        // }
