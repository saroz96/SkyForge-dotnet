    ...
    public class StockAdjustmentService : IStockAdjustmentService
    {
        ...
        public async Task<StockAdjustmentsRegisterDataDTO> GetStockAdjustmentsRegisterAsync(Guid companyId, Guid fiscalYearId, string? fromDate = null, string? toDate = null)
        {
            try
            {
                ...
                _logger.LogInformation("Searching for stock adjustments between {StartDate} and {EndDate} (AD dates)",
                    startDateTime, endDateTime);

                // Build query for stock adjustments - ALWAYS use Date field (AD dates) for filtering
                var query = _context.StockAdjustments
                    .Include(sa => sa.Company)
                    .Include(sa => sa.User)
                    .Include(sa => sa.FiscalYear)
                ...
                // If no adjustments found, log sample for debugging
                if (stockAdjustments.Count == 0)
                {
                    var sampleAdjustments = await _context.StockAdjustments
                        .Where(sa => sa.CompanyId == companyId)
                        .OrderByDescending(sa => sa.Date)
                        .Take(5)
                        .Select(sa => new { sa.Id, sa.BillNumber, sa.Date, sa.NepaliDate })
                        .ToListAsync();

                    _logger.LogInformation("Sample of recent adjustments: {SampleAdjustments}",
                        string.Join(", ", sampleAdjustments.Select(sa => $"{sa.BillNumber} - Date: {sa.Date}, NepaliDate: {sa.NepaliDate}")));
                }
