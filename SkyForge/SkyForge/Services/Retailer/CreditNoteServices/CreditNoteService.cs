...
namespace SkyForge.Services.Retailer.CreditNoteServices
{
    public class CreditNoteService : ICreditNoteService
    {
        ...
        //             query = query.Where(c => string.Compare(c.NepaliDate, fromDate) >= 0
        //                                   && string.Compare(c.NepaliDate, toDate) <= 0);
        //         }
        //         else
        //         {
        ...
        public async Task<CreditNoteRegisterDataDTO> GetCreditNotesRegisterAsync(Guid companyId, Guid fiscalYearId, string? fromDate = null, string? toDate = null)
        {
            try
            {
                ...
                _logger.LogInformation("Searching for credit notes between {StartDate} and {EndDate} (AD dates)",
                ...
                // If no credit notes found, log sample for debugging
                if (creditNotes.Count == 0)
                {
                    var sampleCreditNotes = await _context.CreditNotes
                        .Where(c => c.CompanyId == companyId)
                        .OrderByDescending(c => c.Date)
                        .Take(5)
                        .Select(c => new { c.Id, c.BillNumber, c.Date, c.NepaliDate })
                        .ToListAsync();

                    _logger.LogInformation("Sample of recent credit notes (Date vs NepaliDate): {SampleCreditNotes}",
                        string.Join(", ", sampleCreditNotes.Select(c => $"{c.BillNumber} - Date: {c.Date}, NepaliDate: {c.NepaliDate}")));
                }
