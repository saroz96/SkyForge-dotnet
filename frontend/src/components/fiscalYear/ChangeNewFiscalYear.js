...
const ChangeNewFiscalYear = () => {
    ...
    const getNextFiscalYearDates = () => {
        ...
        if (isNepaliFormat) {
            // Use your EXISTING Nepali date calculation logic (which works correctly)
            ...
            if (currentEndDateNepali && currentEndDateNepali !== 'N/A') {
                try {
                    ...
                    if (endDate) {
                        ...
                        // CALCULATE ENGLISH DATES using the database English dates
                        // Get current fiscal year's English end date from database
                        const currentEnglishEndDate = fiscalData.currentFiscalYear.endDate;
                        let startEnglishDate = null;
                        let endEnglishDate = null;

                        if (currentEnglishEndDate) {
                            const endDateEnglish = new Date(currentEnglishEndDate);
                            // Next fiscal year start = current end date + 1 day
                            startEnglishDate = new Date(endDateEnglish);
                            startEnglishDate.setDate(endDateEnglish.getDate() + 1);

                            // Next fiscal year end = start date + 1 year - 1 day
                            endEnglishDate = new Date(startEnglishDate);
                            endEnglishDate.setFullYear(startEnglishDate.getFullYear() + 1);
                            endEnglishDate.setDate(startEnglishDate.getDate() - 1);
                        }

                        return {
                            start: startEnglishDate ? startEnglishDate.toISOString().split('T')[0] : null,
                            end: endEnglishDate ? endEnglishDate.toISOString().split('T')[0] : null,
            ...
            // Calculate English dates from current fiscal year's English dates
            const currentEnglishEndDate = fiscalData.currentFiscalYear.endDate;
            let startEnglishDate = null;
            let endEnglishDate = null;
            ...
            if (currentEndDate) {
                ...
                // Calculate Nepali dates using your existing logic
                let startNepali = null;
                let endNepali = null;

                if (fiscalData.currentFiscalYear.endDateNepali) {
                    try {
                        const endDateNepali = parseNepaliDate(fiscalData.currentFiscalYear.endDateNepali);
                        if (endDateNepali) {
                            const startNepaliDate = addDaysToNepaliDate(endDateNepali.year, endDateNepali.month, endDateNepali.day, 1);
                            let endYearCalc = startNepaliDate.year + 1;
                            let endMonthCalc = startNepaliDate.month;
                            let endDayCalc = startNepaliDate.day;
                            const endNepaliDateResult = subtractDaysFromNepaliDate(endYearCalc, endMonthCalc, endDayCalc, 1);

                            startNepali = formatNepaliDateString(startNepaliDate.year, startNepaliDate.month, startNepaliDate.day);
                            endNepali = formatNepaliDateString(endNepaliDateResult.year, endNepaliDateResult.month, endNepaliDateResult.day);
                        }
                    } catch (e) {
                        console.error('Error calculating Nepali dates:', e);
                    }
    ...
    const handleCreateFiscalYear = async () => {
        ...
        try {
            ...
            // Prepare request data - ALWAYS send both English and Nepali dates
            const requestData = {
                startDate: nextDates.start !== 'N/A' ? nextDates.start : null,
                endDate: nextDates.end !== 'N/A' ? nextDates.end : null,
                startDateNepali: nextDates.startNepali !== 'N/A' ? nextDates.startNepali : null,
                endDateNepali: nextDates.endNepali !== 'N/A' ? nextDates.endNepali : null
            };
