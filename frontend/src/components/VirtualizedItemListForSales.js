...
const ItemRow = memo(({ item, index, style, onItemClick, searchRef }) => {
  ...
  useEffect(() => {
    ...
    const fetchPriceIfNeeded = async () => {
      ...
      if (currentStock <= 0) {
        try {
          const response = await api.get(`/api/retailer/items/${item.id}/last-sales-price`);
...
const VirtualizedItemListForSales = memo(({
  items,
  onItemClick,
  searchRef,
  hasMore,
...
export default VirtualizedItemListForSales;
...
//   // Determine if item is vatable (13% VAT) - specifically check for string "13"
//   const isVatable = item.vatStatus === '13' || 
//                     item.vatStatus === 'vatable' || 
//                     item.vatStatus === 'false';
...
//     if (item.vatStatus === 'vatExempt' || item.vatStatus === 'exempt') {
...
//   const rowClasses = [
//     'dropdown-item',
//     item.vatStatus === '13' ? 'vat-13' : (isVatable ? 'vatable' : 'vatExempt'),
//     `expiry-${expiryStatus}`
//   ].filter(Boolean).join(' ');
