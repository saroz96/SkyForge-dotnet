...
const ClientDetails = () => {
  ...
  const renewalMonthsRef = useRef(null);
  ...
  const fetchClientDetails = async () => {
    try {
      ...
      const response = await api.get(`/api/client/clients/${id}/details`);
  ...
  // Helper function to add months to Nepali date
  const addMonthsToNepaliDate = (nepaliDateStr, months) => {
    try {
      if (!nepaliDateStr) return getCurrentNepaliDate();
      ...
      console.error('Error adding months to Nepali date:', error);
      return nepaliDateStr || getCurrentNepaliDate();
    }
  };
  ...
  const handleRenew = async () => {
    ...
    try {
      ...
      if (useCustomDate && customRenewalDate) {
        ...
        // Calculate new renewal date based on company's date format
        const dateFormat = client?.dateFormat?.toLowerCase() || 'english';
      ...
      const response = await api.post(
        `/client/clients/${id}/renew`,
        payload
      );

      if (response.data.success) {
        showToast(response.data.message || 'Subscription renewed successfully!', 'success');
        setShowRenewalModal(false);
        // Refresh client details
        await fetchClientDetails();
        // Reset form
  ...
  const getClientStatus = (clientData) => {
    if (!clientData) return 'demo';
    if (!clientData.renewalDate) return 'demo';
    try {
      const renewalDate = new Date(clientData.renewalDate);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `client_${client.name || 'unknown'}_details_${new Date().toISOString().split('T')[0]}.json`;
      a.click();
      URL.revokeObjectURL(url);
      showToast('Client details exported successfully', 'success');
    } catch (err) {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Client Details - ${client?.name || 'Client'}</title>
          <style>
            body { font-family: Arial, sans-serif; margin: 20px; }
            .header { text-align: center; margin-bottom: 30px; }
            .section { margin-bottom: 20px; }
        ...
        <body>
          ...
          <div class="section">
            <h2 class="section-title">Basic Information</h2>
            <table>
              <tr><th>Client Name</th><td>${client?.name || 'N/A'}</td></tr>
              <tr><th>Email</th><td>${client?.email || 'N/A'}</td></tr>
              <tr><th>Phone</th><td>${client?.phone || 'N/A'}</td></tr>
              <tr><th>PAN</th><td>${client?.pan || 'N/A'}</td></tr>
              <tr><th>Trade Type</th><td>${client?.tradeType || 'N/A'}</td></tr>
              <tr><th>Status</th><td>${clientStatus}</td></tr>
              <tr><th>Renewal Date</th><td>${client?.renewalDate || 'N/A'}</td></tr>
              <tr><th>Date Format</th><td>${client?.dateFormat || 'English'}</td></tr>
            </table>
          </div>
          
          <div class="section">
            <h2 class="section-title">Address Information</h2>
            <p>${client?.address || ''}${client?.city ? ', ' + client.city : ''}${client?.state ? ', ' + client.state : ''}${client?.country ? ', ' + client.country : ''}</p>
          </div>
          ...
          <div class="section">
            <h2 class="section-title">Owner Information</h2>
            <table>
              <tr><th>Name</th><td>${client.owner.name || 'N/A'}</td></tr>
              <tr><th>Email</th><td>${client.owner.email || 'N/A'}</td></tr>
              <tr><th>Phone</th><td>${client.owner.phone || 'N/A'}</td></tr>
            </table>
          </div>
          ` : ''}
          ...
          <div class="section">
            <h2 class="section-title">Statistics</h2>
            <table>
              <tr><th>Total Users</th><td>${client?.stats?.totalUsers || 0}</td></tr>
              <tr><th>Total Fiscal Years</th><td>${client?.stats?.totalFiscalYears || 0}</td></tr>
              <tr><th>Total Account Groups</th><td>${client?.stats?.totalAccountGroups || 0}</td></tr>
            </table>
          </div>
  ...
  const copyToClipboard = (text, field) => {
    if (!text) {
      showToast(`No ${field} to copy`, 'warning');
      return;
    }
    navigator.clipboard.writeText(text);
    showToast(`${field} copied to clipboard`, 'success');
    ...
    <Container fluid className="client-details-container py-4">
      ...
      <div className="d-flex justify-content-between align-items-start mb-4 flex-wrap gap-2">
        <div className="d-flex align-items-center gap-3">
          ...
          <div>
            <h1 className="mb-1 display-6">{client.name || 'Unnamed Client'}</h1>
            <div className="d-flex align-items-center gap-3 flex-wrap">
              ...
              <span className="text-muted">
                ...
                {client.dateFormat || 'English'} Calendar
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="d-flex justify-content-between align-items-center flex-wrap gap-2">
            <div className="d-flex align-items-center gap-3">
              ...
              <div>
                <div className="text-muted small">Current Renewal Date</div>
                <h4 className="mb-0">{formatDisplayDate(client.renewalDate)}</h4>
                {client.dateFormat?.toLowerCase() === 'nepali' && (
                  <div className="text-muted small">(Nepali Calendar)</div>
                )}
              </div>
            </div>
            <Badge bg={clientStatus === 'active' ? 'success' : clientStatus === 'expiring_soon' ? 'warning' : 'danger'} className="px-3 py-2">
              {clientStatus === 'active' ? 'Active' : clientStatus === 'expiring_soon' ? 'Expiring Soon' : 'Expired'}
            </Badge>
          </div>
        </Card.Body>
      </Card>
                ...
                <Col md={6}>
                  ...
                  <ListGroup variant="flush" className="border rounded">
                    ...
                    <ListGroup.Item className="d-flex justify-content-between align-items-center flex-wrap">
                      ...
                      <strong>{client.dateFormat || 'English'}</strong>
                    </ListGroup.Item>
                  </ListGroup>
                </Col>
                    ...
                    <ListGroup.Item>
                      ...
                      <div className="mt-1">
                        ...
                        {!client.address && !client.city && !client.state && !client.country && (
                          <span className="text-muted">No address information available</span>
                        )}
                      </div>
                    </ListGroup.Item>
                  </ListGroup>
                ...
                <Row>
                  <Col md={6}>
                    <ListGroup variant="flush" className="border rounded">
                      <ListGroup.Item className="d-flex justify-content-between align-items-center flex-wrap">
                        <span className="text-muted">Owner Name</span>
                        <strong>{client.owner.name || 'N/A'}</strong>
                      </ListGroup.Item>
                      <ListGroup.Item className="d-flex justify-content-between align-items-center flex-wrap">
                        <span className="text-muted">Email</span>
                        <a href={`mailto:${client.owner.email}`} className="text-decoration-none">
            ...
            <Tab.Pane eventKey="fiscal-years" active={activeTab === 'fiscal-years'}>
              {client.fiscalYears?.length > 0 ? (
                <Table responsive striped hover>
                  ...
                  <tbody>
                    {client.fiscalYears.map((fy) => (
                      <tr key={fy.id || Math.random()}>
                        <td><strong>{fy.name || 'N/A'}</strong></td>
                        <td>{fy.startDate ? formatDate(fy.startDate) : 'N/A'}</td>
                        <td>{fy.endDate ? formatDate(fy.endDate) : 'N/A'}</td>
                        <td>
                          {fy.isActive ? (
                ...
                <Col md={6}>
                  ...
                  <ListGroup variant="flush" className="border rounded">
                    ...
                    {client.attendanceSettings?.autoClockOut?.enabled && (
                      <ListGroup.Item className="d-flex justify-content-between align-items-center">
                        <span>Auto Clock-out Time</span>
                        <strong>{client.attendanceSettings.autoClockOut.time || '17:00'}</strong>
                      </ListGroup.Item>
                    )}
                  </ListGroup>
                </Col>
              ...
              {client.settings?.length > 0 ? (
                <Table responsive striped hover>
                  ...
                  <tbody>
                    {client.settings.map((setting, idx) => (
                      <tr key={idx}>
                        <td><code>{setting.key || 'N/A'}</code></td>
                        <td>{setting.value || 'N/A'}</td>
                      </tr>
                    ))}
                  </tbody>
                </Table>
          ...
          <Form>
            ...
            {!useCustomDate ? (
              <Form.Group className="mb-3">
                ...
                <Form.Text className="text-muted">
                  Subscription will be extended by {renewalMonths} months from current {client.dateFormat?.toLowerCase() === 'nepali' ? 'Nepali' : 'English'} date
                </Form.Text>
              </Form.Group>
            ) : (
              <Form.Group className="mb-3">
                <Form.Label className="fw-bold">
                  Custom Renewal Date ({client.dateFormat?.toLowerCase() === 'nepali' ? 'Nepali Calendar' : 'English Calendar'})
                </Form.Label>
                <Form.Control
                  type={client.dateFormat?.toLowerCase() === 'nepali' ? 'text' : 'date'}
                  value={customRenewalDate}
                  onChange={(e) => setCustomRenewalDate(e.target.value)}
                  placeholder={client.dateFormat?.toLowerCase() === 'nepali' ? 'YYYY-MM-DD (e.g., 2084-04-12)' : 'YYYY-MM-DD'}
                  ref={customDateRef}
                />
                <Form.Text className="text-muted">
                  {client.dateFormat?.toLowerCase() === 'nepali' 
