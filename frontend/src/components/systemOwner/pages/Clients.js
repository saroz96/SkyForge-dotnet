...
const Clients = () => {
  ...
  const handleExport = (format = 'csv') => {
    const exportData = filteredClients.map(client => ({
      ...
      'Trade Type': client.tradeType || 'N/A',
      'Status': getClientStatus(client),
      'Renewal Date': client.renewalDate || 'N/A',
      'Created At': formatDate(client.createdAt, 'short'),
      'Last Updated': formatDate(client.updatedAt, 'short'),
                      ...
                      <Table hover className="mb-0 clients-table">
                        <thead className="bg-light">
                          <tr>
                            ...
                            <th onClick={() => handleSort('tradeType')} className="sortable-header" style={{ cursor: 'pointer' }}>
                              <div className="d-flex align-items-center">
                                Trade Type
                                <SortIcon column="tradeType" />
                              </div>
                            ...
                            <th onClick={() => handleSort('renewalDate')} className="sortable-header" style={{ cursor: 'pointer' }}>
                              <div className="d-flex align-items-center">
                                Renewal
                                <SortIcon column="renewalDate" />
                              </div>
                        ...
                        <tbody>
                          {currentClients.map(client => {
                            ...
                            return (
                              <tr key={client.id || Math.random()} className="client-row">
                                <td>
                                  <Form.Check
                                    type="checkbox"
                                    checked={selectedClients.includes(client.id)}
            ...
            {viewMode === 'grid' && (
              <Row className="g-4">
                {currentClients.map(client => {
                  const status = getClientStatus(client);
                  return (
                    <Col key={client.id || Math.random()} lg={4} md={6}>
                      <Card className="client-grid-card h-100 shadow-sm border-0">
                        <Card.Body className="p-4">
                          <div className="d-flex justify-content-between align-items-start mb-3">
                            <div className="client-avatar-lg">
                          ...
                          <h5 className="mb-1">{client.name || 'Unnamed Client'}</h5>
                          <p className="text-muted small mb-3">{client.tradeType || 'Retailer'}</p>
