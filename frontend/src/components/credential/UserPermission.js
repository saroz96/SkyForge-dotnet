...
const UserPermission = () => {
    ...
    useEffect(() => {
        const fetchUserPermissions = async () => {
            try {
                setLoading(true);
                const response = await api.get(`/api/user/admin/users/user-permissions/${id}`);
                ...
                const errorMessage = err.response?.data?.error || 'Failed to load user permissions';
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await api.put(`/api/user/admin/users/user-permissions/${id}`, {
            ...
            if (response.data.success) {
                setNotification({
                    show: true,
                    message: response.data.data?.message || 'Permissions updated successfully',
                    type: 'success',
                    duration: 3000
                });
                ...
                // Reload after 1 second to show updated info
                setTimeout(() => {
            ...
            setNotification({
                show: true,
                message: err.response?.data?.error || err.response?.data?.message || 'Failed to update permissions',
                type: 'error',
                duration: 3000
            });
        }
            ...
            <div className="container mt-4">
                ...
                <div className="card shadow-sm border-0 mb-4">
                    <div className="card-header bg-primary text-white py-2">
                        <h5 className="mb-0">User Permissions</h5>
                    </div>
                    <div className="card-body p-3">
                        <div className="d-flex align-items-center">
                            <div 
                                className="user-avatar d-flex align-items-center justify-content-center rounded-circle me-3"
                                style={{ 
                                    width: '50px', 
                                    height: '50px', 
                            ...
                            <div>
                                ...
                                <div className="d-flex flex-wrap gap-2">
                                    <span className="badge bg-primary" style={{ fontSize: '0.7rem' }}>
                                        <FaUserTag className="me-1" size={10} /> {user?.role || 'User'}
                                    </span>
                                    <span className="badge bg-secondary" style={{ fontSize: '0.7rem' }}>
                                        <FaEnvelope className="me-1" size={10} /> {user?.email}
                                    </span>
                                </div>
                            </div>
                        </div>
                ...
                {isAdminRole || isOwner ? (
                    <div className="alert alert-info d-flex align-items-center" style={{ fontSize: '0.85rem' }}>
                        <FaInfoCircle className="me-3 fs-5" />
                        <div>
                            <h6 className="alert-heading mb-1">Administrative User Detected</h6>
                            <p className="mb-0 small">
                                This user has the <strong>{user?.role}</strong> role {isOwner && 'and is the Company Owner'} and has full access to all system features.
                                Permissions cannot be modified for users with this role.
                            </p>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="card shadow-sm border-0">
                            <div className="card-header bg-white d-flex justify-content-between align-items-center py-2">
                                <h5 className="mb-0">Menu Access Permissions</h5>
                                <div className="form-check form-switch">
                                    <input
                                        className="form-check-input"
                                        type="checkbox"
                                        id="selectAllPermissions"
                                        style={{ width: '2em', height: '1.2em' }}
                                        checked={Object.values(permissions).every(val => val === true)}
                                        onChange={(e) => {
                                            const newPermissions = {};
                                            Object.keys(permissions).forEach(key => {
                                                newPermissions[key] = e.target.checked;
                                    ...
                                    />
                                    <label className="form-check-label small" htmlFor="selectAllPermissions">
                                        Toggle All
                                    </label>
                                </div>
                            </div>
                            <div className="card-body p-3">
                                {permissionCategories.map(category => (
                                    <div key={category.id} className="mb-3">
                                        <div 
                                            className="d-flex justify-content-between align-items-center category-header p-2 bg-light rounded cursor-pointer"
                                            onClick={() => toggleSection(category.id)}
                                            style={{ cursor: 'pointer' }}
                                        >
                                            <h6 className="mb-0">{category.title}</h6>
                                            {expandedSections[category.id] ? <FaChevronUp size={12} /> : <FaChevronDown size={12} />}
                                        </div>
                                        ...
                                        {expandedSections[category.id] && (
                                            <div className="table-responsive mt-2">
                                                <table className="table table-sm table-hover mb-0" style={{ fontSize: '0.75rem' }}>
                                                    <thead className="table-light">
                                                        <tr>
                                                            <th style={{ padding: '6px 8px', width: '30%' }}>Permission</th>
                                                            <th style={{ padding: '6px 8px', width: '55%' }}>Description</th>
                                                            <th style={{ padding: '6px 8px', width: '15%' }} className="text-center">Access</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        {category.permissions.map(permission => (
                                                            <tr key={permission.id}>
                                                                <td style={{ padding: '6px 8px' }}>
                                                                    <label htmlFor={permission.id} className="mb-0">
                                                                        {permission.label}
                                                                    </label>
                                                                </td>
                                                                <td style={{ padding: '6px 8px' }}>{permission.description}</td>
                                                                <td className="text-center" style={{ padding: '6px 8px' }}>
                                                                    <div className="form-check form-switch d-inline-block">
                                                                        <input
                                                                            ...
                                                                            disabled={permission.id === 'dashboard'}
                                                                        />
                                                                        <label className="form-check-label ms-1" htmlFor={permission.id}>
                                                                            {permissions[permission.id] ? (
                                                                                <span className="text-success"><FaLockOpen size={12} /></span>
                                                                            ) : (
                                                                                <span className="text-danger"><FaLock size={12} /></span>
                                                                            )}
                                                                        </label>
                                                                    </div>
                                                                </td>
                                ...
                                <div className="d-flex justify-content-between align-items-center mt-3 pt-2 border-top">
                                    ...
                                    <button type="submit" className="btn btn-primary btn-sm px-3" style={{ height: '32px', fontSize: '0.8rem' }}>
                                        <FaSave className="me-1" size={12} /> Save Changes
                                    </button>
                                </div>
                            </div>
                        </div>
