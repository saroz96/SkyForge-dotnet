...
const AddSalesOpen = () => {
    const { salesOpenDraftSave, setSalesOpenDraftSave, clearSalesOpenDraft } = usePageNotRefreshContext();
    ...
    const [printAfterSave, setPrintAfterSave] = useState(
        localStorage.getItem('printAfterSaveSalesOpen') === 'true' || false
    );
    ...
    const [items, setItems] = useState(salesOpenDraftSave?.items || []);
    const [accounts, setAccounts] = useState(salesOpenDraftSave?.accounts || []);
    ...
    }, [formData, items, accounts, setSalesOpenDraftSave]);
    ...
    const fetchDatePreference = async () => {
        try {
            console.log('=== fetchDatePreferenceForSalesOpen CALLED ===');
            ...
            if (response.data.success) {
                ...
                console.log('useVoucherLastDateForSalesOpen value from API:', useVoucherDate);
    ...
    const fetchLastSalesDate = async () => {
        try {
            console.log('=== fetchLastSalesDateForOpen CALLED ===');

            // Use the endpoint: /api/retailer/last-sales-date
    ...
    const addItemToBill = async (item, batchInfo) => {
        ...
        // Transaction fetching logic for Sales Open
    ...
    // Reset after save - increments bill number (but we get it from the backend)
    ...
    const handleSubmit = async (e, print = false) => {
        ...
        try {
            ...
            if ((print || printAfterSave) && response.data.bill?._id) {
                setItems([]);
                setIsSaving(false);
            ...
            if (error.response?.data?.error) {
                ...
                if (errorMsg.includes('Cannot save VAT exempt bill with vatable items')) {
                    setNotification({
                        show: true,
                        message: 'Cannot save VAT exempt bill with vatable items',
                        type: 'error'
                    });
                } else if (errorMsg.includes('Cannot save bill with non-vatable items when VAT is applied')) {
                    setNotification({
                        show: true,
                        message: 'Cannot save bill with non-vatable items when VAT is applied',
                        type: 'error'
                    });
                } else if (errorMsg.includes('Not enough stock')) {
                    setNotification({
                        show: true,
                        message: errorMsg,
                        type: 'error'
    ...
    const handlePrintAfterSaveChange = (e) => {
        ...
        localStorage.setItem('printAfterSaveSalesOpen', isChecked);
    };
    ...
    }, [showTransactionModal, showAccountCreationModal, showItemsModal, handleTransactionModalClose]);
