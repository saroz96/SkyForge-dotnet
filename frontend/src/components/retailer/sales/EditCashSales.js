...
const EditCashSales = () => {
    ...
    const [printAfterSave, setPrintAfterSave] = useState(
        localStorage.getItem('printAfterSaveCashSales') === 'true' || false
    );
    ...
    useEffect(() => {
        const fetchBillData = async () => {
            try {
                ...
                const response = await api.get(`/api/retailer/cash-sales/edit/${id}`);
                ...
                // Find the matching account from accounts list
                const accountDoc = data.accounts?.find(acc => acc.name === salesBill.cashAccount);

                const itemsData = salesBill.items.map(item => ({
                    itemId: item.itemId,
                ...
                setFormData({
                    ...
                    transactionDateNepali: formatDate(salesBill.transactionDateNepali || salesBill.transactionDate),
                    transactionDateRoman: formatDate(salesBill.transactionDate || salesBill.transactionDateNepali),
                    nepaliDate: formatDate(salesBill.nepaliDate || salesBill.date),
                    billDate: formatDate(salesBill.date || salesBill.nepaliDate),
                    billNumber: salesBill.billNumber,
                    paymentMode: salesBill.paymentMode || 'cash',
                    isVatExempt: salesBill.isVatExempt ? 'true' : (salesBill.isVatAll === 'all' ? 'all' : 'false'),
                ...
                navigate('/retailer/sales-register');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    // Auto-focus on transaction date after data loads
    useEffect(() => {
    ...
    //     if (discountAmount > 0) {
    //         // If discount amount is manually set, use it directly
    //         effectiveDiscount = discountAmount;

    //         // Distribute discount proportionally between taxable and non-taxable
    //         if (subTotal > 0) {
    //             const taxableRatio = taxableAmount / subTotal;
    //             const nonTaxableRatio = nonTaxableAmount / subTotal;
    ...
    //         vatAmount: Math.round(vatAmount * 100) / 100,
    //         totalAmount: Math.round(totalAmount * 100) / 100,
    //         // Don't set discountAmount here when using percentage - let the user input control it
    //         discountAmount: discountAmount > 0 ? Math.round(effectiveDiscount * 100) / 100 : prev.discountAmount
    //     }));
    // };
    ...
    useEffect(() => {
        if (roundOffSales && !manualRoundOffOverride) {
            // Calculate the auto round-off value
            const subTotal = formData.subTotal || 0;
            const taxableAmount = formData.taxableAmount || 0;
            const vatAmount = formData.vatAmount || 0;

            let totalBeforeRoundOff = subTotal + vatAmount;

            // Add non-taxable amount if present
            const nonTaxableAmount = formData.nonTaxableAmount || 0;
            totalBeforeRoundOff += nonTaxableAmount;

            const roundedTotal = Math.round(totalBeforeRoundOff);
            const autoRoundOffAmount = Math.round((roundedTotal - totalBeforeRoundOff) * 100) / 100;
    ...
    }, [roundOffSales, manualRoundOffOverride, formData.subTotal, formData.taxableAmount, formData.vatAmount, formData.nonTaxableAmount]);
    ...
    const handleDiscountAmountChange = (e) => {
        ...
        // Validate that discount amount doesn't exceed subtotal
        ...
        setFormData(prev => ({
            ...prev,
            discountAmount: Math.round(validatedValue * 100) / 100,
    ...
    const handleAccountSearch = (e) => {
        ...
        // Update the form data with typed value
        setFormData(prev => ({
            ...prev,
            cashAccount: searchText,
            cashAccountId: '',
            cashAccountAddress: '',
            cashAccountPan: '',
            cashAccountEmail: '',
    ...
    }, [showAccountCreationModal, showItemsModal]);
    ...
    const handlePrintAfterSaveChange = (e) => {
        ...
        localStorage.setItem('printAfterSaveCashSales', isChecked);
    };
    ...
    const handleSubmit = async (e, print = false) => {
        ...
        try {
            ...
            if (print || printAfterSave) {
                if (response.data.data?.bill?._id) {
        ...
        try {
                    ...
                    <div class="print-invoice-container">
                        ...
                        <div class="print-invoice-details">
                            ...
                            <div>
                                ...
                                <div><strong>Trans. Date:</strong> ${printData.companyDateFormat === 'nepali' ? formatDateForInput(printData.transactionDateNepali, 'Nepali') : formatDateForInput(printData.bill.transactionDate)}(${new Date(printData.bill.transactionDate).toLocaleDateString()})</div>
                                <div><strong>Invoice Date:</strong> ${printData.companyDateFormat === 'nepali' ? formatDateForInput(printData.nepaliDate, 'Nepali') : formatDateForInput(printData.bill.date)}(${new Date(printData.bill.date).toLocaleDateString()})</div>
                            ...
                            <tbody>
                                ${printData.bill.items?.map((item, i) => `
                                    <tr key="${i}">
                                        ...
                                        <td>${item.hscode || ''}</td>
                                        <td>
                                            ${item.vatStatus === 'vatExempt' ? `${item.itemName || ''} *` : (item.itemName || '')}
                                        </td>
                                        <td>${item.unitName || ''}</td>
                                        <td>${item.batchNumber || ''}</td>
                                        <td>${item.expiryDate ? new Date(item.expiryDate).toLocaleDateString() : 'N/A'}</td>
                                        <td>${item.quantity || 0}</td>
                                        <td>${formatTo2Decimal(item.price || 0)}</td>
                                        <td>${formatTo2Decimal((item.quantity || 0) * (item.price || 0))}</td>
                                    </tr>
                                `).join('') || ''}
                            </tbody>
                        </table>
                        ...
                        <table class="print-totals-table">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="print-text-right">${formatTo2Decimal(printData.bill.nonVatSales || 0)}</td>
                                </tr>
                                <tr>
            ...
            const styles = `
                @media print {
                    ...
                    .print-invoice-header { text-align: center; margin-bottom: 3mm; border-bottom: 1px solid #000; padding-bottom: 2mm; }
                    ...
                    .print-invoice-table thead { border-top: 1px solid #000; border-bottom: 1px solid #000; }
                    ...
                    <form onSubmit={handleSubmit} id="billForm" className="needs-validation" noValidate>
                        ...
                        <div className="row g-2 mb-3">
                            ...
                            {company.dateFormat === 'nepali' || company.dateFormat === 'Nepali' ? (
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    if (sanitizedValue.length <= 10) {
                                                        ...
                                                        if (sanitizedValue.length === 10 && /^\d{4}-\d{2}-\d{2}$/.test(sanitizedValue)) {
                                                            console.log('Invoice Date - Converting BS to AD:', sanitizedValue);
                                                            const adDate = convertBsToAd(sanitizedValue);
                                                            console.log('Invoice Date - Converted AD date:', adDate);
                                                            if (adDate) {
                                                                setFormData(prev => ({
                                                                    ...prev,
                                                                    billDate: adDate
                                ...
                                // English date format section (remains the same as your existing code)
                                <>
                                    <div className="col-12 col-md-6 col-lg-3">
                                        <div className="position-relative">
                                            <input
                                ...
                                <div className="position-relative">
                                    ...
                                    >
                                        <AccountBalanceDisplay
                                            ...
                                            api={api}
                                            newTransactionAmount={parseFloat(formData.totalAmount) || 0}
                                            compact={true}
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td style={{ width: '20%', padding: '1px' }}>
                                            <p className="form-control-plaintext mb-0" style={{ fontSize: '0.8rem' }}>Rs. {formData.subTotal.toFixed(2)}</p>
                                        </td>
                                        <td style={{ width: '15%', padding: '1px' }}>
                                    ...
                                    {company.vatEnabled && formData.isVatExempt !== 'true' && (
                                        <tr id="taxableAmountRow">
                                            ...
                                            <td style={{ padding: '1px' }}>
                                                <p className="form-control-plaintext mb-0" style={{ fontSize: '0.8rem' }}>Rs. {formData.taxableAmount.toFixed(2)}</p>
                                            </td>
                                            <td style={{ padding: '1px' }}>
                                            ...
                                            <td style={{ padding: '1px' }}>
                                                <p className="form-control-plaintext mb-0" style={{ fontSize: '0.8rem' }}>Rs. {formData.vatAmount.toFixed(2)}</p>
                                            </td>
                                        </tr>
                                    )}
                                    ...
                                    <tr>
                                            ...
                                            <div className="position-relative" style={{ minWidth: '150px' }}>
                                                <div className="input-group input-group-sm" style={{ flexWrap: 'nowrap' }}>
                                                    <input
                                                        ...
                                                        id="roundOffAmount"
                                                        name="roundOffAmount"
                                                        value={roundOffSales && !manualRoundOffOverride ?
                                                            (formData.autoRoundOffAmount !== undefined ? formData.autoRoundOffAmount.toFixed(2) :
                                                                (Math.round((formData.totalAmount - formData.roundOffAmount) * 100) / 100).toFixed(2)) :
                                                        ...
                                                        onFocus={(e) => {
                                                            e.target.select();
                                                            if (roundOffSales && !manualRoundOffOverride) {
                                                                // Calculate the expected total before round off
                                                                const totalBeforeRoundOff = (formData.totalAmount || 0) - (formData.roundOffAmount || 0);
                                                                const roundedTotal = Math.round(totalBeforeRoundOff);
                                                                const autoRoundOffAmount = Math.round((roundedTotal - totalBeforeRoundOff) * 100) / 100;
                                                                setFormData(prev => ({
                                                                    ...prev,
                                                                    roundOffAmount: autoRoundOffAmount,
                                                                    autoRoundOffAmount: autoRoundOffAmount
                                                                }));
                                                            }
                                                        }}
                                                        onBlur={(e) => {
                                                            if (roundOffSales && parseFloat(e.target.value) === (formData.autoRoundOffAmount ||
                                                                Math.round((formData.totalAmount - formData.roundOffAmount) * 100) / 100)) {
                                                        ...
                                                        <button
                                                            ...
                                                            onClick={() => {
                                                                if (manualRoundOffOverride) {
                                                                    ...
                                                                    setFormData(prev => ({
                                                                        ...prev,
                                                                        autoRoundOffAmount: prev.roundOffAmount
                                                                    }));
                                                                }
                                                            }}
                                        ...
                                        <td style={{ padding: '1px' }}>
                                            <p className="form-control-plaintext mb-0" style={{ fontSize: '0.8rem' }}>Rs. {formData.totalAmount.toFixed(2)}</p>
                                        </td>
                                        <td style={{ padding: '1px' }}>
                                        ...
                                        <td style={{ padding: '1px' }}>
                                            <div
                                                ...
                                                id="amountInWords"
                                                title={convertToRupeesAndPaisa(formData.totalAmount) + " Only."}
                                            >
                        ...
                        <div className="modal-content" style={{ height: '400px' }}>
                            ...
                            <div className="p-2 bg-white sticky-top">
                                <input
                                    ...
                                    id="searchAccount"
                                    className="form-control form-control-sm"
                                    placeholder="Type to search or enter new account name... (Press F6 to create new account)"
                                    autoFocus
                                    autoComplete='off'
                                    ...
                                    onKeyDown={(e) => {
                                        if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
                                            ...
                                            } else if (formData.cashAccount.trim()) {
                                                handleAccountModalClose();
                                                setTimeout(() => {
                                                    document.getElementById('cashAccountAddress').focus();
                                                }, 0);
                                ...
                                <div className="d-flex justify-content-between w-100 align-items-center">
                                    ...
                                    <div className="d-flex gap-2">
                                        <button
                                            ...
                                            disabled={!formData.cashAccount.trim()}
                                        >
                                            Use Entered Name
                                        </button>
                                        <button
