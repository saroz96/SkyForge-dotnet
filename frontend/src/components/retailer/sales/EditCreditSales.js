...
const EditCreditSales = () => {
    ...
    const [printAfterSave, setPrintAfterSave] = useState(
        localStorage.getItem('printAfterSaveCreditSales') === 'true' || false
    );
    ...
    useEffect(() => {
        const fetchBillData = async () => {
            try {
                ...
                // const accountDoc = data.accounts.find(acc => acc.id === salesBill.accountId);
                // const accountNameWithNumber = accountDoc?.uniqueNumber
                //     ? `${accountDoc.uniqueNumber} ${salesBill.accountName}`
                //     : salesBill.accountName;

                const accountDoc = data.accounts?.find(acc => acc.name === salesBill.cashAccount);
                const accountNameWithNumber = accountDoc?.uniqueNumber
                    ? `${accountDoc.uniqueNumber} ${salesBill.accountName || salesBill.cashAccount}`
                    : salesBill.accountName || salesBill.cashAccount || '';
                ...
                setFormData({
                    ...
                    accountAddress: salesBill.accountAddress || '',
                    accountPan: salesBill.accountPan || '',
    ...
    }, [showTransactionModal, showAccountCreationModal, showItemsModal]);
    ...
    const handlePrintAfterSaveChange = (e) => {
        ...
        localStorage.setItem('printAfterSaveCreditSales', isChecked);
    };
    ...
    const handleSubmit = async (e, print = false) => {
        ...
        try {
            // Prepare data according to UpdateCreditSalesBillDTO
            const billData = {
                // accountId: formData.accountId,
                ...accountData,
                paymentMode: formData.paymentMode,
            ...
            setNotification({
                show: true,
                message: error.response?.data?.error || 'Failed to update sales bill. Please try again.',
                type: 'error'
            });
        } finally {
            setIsSaving(false);
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td style={{ width: '20%', padding: '1px' }}>
                                            <p className="form-control-plaintext mb-0" style={{ fontSize: '0.8rem' }}>Rs. {formatter.format(formData.subTotal)}</p>
                                        </td>
                                        <td style={{ width: '15%', padding: '1px' }}>
                                    ...
                                    {company.vatEnabled && formData.isVatExempt !== 'true' && (
                                        <tr id="taxableAmountRow">
                                            ...
                                            <td style={{ padding: '1px' }}>
                                                <p className="form-control-plaintext mb-0" style={{ fontSize: '0.8rem' }}>Rs. {formatter.format(formData.taxableAmount)}</p>
                                            </td>
                                            <td style={{ padding: '1px' }}>
                                            ...
                                            <td style={{ padding: '1px' }}>
                                                <p className="form-control-plaintext mb-0" style={{ fontSize: '0.8rem' }}>Rs. {formatter.format(formData.vatAmount)}</p>
                                            </td>
                                        </tr>
                                    )}
                                    ...
                                    <tr>
                                        ...
                                        <td style={{ padding: '1px' }}>
                                            <p className="form-control-plaintext mb-0" style={{ fontSize: '0.8rem' }}>Rs. {formatter.format(formData.totalAmount)}</p>
                                        </td>
                                        {/* <td style={{ padding: '1px' }}>
                                        ...
                                        <td style={{ padding: '1px' }}>
                                            <div
                                                ...
                                                id="amountInWords"
                                                title={convertToRupeesAndPaisaNepali(formData.totalAmount) + " Only."}
                                            >
                                                {convertToRupeesAndPaisaNepali(formData.totalAmount)} Only.
                                            </div>
                                        </td> */}
