...
const AddSales = () => {
    ...
    const fetchItemsFromBackend = async (searchTerm = '', page = 1, isHeaderModal = false) => {
        try {
            ...
            // FIX: Always use the transaction date from the form (like AddPurchase)
            // For Sales, we need stock as of the transaction date
    ...
    //     if (totalStock === 0) {
    //         // No stock - show the Stock Adjustment Modal
    ...
    const selectItemForInsert = async (item) => {
        ...
        // Set batch and expiry from first stock entry
        if (item.stockEntries && item.stockEntries.length > 0) {
            const sortedStockEntries = [...(item.stockEntries || [])].sort((a, b) =>
                new Date(a.date) - new Date(b.date)
            );
    ...
    // Helper function to get stock entries sorted by date (FIFO - oldest first)
    const getFifoStockEntries = (item, requestedQuantity) => {
        if (!item.stockEntries || item.stockEntries.length === 0) {
            return [];
        }
        ...
        const availableEntries = sortedEntries.filter(entry => entry.quantity > 0);
        ...
        for (const entry of availableEntries) {
            ...
            batches.push({
                ...
                remainingStock: availableQty - takenQty
            });
