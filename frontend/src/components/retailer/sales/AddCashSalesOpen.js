...
const AddCashSalesOpen = () => {
    ...
    const [printAfterSave, setPrintAfterSave] = useState(
        localStorage.getItem('printAfterSaveCashSalesOpen') === 'true' || false
    );
    ...
    const getCurrentBillNumber = async () => {
        try {
            const response = await api.get('/api/retailer/cash-sales/open/current-number');
            return response.data.data.currentCashSalesOpenBillNumber;
        } catch (error) {
    ...
    //                         ...entry,
    //                         mrp: entry.mrp || 0 // Ensure MRP is present
    //                     }));
    //                 }
    ...
    // Reset after save - gets next bill number
    ...
    const handleSubmit = async (e, print = false) => {
        ...
        try {
            ...
            // Prepare data according to CreateCashSalesOpenDTO
            const billData = {
            ...
            if (error.response?.data?.error) {
                ...
                setNotification({
                    show: true,
                    message: 'Failed to save cash sales bill. Please try again.',
                    type: 'error'
                });
            }
            setIsSaving(false);
    ...
    const handlePrintAfterSaveChange = (e) => {
        ...
        localStorage.setItem('printAfterSaveCashSalesOpen', isChecked);
    };
    ...
    const printImmediately = async (billId) => {
        ...
        `;
            ...
            <html>
                <head>
                    <title>Open_Cash_Sales_Invoice_${printData.bill.billNumber}</title>
                    <style>${styles}</style>
                </head>
                <body>
                    ${tempDiv.innerHTML}
