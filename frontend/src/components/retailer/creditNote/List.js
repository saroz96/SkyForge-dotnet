...
const CreditNoteList = () => {
    ...
    const [dateRange, setDateRange] = useState(() => {
        if (draftSave?.creditNoteData) {
            return { fromDate: draftSave.creditNoteData.fromDate || '', toDate: draftSave.creditNoteData.toDate || '', fromDateAd: draftSave.creditNoteData.fromDateAd || '', toDateAd: draftSave.creditNoteData.toDateAd || '' };
        }
        return { fromDate: '', toDate: '', fromDateAd: '', toDateAd: '' };
    });

    const [creditNotes, setCreditNotes] = useState(() => draftSave?.creditNoteData?.creditNotes || []);
    const [companyInfo, setCompanyInfo] = useState(() => {
        if (draftSave?.creditNoteData) {
            return {
                company: draftSave.creditNoteData.company, currentFiscalYear: draftSave.creditNoteData.currentFiscalYear,
                currentCompanyName: draftSave.creditNoteData.currentCompanyName || '', companyDateFormat: draftSave.creditNoteData.companyDateFormat || 'english',
                vatEnabled: draftSave.creditNoteData.vatEnabled !== undefined ? draftSave.creditNoteData.vatEnabled : true,
                isAdminOrSupervisor: draftSave.creditNoteData.isAdminOrSupervisor || false
            };
        }
    ...
    const [searchQuery, setSearchQuery] = useState(() => draftSave?.creditNoteSearch?.searchQuery || '');
    const [selectedRowIndex, setSelectedRowIndex] = useState(() => draftSave?.creditNoteSearch?.selectedRowIndex || 0);

    const [columnWidths, setColumnWidths] = useState({
        bsDate: 80, adDate: 80, voucherNo: 100, creditAccounts: 150, credit: 80, debitAccounts: 150, debit: 80, description: 130, actions: 100
    });
    ...
    useEffect(() => {
        setDraftSave({
            ...draftSave,
            creditNoteData: { ...companyInfo, creditNotes: creditNotes, fromDate: dateRange.fromDate, toDate: dateRange.toDate, fromDateAd: dateRange.fromDateAd, toDateAd: dateRange.toDateAd },
            creditNoteSearch: { searchQuery, selectedRowIndex, fromDate: dateRange.fromDate, toDate: dateRange.toDate }
        });
    }, [creditNotes, searchQuery, selectedRowIndex, dateRange, companyInfo]);
    ...
    useEffect(() => localStorage.setItem('creditNoteTableColumnWidths', JSON.stringify(columnWidths)), [columnWidths]);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const response = await api.get(`/api/retailer/credit-notes/register?${params.toString()}`, { signal: abortController.signal });
                ...
                if (!draftSave?.creditNoteSearch?.selectedRowIndex) setSelectedRowIndex(0);
            } catch (err) {
                if (err.name !== 'AbortError') {
                    console.error('Fetch error:', err);
                    setError(err.response?.data?.error || 'Failed to fetch credit notes');
    ...
    useEffect(() => {
        const creditNotesArray = Array.isArray(creditNotes) ? creditNotes : [];
        const filtered = creditNotesArray.filter(creditNote => {
            const matchesSearch = (creditNote.billNumber?.toLowerCase() || '').includes(searchQuery.toLowerCase()) ||
                (creditNote.description?.toLowerCase() || '').includes(searchQuery.toLowerCase()) ||
                (creditNote.creditAccountNames?.some(name => name?.toLowerCase().includes(searchQuery.toLowerCase())) || false) ||
                (creditNote.debitAccountNames?.some(name => name?.toLowerCase().includes(searchQuery.toLowerCase())) || false) ||
                (creditNote.userName?.toLowerCase() || '').includes(searchQuery.toLowerCase());
    ...
    // Calculate totals
    useEffect(() => {
        if (filteredCreditNotes.length === 0) { setTotalCredit(0); setTotalDebit(0); return; }
        const newTotalCredit = filteredCreditNotes.reduce((acc, creditNote) => {
            if (creditNote.status !== 'Active') return acc;
            return acc + (creditNote.creditAmounts?.reduce((sum, amt) => sum + (amt || 0), 0) || 0);
        }, 0);
        const newTotalDebit = filteredCreditNotes.reduce((acc, creditNote) => {
            if (creditNote.status !== 'Active') return acc;
            return acc + (creditNote.debitAmounts?.reduce((sum, amt) => sum + (amt || 0), 0) || 0);
        }, 0);
        setTotalCredit(newTotalCredit);
        setTotalDebit(newTotalDebit);
    }, [filteredCreditNotes]);
    ...
    useEffect(() => {
        const handleKeyDown = (e) => {
            ...
            switch (e.key) {
                ...
                case 'ArrowDown': e.preventDefault(); setSelectedRowIndex(prev => Math.min(filteredCreditNotes.length - 1, prev + 1)); break;
                default: break;
            }
        };
        window.addEventListener('keydown', handleKeyDown);
    ...
    const handlePrint = (filtered = false) => {
        const rowsToPrint = filtered ? filteredCreditNotes : (Array.isArray(creditNotes) ? creditNotes : []);
        if (rowsToPrint.length === 0) { setNotification({ show: true, message: 'No credit notes to print', type: 'warning' }); return; }
        ...
        let tableContent = `
            ...
            <div class="report-title">Credit Note Register</div>
            <table><thead><tr><th>Miti</th><th>Date</th><th>Vch No.</th><th>Credit Accounts</th><th>Credit</th><th>Debit Accounts</th><th>Debit</th><th>Description</th></tr></thead><tbody>
        `;
        let printTotalCredit = 0;
        let printTotalDebit = 0;
        rowsToPrint.forEach(creditNote => {
            ...
            const debitAccountsDisplay = isCanceled ? 'Canceled' : (creditNote.debitAccountNames?.join(', ') || 'N/A');
            const debitAmountsDisplay = isCanceled ? '0.00' : (creditNote.debitAmounts?.map(amt => amt?.toFixed(2)).join(', ') || '0.00');
            tableContent += `<tr><td>${creditNote.nepaliDate || ''}</td><td>${creditNote.date ? new Date(creditNote.date).toLocaleDateString() : ''}</td><td>${creditNote.billNumber || ''}</td><td>${isCanceled ? '<span class="text-danger">Canceled</span>' : creditAccountsDisplay}</td><td class="text-end">${isCanceled ? '<span class="text-danger">0.00</span>' : creditAmountsDisplay}</td><td>${isCanceled ? '<span class="text-danger">Canceled</span>' : debitAccountsDisplay}</td><td class="text-end">${isCanceled ? '<span class="text-danger">0.00</span>' : debitAmountsDisplay}</td><td>${creditNote.description || ''}</td></tr>`;
            if (!isCanceled) {
                printTotalCredit += creditNote.creditAmounts?.reduce((sum, amt) => sum + (amt || 0), 0) || 0;
                printTotalDebit += creditNote.debitAmounts?.reduce((sum, amt) => sum + (amt || 0), 0) || 0;
            }
        });
        tableContent += `<tr class="grand-total-row"><td colspan="4">Grand Totals</td><td class="text-end">${printTotalCredit.toFixed(2)}</td><td></td><td class="text-end">${printTotalDebit.toFixed(2)}</td><td></td></tr></tbody></table>
            <script>window.onload=function(){window.print();window.onafterprint=function(){window.close()}}<\/script>
        `;
        printWindow.document.write(`<!DOCTYPE html><html><head><title>Credit Note Register</title></head><body>${tableContent}</body></html>`);
        printWindow.document.close();
    };
    ...
    const handleRowDoubleClick = useCallback(() => {
        if (filteredCreditNotes[selectedRowIndex]) navigate(`/retailer/credit-note/${filteredCreditNotes[selectedRowIndex].id}/print`);
    }, [navigate, filteredCreditNotes, selectedRowIndex]);
    ...
    const ResizeHandle = React.memo(({ onResizeStart, left, columnName }) => (
        <div className="cn-resize-handle" style={{ position: 'absolute', top: 0, left: `${left}px`, width: '5px', height: '100%', cursor: 'col-resize', zIndex: 10 }} onMouseDown={(e) => { e.preventDefault(); onResizeStart(e, columnName); }} />
    ));
    ...
    const TableHeader = React.memo(() => {
        ...
        return (
            <div className="cn-header" style={{ minWidth: `${totalWidth}px` }}
            ...
            >
                <div className="cn-header-cell cn-cell--center" style={{ width: `${columnWidths.bsDate}px`, flexShrink: 0 }}>Miti<ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.bsDate - 2} columnName="bsDate" /></div>
                <div className="cn-header-cell cn-cell--center" style={{ width: `${columnWidths.adDate}px`, flexShrink: 0 }}>Date<ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.adDate - 2} columnName="adDate" /></div>
                <div className="cn-header-cell" style={{ width: `${columnWidths.voucherNo}px`, flexShrink: 0 }}>Vch No.<ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.voucherNo - 2} columnName="voucherNo" /></div>
                <div className="cn-header-cell" style={{ width: `${columnWidths.creditAccounts}px`, flexShrink: 0 }}>Credit A/cs<ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.creditAccounts - 2} columnName="creditAccounts" /></div>
                <div className="cn-header-cell cn-cell--end" style={{ width: `${columnWidths.credit}px`, flexShrink: 0 }}>Credit<ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.credit - 2} columnName="credit" /></div>
                <div className="cn-header-cell" style={{ width: `${columnWidths.debitAccounts}px`, flexShrink: 0 }}>Debit A/cs<ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.debitAccounts - 2} columnName="debitAccounts" /></div>
                <div className="cn-header-cell cn-cell--end" style={{ width: `${columnWidths.debit}px`, flexShrink: 0 }}>Debit<ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.debit - 2} columnName="debit" /></div>
                <div className="cn-header-cell" style={{ width: `${columnWidths.description}px`, flexShrink: 0 }}>Description<ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.description - 2} columnName="description" /></div>
                <div className="cn-header-cell" style={{ width: `${columnWidths.actions}px`, flexShrink: 0 }}>Actions<ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.actions - 2} columnName="actions" /></div>
    ...
    const TableRow = React.memo(({ index, style, data }) => {
        const { creditNotes, selectedRowIndex, formatCurrency, navigate, handleRowClick } = data;
        const creditNote = creditNotes[index];
        if (!creditNote) return null;
        const isSelected = selectedRowIndex === index;
        const isCanceled = creditNote.status !== 'Active';
        ...
        return (
            <div style={{ ...style, display: 'flex', alignItems: 'center', height: '28px', borderBottom: '1px solid #e2e8f0', cursor: 'pointer', backgroundColor: isSelected ? '#eff6ff' : (index % 2 === 0 ? '#f8fafc' : 'white') }} className="cn-row" onClick={() => handleRowClick(index)} onDoubleClick={() => { if (creditNote && creditNote.id) navigate(`/retailer/credit-note/${creditNote.id}/print`); }}>
                <div className="cn-cell cn-cell--center" style={{ width: `${columnWidths.bsDate}px`, flexShrink: 0 }}><span>{creditNote.nepaliDate || ''}</span></div>
                <div className="cn-cell cn-cell--center" style={{ width: `${columnWidths.adDate}px`, flexShrink: 0 }}><span>{creditNote.date ? new Date(creditNote.date).toLocaleDateString() : ''}</span></div>
                <div className="cn-cell" style={{ width: `${columnWidths.voucherNo}px`, flexShrink: 0 }}><span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{creditNote.billNumber || ''}</span></div>
                <div className="cn-cell" style={{ width: `${columnWidths.creditAccounts}px`, flexShrink: 0 }} title={creditAccountsDisplay}><span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', color: isCanceled ? '#dc2626' : 'inherit' }}>{isCanceled ? 'Canceled' : creditAccountsDisplay}</span></div>
                <div className="cn-cell cn-cell--end" style={{ width: `${columnWidths.credit}px`, flexShrink: 0 }} title={creditAmountsDisplay}><span style={{ color: isCanceled ? '#dc2626' : 'inherit' }}>{isCanceled ? '0.00' : creditAmountsDisplay}</span></div>
                <div className="cn-cell" style={{ width: `${columnWidths.debitAccounts}px`, flexShrink: 0 }} title={debitAccountsDisplay}><span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', color: isCanceled ? '#dc2626' : 'inherit' }}>{isCanceled ? 'Canceled' : debitAccountsDisplay}</span></div>
                <div className="cn-cell cn-cell--end" style={{ width: `${columnWidths.debit}px`, flexShrink: 0 }} title={debitAmountsDisplay}><span style={{ color: isCanceled ? '#dc2626' : 'inherit' }}>{isCanceled ? '0.00' : debitAmountsDisplay}</span></div>
                <div className="cn-cell" style={{ width: `${columnWidths.description}px`, flexShrink: 0 }} title={creditNote.description || ''}><span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{creditNote.description || ''}</span></div>
                <div className="cn-cell cn-cell--center gap-1" style={{ width: `${columnWidths.actions}px`, flexShrink: 0 }}>
                    <button className="cn-btn-action cn-btn-action--info" onClick={(e) => { e.stopPropagation(); if (creditNote && creditNote.id) navigate(`/retailer/credit-note/${creditNote.id}/print`); }} title="View"><i className="bi bi-eye" /></button>
                    {canEdit && (
                        <button className="cn-btn-action cn-btn-action--warning" onClick={(e) => { e.stopPropagation(); if (creditNote && creditNote.id) navigate(`/retailer/credit-note/edit/${creditNote.id}`); }} title="Edit"><i className="bi bi-pencil-square" /></button>
                    )}
                </div>
            </div>
        );
    ...
    const resetColumnWidths = () => {
        setColumnWidths({ bsDate: 80, adDate: 80, voucherNo: 100, creditAccounts: 150, credit: 80, debitAccounts: 150, debit: 80, description: 130, actions: 100 });
        setNotification({ show: true, message: 'Column widths reset', type: 'success', duration: 2000 });
    };

    if (loading && creditNotes.length === 0) return <Loader />;
    if (error) return <div className="cn-page"><Header /><div className="cn-shell"><div className="cn-state"><h3>Error</h3><p>{error}</p></div></div></div>;
        ...
        <div className="cn-page">
            ...
            <div className="cn-shell">
                ...
                <div className="cn-topbar">
                    <div className="cn-topbar__left">
                        <div className="cn-topbar__icon"><FiFileText /></div>
                        <div><h1>Credit Note Register</h1></div>
                    </div>
                    <div className="cn-topbar__actions">
                        <button className="cn-btn-icon" onClick={() => navigate('/retailer/credit-note')} title="Add Credit Note"><FiPlus /> Add</button>
                        <button className="cn-btn-icon" onClick={() => handlePrint(true)} disabled={creditNotesArray.length === 0}><FiPrinter /> Print</button>
                        <button className="cn-btn-icon" onClick={resetColumnWidths} title="Reset columns"><FiRefreshCw /> Reset</button>
                    </div>
                </div>

                {/* Toolbar */}
                <div className="cn-toolbar">
                    <div className="cn-field cn-field--date">
                        ...
                        {dateErrors.fromDate && <div className="cn-field-error">{dateErrors.fromDate}</div>}
                    </div>
                    <div className="cn-field cn-field--date">
                        <label>From (AD)</label>
                    ...
                    <div className="cn-field cn-field--date">
                        ...
                        {dateErrors.toDate && <div className="cn-field-error">{dateErrors.toDate}</div>}
                    </div>
                    <div className="cn-field cn-field--date">
                        <label>To (AD)</label>
                    ...
                    <button type="button" id="generateReport" ref={generateReportRef} className="cn-btn-gen" onClick={handleGenerateReport} disabled={loading}>
                        {loading ? <span className="spinner-border spinner-border-sm" style={{ width: 12, height: 12 }} /> : <><FiSearch className="me-1" /> Generate</>}
                    </button>

                    <div className="cn-toolbar-divider" />

                    <div className="cn-field cn-field--search">
                        <label>Search</label>
                        <div className="cn-search-wrap">
                            <FiSearch className="cn-search-icon" />
                            <input type="text" id="searchInput" ref={searchInputRef} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} disabled={creditNotesArray.length === 0} autoComplete="off" />
                            {searchQuery && <button className="cn-search-clear" onClick={() => setSearchQuery('')}>×</button>}
                        </div>
                    </div>
                </div>
                ...
                <div className="cn-main">
                    {creditNotesArray.length === 0 && !loading ? (
                        ...
                        <div className="cn-state"><div className="spinner-border text-primary" /><p>Loading credit notes...</p></div>
                    ) : filteredCreditNotes.length === 0 ? (
                        <div className="cn-state"><FiSearch size={32} style={{ opacity: 0.3, marginBottom: '0.5rem' }} /><h3>No credit notes found</h3><p>{searchQuery ? 'Try a different search term' : 'No data for the selected date range'}</p></div>
                    ) : (
                        <>
                            <div className="cn-main__bar">
                                <span><strong>{filteredCreditNotes.length}</strong> credit notes</span>
                                <span>{dateRange.fromDate} — {dateRange.toDate}</span>
                            </div>

                            <div className="cn-table-wrap" ref={tableBodyRef}>
                                <AutoSizer>
                                    {({ height, width }) => {
                                        const totalWidth = Object.values(columnWidths).reduce((a, b) => a + b, 0);
                                        return (
                                            <div style={{ position: 'relative', height: height, width: Math.max(width, totalWidth) }}>
                                                <TableHeader />
                                                <List height={height - 28} itemCount={filteredCreditNotes.length} itemSize={28} width={Math.max(width, totalWidth)} itemData={{ creditNotes: filteredCreditNotes, selectedRowIndex, formatCurrency, navigate, handleRowClick }}>{TableRow}</List>
                                            </div>
                                        );
                                    }}
                                </AutoSizer>
                            ...
                            {/* ✅ FIXED FOOTER: Aligns Credit and Debit in a single row correctly */}
                            <div className="cn-footer">
                                <div className="cn-footer-cell" style={{ width: `${columnWidths.bsDate + columnWidths.adDate + columnWidths.voucherNo + columnWidths.creditAccounts}px`, flexShrink: 0 }}>
                                    <strong>Total:</strong>
                                </div>
                                <div className="cn-footer-cell cn-cell--end" style={{ width: `${columnWidths.credit}px`, flexShrink: 0 }}>
                                    <strong>{formatCurrency(totalCredit)}</strong>
                                </div>
                                <div className="cn-footer-cell" style={{ width: `${columnWidths.debitAccounts}px`, flexShrink: 0, borderRight: '1px solid var(--cn-border)' }}>
                                    {/* Empty spacer for Debit Accounts column */}
                                </div>
                                <div className="cn-footer-cell cn-cell--end" style={{ width: `${columnWidths.debit}px`, flexShrink: 0 }}>
                                    <strong>{formatCurrency(totalDebit)}</strong>
                                </div>
                                <div className="cn-footer-cell" style={{ flex: 1, minWidth: `${columnWidths.description + columnWidths.actions}px` }}>
