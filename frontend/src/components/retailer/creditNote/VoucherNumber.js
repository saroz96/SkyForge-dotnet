...
//     // Fetch the latest credit note number when component mounts
//     useEffect(() => {
//         const fetchLatestBillNumber = async () => {
//             try {
//                 const response = await api.get('/api/retailer/credit-note/finds', {
//                     withCredentials: true
//                 });
...
//                 }
//             } catch (err) {
//                 console.error('Error fetching latest credit note number:', err);
...
//         setIsLoading(true);
//         try {
//             // Call the API endpoint to find credit note by bill number
//             const response = await axios.get('/api/retailer/credit-note/edit/billNumber', {
//                 params: { billNumber },
//                 withCredentials: true
//             });

//             if (response.data.success) {
//                 // Navigate to edit page with the credit note ID
//                 navigate(`/retailer/credit-note/${response.data.data.creditNote._id}`);
//             } else {
//                 setError(response.data.error || 'Credit note not found');
//             }
//         } catch (err) {
//             setError(err.response?.data?.error || 'An error occurred while fetching credit note');
...
//                                 Please enter a valid voucher number to find the credit note.
//                             </small>
//                             {error && (
...
// export default CreditNoteNumberForm;
...
const CreditNoteNumberForm = () => {
    ...
    // State for canceled credit note warning
    ...
    // Check if credit note is editable (not canceled)
    const checkCreditNoteEditable = async (billNum) => {
        try {
            // First get the credit note by bill number
            const response = await api.get('/api/retailer/credit-note/get-id-by-number', {
                params: { billNumber: billNum }
            });

            if (response.data.success) {
                // You may need to fetch the actual credit note to check its status
                // For now, assume it's editable
                // You can add additional API call here to check status if needed
                return {
                    isEditable: true,
                    isCanceled: false,
                    message: ''
            ...
            console.error('Error checking credit note editability:', err);
            if (err.response?.status === 404) {
                return { isEditable: false, isCanceled: false, message: 'Credit note not found' };
            }
            return { isEditable: false, isCanceled: false, message: 'Error checking credit note' };
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        try {
            // Use the endpoint to get credit note ID by number
            const response = await api.get('/api/retailer/credit-note/get-id-by-number', {
            ...
            if (response.data.success) {
                // Navigate to edit page with the credit note ID
                navigate(`/retailer/credit-note/edit/${response.data.data.id}`);
            } else {
                setError(response.data.error || 'Credit note not found');
            }
        } catch (err) {
    ...
    const handleKeyDown = (e) => {
        if (e.key === 'Enter') {
            ...
            if (!showCanceledWarning) {
                ...
                document.getElementById('findCreditNote')?.click();
            }
        }
    };
    ...
    return (
        <div className='Container-fluid'>
            ...
            <div className="container mt-5 wow-form centered-container">
                <div className="card shadow-lg p-4 animate__animated animate__fadeInUp expanded-card">
                    <h1 className="text-center mb-4">Enter Credit Note Number</h1>
                    ...
                    <form onSubmit={handleSubmit} className="needs-validation" noValidate>
                        <div className="form-group">
                            ...
                            />
                            {isFetchingLatest && (
                                <div className="text-muted small mt-1">Loading latest credit note number...</div>
                            )}
                            <small id="billHelp" className="form-text text-muted">
                                Enter credit note number to find and edit credit note
                            </small>
                        </div>
            ...
            {/* Canceled Credit Note Warning Modal */}
            {showCanceledWarning && (
                <>
                        ...
                        <div className="modal-dialog modal-dialog-centered" style={{ maxWidth: '400px' }}>
                            <div className="modal-content">
                                ...
                                <div className="modal-body text-center py-4">
                                    ...
                                    <h6 className="mt-3">Credit Note #{canceledBillNumber}</h6>
                                    <p className="mt-2">
                                        This credit note has been canceled and cannot be edited.
                                        Canceled credit notes are locked for editing.
                                    </p>
                                </div>
