// src/components/retailer/dashboard/modals/DailyBankSummary.jsx

import React, { useState, useEffect } from 'react';
import axios from 'axios';
import NepaliDate from 'nepali-datetime';

// Date conversion utilities (same as in DailyCashSummary)
...
const DailyBankSummary = ({ show, onClose, companyId, accountId }) => {
    ...
    }, [show, companyId, selectedBankAccount]);
                ...
                <div className="modal-content">
                    ...
                    <div className="modal-body">
                        ...
                        <div className="row g-1 mb-2">
                            <div className="col-12 col-md-4">
                                <div className="card border-start border-success border-3">
                                    <div className="card-body py-2 px-3">
                                        <h6 className="text-muted mb-0 small">Bank Inflow</h6>
                                        <h5 className="mb-0 text-success">
                                            Rs. {formatCurrency(bankData.totalBankInflow)}
                                        </h5>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="col-12 col-md-4">
                                <div className="card border-start border-danger border-3">
                                    <div className="card-body py-2 px-3">
                                        <h6 className="text-muted mb-0 small">Bank Outflow</h6>
                                        <h5 className="mb-0 text-danger">
                                            Rs. {formatCurrency(bankData.totalBankOutflow)}
                                        </h5>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="col-12 col-md-4">
                                <div className="card border-start border-primary border-3">
                                    <div className="card-body py-2 px-3">
                                        <h6 className="text-muted mb-0 small">Net Bank</h6>
                                        <h5 className="mb-0 text-primary">
                                            Rs. {formatCurrency(bankData.netBank)}
                                        </h5>
                                    </div>
                                </div>
                            </div>
                        ...
                        {/* {bankData.bankAccounts.length > 0 && (
                            <div className="mb-0">
                                <h6 className="text-muted mb-0" style={{ fontSize: '0.75rem' }}>Bank Accounts Summary</h6>
                                <div className="row g-1">
                                    {bankData.bankAccounts.map((account, index) => {
                                        ...
                                        const inflow = account.inflow || account.Inflow || 0;
                                        const outflow = account.outflow || account.Outflow || 0;

                                        return (
                                            <div className="col-12 col-md-3" key={accountId || index}>
                                                <div
                                                    className={`card border-start border-${net >= 0 ? 'success' : 'danger'} border-2 ${selectedBankAccount === accountId ? 'bg-light' : ''}`}
                                                    style={{ cursor: 'pointer' }}
                                                    onClick={() => handleBankAccountFilter(accountId)}
                                                >
                                                    <div className="card-body py-1 px-2">
                                                        <div className="d-flex justify-content-between align-items-center">
                                                            <small className="text-muted" style={{ fontSize: '0.6rem' }}>{accountName}</small>
                                                            <span className={`badge ${net >= 0 ? 'bg-success' : 'bg-danger'}`} style={{ fontSize: '0.55rem' }}>
                        ...
                        {/* Bank Account Summary - Button Sized */}
{bankData.bankAccounts.length > 0 && (
    <div className="mb-2">
        <h6 className="text-muted mb-1" style={{ fontSize: '0.7rem' }}>Bank Accounts Summary</h6>
        <div className="d-flex flex-wrap gap-1">
            {bankData.bankAccounts.map((account, index) => {
                ...
                return (
                    <button
                        key={accountId || index}
                        className={`btn btn-sm ${selectedBankAccount === accountId ? 'btn-primary' : 'btn-outline-secondary'} d-flex align-items-center gap-1`}
                        onClick={() => handleBankAccountFilter(accountId)}
                        style={{ 
                            fontSize: '0.65rem', 
                            padding: '2px 8px',
                    ...
                    >
                        ...
                        <span className={`badge ${net >= 0 ? 'bg-success' : 'bg-danger'}`} style={{ fontSize: '0.55rem', padding: '1px 4px' }}>
                            {net >= 0 ? '+' : ''}{formatCurrency(Math.abs(net))}
                        </span>
                        {selectedBankAccount === accountId && (
                            <i className="bi bi-x-circle" style={{ fontSize: '0.65rem' }}></i>
                        )}
                    </button>
                );
            })}
            {selectedBankAccount && (
                <button 
                    className="btn btn-sm btn-outline-secondary"
                    onClick={() => setSelectedBankAccount(null)}
                    style={{ fontSize: '0.65rem', padding: '2px 8px', height: '28px' }}
                >
                                    ...
                                    <table className="table table-sm table-hover table-bordered">
                                        <thead className="table-light sticky-top">
                                            <tr>
                                                ...
                                                <th style={{ fontSize: '0.75rem' }}>Bank Account</th>
