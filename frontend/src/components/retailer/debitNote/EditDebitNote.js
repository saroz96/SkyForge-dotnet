...
const EditDebitNote = () => {
    ...
    useEffect(() => {
        const fetchDebitNoteData = async () => {
            try {
                ...
                const response = await api.get(`/api/retailer/debit-note/edit/${id}`);
                ...
                setBillNumber(debitNote.billNumber || '');
                setFormData({
                    date: formatDate(debitNote.date) || new Date().toISOString().split('T')[0],
                    nepaliDate: formatDate(debitNote.nepaliDate) || currentNepaliDate,
                    description: debitNote.description || '',
                    status: debitNote.status || 'Active',
                ...
                setError(error.response?.data?.error || 'Failed to load debit note data');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleSubmit = async (print = false) => {
        ...
        try {
            ...
            const response = await api.put(`/api/retailer/debit-note/edit/${id}`, payload);
            ...
            setNotification({
                show: true,
                message: err.response?.data?.error || 'Failed to update debit note. Please try again.',
                type: 'error'
            });
        } finally {
            setIsSaving(false);
    ...
    const handleCancelVoucher = async () => {
        if (window.confirm("Are you sure you want to cancel this voucher?")) {
            try {
                await api.post(`/api/retailer/debit-notes/cancel/${billNumber}`);
                setNotification({
                    show: true,
    ...
    const handleReactivateVoucher = async () => {
        if (window.confirm("Are you sure you want to reactivate this voucher?")) {
            try {
                await api.post(`/api/retailer/debit-notes/reactivate/${billNumber}`);
                setNotification({
                    show: true,
            ...
            <div id="printableContent">
                <div class="print-voucher-container">
                    ...
                    <div class="print-voucher-details">
                        ...
                        <div><strong>Date:</strong> ${printData.companyDateFormat === 'nepali' ? formatDateForInput(printData.debitNote.nepaliDate) : formatDateForInput(printData.debitNote.date)}(${new Date(printData.debitNote.date).toLocaleDateString()})</div>
                    </div>
                    ...
                    <div style="margin-top: 3mm;"><strong>Note:</strong> ${printData.debitNote?.description || 'N/A'}</div>

                    <div class="print-signature-area">
                        <div class="print-signature-box">
                            <div><strong>${printData.debitNote?.user?.name || 'N/A'}</strong></div>
                            Prepared By
                        </div>
                        <div class="print-signature-box">
                            <div>&nbsp;</div>
                                        ...
                                        <div className="position-relative">
                                            <input
                                                ...
                                                style={{
                                                    ...
                                                    paddingTop: '0.75rem',
                                                    width: '100%',
                                                    backgroundColor: isCanceled ? '#e9ecef' : '#f8f9fa',
                                                    cursor: 'not-allowed'
                                                }}
                                            />
                                            <label className="position-absolute" style={{
