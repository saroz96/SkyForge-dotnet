...
const StockAdjustmentsList = () => {
    ...
    // Fetch company and fiscal year info from stock adjustments entry data
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                // Fetch stock adjustments entry data from ASP.NET endpoint
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (response.data.success) {
                    setData(prev => ({
                        ...prev,
                        stockAdjustments: response.data.data.stockAdjustments || [],
                        ...
                        vatEnabled: response.data.data.company?.vatEnabled || true,
                        isVatExempt: response.data.data.company?.isVatExempt || false,
                        isAdminOrSupervisor: response.data.data.isAdminOrSupervisor || false,
                        nepaliDate: response.data.data.nepaliDate || currentNepaliDate
                    }));
                    setError(null);
                } else {
    ...
    const handlePrint = (filtered = false) => {
        ...
        ${printHeader}
        <h1 style="text-align:center;text-decoration:underline;">Stock Adjustments Register</h1>
        <table>
            <thead>
                <tr>
                    <th class="nowrap">Date</th>
        ...
        rowsToPrint.forEach(adjustment => {
            const date = adjustment.date ? new Date(adjustment.date) : new Date();
            const displayDate = company.dateFormat === 'nepali'
                ? new NepaliDate(adjustment.date).format('YYYY-MM-DD')
            ...
            <tr>
                ...
                <td class="nowrap">${(adjustment.quantity || 0).toFixed(2)}</td>
                <td class="nowrap">${adjustment.unitName || ''}</td>
                <td class="nowrap">${(adjustment.puPrice || 0).toFixed(2)}</td>
                <td class="nowrap">
                    <span class="badge-${adjustment.adjustmentType || ''}">
                        ${adjustment.adjustmentType || ''}
                    </span>
    ...
    const TableRow = React.memo(({ index, style, data: rowData }) => {
        ...
        const handleDoubleClick = () => {
            navigate(`/retailer/stock-adjustments/${adjustment.adjustmentId}/print`);
        };
                        ...
                        <div className="col-12 col-md-1">
                            <div className="position-relative">
                                <input
                                    ...
                                    onBlur={(e) => {
                                        // Date validation logic (same as before but simplified for brevity)
                                        try {
                                            const dateStr = e.target.value.trim();
                                            if (!dateStr) {
                                                setDateErrors(prev => ({ ...prev, fromDate: '' }));
