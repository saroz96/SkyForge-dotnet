...
const InvoiceWiseProfitLossReport = () => {
    ...
    const [company, setCompany] = useState({
        dateFormat: 'english', vatEnabled: true, fiscalYear: {}
    });
    ...
    const [data, setData] = useState(() => {
        ...
        return {
            ...
            currentCompanyName: '',
            companyDateFormat: 'english',
            summary: { totalProfit: 0, totalSales: 0, totalCost: 0, totalInvoices: 0 }
        };
    });
    ...
    // Column resizing state
    const [columnWidths, setColumnWidths] = useState({
        nepaliDate: 90, date: 90, invNo: 100, account: 180,
        cost: 100, sales: 100, cpPercentage: 80, spPercentage: 80,
        profit: 100, actions: 80
    });
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                ...
                console.error('Error fetching initial data:', err);
                setDateRange({
                    fromDate: currentEnglishDate, toDate: currentEnglishDate,
                    fromDateAd: currentEnglishDate, toDateAd: currentEnglishDate
                });
                setNotification({ show: true, message: 'Error loading company data, using default dates', type: 'warning' });
            }
        };
        fetchInitialData();
    }, []);
            ...
            try {
                ...
                if (response.data.success) {
                    ...
                    setData(prev => ({
                        ...prev,
                        results: responseData.results || [],
                        summary: responseData.summary || { totalProfit: 0, totalSales: 0, totalCost: 0, totalInvoices: 0 },
                        company: responseData.company || prev.company,
                        currentFiscalYear: responseData.currentFiscalYear || prev.currentFiscalYear,
                        currentCompanyName: responseData.currentCompanyName || prev.currentCompanyName,
                        companyDateFormat: responseData.companyDateFormat || prev.companyDateFormat
    ...
    useEffect(() => {
        ...
        setFilteredResults(filtered);
        if (!draftSave?.profitLossSearch?.selectedRowIndex) setSelectedRowIndex(0);
    }, [data.results, searchQuery]);
    ...
    const totals = useMemo(() => {
        ...
        return {
            ...
            cpPercentage: totalCost !== 0 ? (totalProfit / totalCost) * 100 : 0,
            spPercentage: totalSales !== 0 ? (totalProfit / totalSales) * 100 : 0
        };
    }, [filteredResults]);
    ...
    useEffect(() => {
        const handleKeyDown = (e) => {
            ...
            switch (e.key) {
                ...
                case 'ArrowDown': e.preventDefault(); setSelectedRowIndex(prev => Math.min(filteredResults.length - 1, prev + 1)); break;
                default: break;
            }
        };
        window.addEventListener('keydown', handleKeyDown);
    ...
    const handleBsDateChange = (e) => {
        ...
        const sanitizedValue = value.replace(/[^0-9/-]/g, '').slice(0, 10);
        const adDate = convertBsToAd(sanitizedValue);
        setDateRange(prev => ({ ...prev, [name]: sanitizedValue, [`${name}Ad`]: adDate || prev[`${name}Ad`] }));
        setDateErrors(prev => ({ ...prev, [name]: '' }));
    };
    ...
    const handleAdDateChange = (e) => {
        ...
        const bsDate = convertAdToBs(value);
        const fieldName = name.replace('Ad', '');
        setDateRange(prev => ({ ...prev, [name]: value, [fieldName]: bsDate || prev[fieldName] }));
    };

    const handleBillNumberChange = (e) => setData(prev => ({ ...prev, billNumber: e.target.value }));
    ...
    const handleBsDateBlur = (e) => {
        const fieldName = e.target.name;
        const dateStr = e.target.value.trim();
        if (!dateStr) { setDateErrors(prev => ({ ...prev, [fieldName]: '' })); return; }
        const correctedDate = validateAndCorrectNepaliDate(dateStr);
        if (!correctedDate) {
            const fallbackDate = currentNepaliDate;
            const adDate = convertBsToAd(fallbackDate);
            setDateRange(prev => ({ ...prev, [fieldName]: fallbackDate, [`${fieldName}Ad`]: adDate }));
    ...
    const handleReset = () => {
        ...
        if (isNepaliFormat) {
            ...
            fromDateFormatted = data.currentFiscalYear?.startDate ? new Date(data.currentFiscalYear.startDate).toISOString().split('T')[0] : currentEnglishDate;
            toDateFormatted = data.currentFiscalYear?.endDate ? new Date(data.currentFiscalYear.endDate).toISOString().split('T')[0] : currentEnglishDate;
            fromDateAd = fromDateFormatted;
            toDateAd = toDateFormatted;
        }
        setDateRange({ fromDate: fromDateFormatted, toDate: toDateFormatted, fromDateAd, toDateAd });
        setData(prev => ({ ...prev, billNumber: '' }));
        setSearchQuery('');
        setError(null);
    };
    ...
    const formatDate = useCallback((dateString) => {
        if (!dateString) return '';
        try { return company.dateFormat === 'nepali' ? new NepaliDate(dateString).format('YYYY-MM-DD') : new Date(dateString).toISOString().split('T')[0]; }
    ...
    const handlePrint = () => {
        ...
        let tableContent = `
            <style>
                ...
                th { background-color: #f2f2f2 !important; -webkit-print-color-adjust: exact; font-size: 8px; font-weight: bold; }
                ...
                .grand-total-row td { font-weight: bold; border-top: 2px solid #000; }
            </style>
            <div class="report-title">Invoice Wise Profit/Loss Report</div>
            <div style="text-align:center;font-size:8px;margin:2px 0;">Period: ${dateRange.fromDate} (${company.dateFormat === 'nepali' ? 'BS' : 'AD'}) to ${dateRange.toDate} (${company.dateFormat === 'nepali' ? 'BS' : 'AD'})</div>
            <table>
                <thead><tr>
                    <th class="text-center">S.N</th><th>Miti</th><th>Date</th><th>Inv No.</th><th>Account</th>
                    <th class="text-end">Cost</th><th class="text-end">Sales</th><th class="text-end">C.P(%)</th><th class="text-end">S.P(%)</th><th class="text-end">Profit</th>
                </tr></thead>
                <tbody>
        `;
        filteredResults.forEach((bill, index) => {
            ...
            tableContent += `
                <tr>
                    <td class="text-center">${index + 1}</td>
                    <td>${formatDate(bill.nepaliDate)}</td>
        ...
        tableContent += `
                <tr class="grand-total-row">
                    ...
                    <td class="text-end ${totals.totalProfit >= 0 ? 'profit-positive' : 'profit-negative'}">${formatCurrency(totals.totalProfit)}</td>
                </tr>
            </tbody></table>
            <script>window.onload=function(){window.print();window.onafterprint=function(){window.close()}}<\/script>
        `;
        printWindow.document.write(`<!DOCTYPE html><html><head><title>Invoice Wise Profit/Loss Report</title><meta charset="UTF-8"></head><body>${tableContent}</body></html>`);
        printWindow.document.close();
    };

    const resetColumnWidths = () => {
        setColumnWidths({ nepaliDate: 90, date: 90, invNo: 100, account: 180, cost: 100, sales: 100, cpPercentage: 80, spPercentage: 80, profit: 100, actions: 80 });
    };
    ...
    const ResizeHandle = React.memo(({ onResizeStart, left, columnName }) => {
        return (<div className="iw-resize-handle" style={{ position: 'absolute', top: 0, left: `${left}px`, width: '5px', height: '100%', cursor: 'col-resize', zIndex: 10 }} onMouseDown={(e) => { e.preventDefault(); onResizeStart(e, columnName); }} />);
    });
    ...
    const TableHeader = React.memo(() => {
        ...
        return (
            <div className="iw-header" style={{ minWidth: `${totalWidth}px` }}
                onMouseMove={(e) => { if (isResizing && resizingColumn) { const diff = e.clientX - startX; setColumnWidths(prev => ({ ...prev, [resizingColumn]: Math.max(60, startWidth + diff) })); } }}
                onMouseUp={() => { setIsResizing(false); setResizingColumn(null); }}
                onMouseLeave={() => { setIsResizing(false); setResizingColumn(null); }}
            >
                {[
                    { key: 'nepaliDate', label: 'Miti', w: columnWidths.nepaliDate, align: 'center' },
                    { key: 'date', label: 'Date', w: columnWidths.date, align: 'center' },
                    { key: 'invNo', label: 'Inv No.', w: columnWidths.invNo, align: 'left' },
                    { key: 'account', label: 'Account', w: columnWidths.account, align: 'left' },
                    { key: 'cost', label: 'Cost', w: columnWidths.cost, align: 'end' },
                    { key: 'sales', label: 'Sales', w: columnWidths.sales, align: 'end' },
                    { key: 'cpPercentage', label: 'C.P(%)', w: columnWidths.cpPercentage, align: 'end' },
                    { key: 'spPercentage', label: 'S.P(%)', w: columnWidths.spPercentage, align: 'end' },
                    { key: 'profit', label: 'Profit', w: columnWidths.profit, align: 'end' },
                    { key: 'actions', label: 'Actions', w: columnWidths.actions, align: 'center' }
                ].map((col, i) => (
                    <div key={col.key} className={`iw-header-cell iw-header-cell--${col.align}`} style={{ width: `${col.w}px`, flexShrink: 0 }}>
                        <strong>{col.label}</strong>
                        <ResizeHandle onResizeStart={handleResizeStart} left={col.w - 2} columnName={col.key} />
                    </div>
                ))}
                {isResizing && <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 1000, cursor: 'col-resize' }} />}
            </div>
        );
    });
    ...
    const handleViewItems = (bill) => { setSelectedBill(bill); setShowItemModal(true); };
    ...
    const TableRow = React.memo(({ index, style, data: rowData }) => {
        ...
        const isSelected = selectedRowIndex === index;
        return (
            <div style={{ ...style, display: 'flex', alignItems: 'center', height: '28px', padding: '0', borderBottom: '1px solid #e2e8f0', cursor: 'pointer', backgroundColor: isSelected ? '#eff6ff' : (index % 2 === 0 ? '#f8fafc' : 'white') }} onClick={() => handleRowClick(index)}>
                <div className="iw-cell iw-cell--center" style={{ width: `${columnWidths.nepaliDate}px` }}><span>{formatDate(bill.nepaliDate)}</span></div>
                <div className="iw-cell iw-cell--center" style={{ width: `${columnWidths.date}px` }}><span>{new Date(bill.date).toLocaleDateString()}</span></div>
                <div className="iw-cell" style={{ width: `${columnWidths.invNo}px`, overflow: 'hidden' }}><span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{bill.billNumber || ''}</span></div>
                <div className="iw-cell" style={{ width: `${columnWidths.account}px`, overflow: 'hidden' }} title={bill.accountName || bill.cashAccount || 'N/A'}><span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{bill.accountName || bill.cashAccount || 'N/A'}</span></div>
                <div className="iw-cell iw-cell--end" style={{ width: `${columnWidths.cost}px` }}><span>{formatCurrency(bill.totalCost)}</span></div>
                <div className="iw-cell iw-cell--end" style={{ width: `${columnWidths.sales}px` }}><span>{formatCurrency(bill.totalSales)}</span></div>
                <div className="iw-cell iw-cell--end" style={{ width: `${columnWidths.cpPercentage}px` }}><span>{formatPercentage(bill.totalProfit, bill.totalCost)}</span></div>
                <div className="iw-cell iw-cell--end" style={{ width: `${columnWidths.spPercentage}px` }}><span>{formatPercentage(bill.totalProfit, bill.totalSales)}</span></div>
                <div className={`iw-cell iw-cell--end ${bill.totalProfit >= 0 ? 'iw-text-success' : 'iw-text-danger'}`} style={{ width: `${columnWidths.profit}px` }}>
                    <span style={{ fontWeight: 'bold' }}>{formatCurrency(bill.totalProfit)}</span>
                    {bill.totalProfit > 0 && <span className="iw-text-success ms-1"><i className="fas fa-caret-up" /></span>}
                    {bill.totalProfit < 0 && <span className="iw-text-danger ms-1"><i className="fas fa-caret-down" /></span>}
                </div>
                <div className="iw-cell iw-cell--center" style={{ width: `${columnWidths.actions}px` }}>
                    {bill.items && bill.items.length > 0 && (
                        <button className="iw-btn-view" onClick={(e) => { e.stopPropagation(); onViewItems(bill); }}><i className="fas fa-eye me-1" />View</button>
                    )}
                </div>
            </div>
        );
        ...
        <div className="iw-page">
            ...
            <div className="iw-shell">
                ...
                <div className="iw-topbar">
                    <div className="iw-topbar__left">
                        <div className="iw-topbar__icon"><FiFileText /></div>
                        <div><h1>Invoice Wise Profit/Loss</h1></div>
                    </div>
                    <div className="iw-topbar__actions">
                        <button className="iw-btn-icon" onClick={handlePrint} disabled={data.results.length === 0}><FiPrinter /> Print</button>
                        <button className="iw-btn-icon" onClick={resetColumnWidths} title="Reset columns"><FiRefreshCw /> Reset</button>
                    </div>
                </div>

                {/* Toolbar */}
                <div className="iw-toolbar">
                    <div className="iw-field iw-field--date">
                        <label>From (BS) <span className="req">*</span></label>
                        <input type="text" name="fromDate" ref={fromDateRef} className={dateErrors.fromDate ? 'is-invalid' : ''} value={dateRange.fromDate || ''} onChange={handleBsDateChange} onKeyDown={(e) => handleKeyDown(e, 'fromDateAd')} onBlur={handleBsDateBlur} placeholder={isNepaliFormat ? "YYYY-MM-DD" : "YYYY-MM-DD"} autoComplete="off" autoFocus />
                    </div>
                    <div className="iw-field iw-field--date">
                        <label>From (AD)</label>
                        <input type="date" name="fromDateAd" ref={fromDateAdRef} value={dateRange.fromDateAd || ''} onChange={handleAdDateChange} onKeyDown={(e) => handleKeyDown(e, 'toDate')} />
                    </div>
                    <div className="iw-field iw-field--date">
                        <label>To (BS) <span className="req">*</span></label>
                        <input type="text" name="toDate" ref={toDateRef} className={dateErrors.toDate ? 'is-invalid' : ''} value={dateRange.toDate || ''} onChange={handleBsDateChange} onKeyDown={(e) => handleKeyDown(e, 'toDateAd')} onBlur={handleBsDateBlur} placeholder={isNepaliFormat ? "YYYY-MM-DD" : "YYYY-MM-DD"} autoComplete="off" />
                    </div>
                    <div className="iw-field iw-field--date">
                        <label>To (AD)</label>
                        <input type="date" name="toDateAd" ref={toDateAdRef} value={dateRange.toDateAd || ''} onChange={handleAdDateChange} onKeyDown={(e) => handleKeyDown(e, 'billNumber')} />
                    </div>
                    <div className="iw-field iw-field--inv">
                        <label>Inv. No.</label>
                        <input type="text" name="billNumber" ref={billNumberRef} value={data.billNumber} onChange={handleBillNumberChange} onKeyDown={(e) => handleKeyDown(e, 'generateReport')} placeholder="Search invoice" autoComplete="off" />
                    </div>
                    <button type="button" id="generateReport" ref={generateReportRef} className="iw-btn-gen" onClick={handleGenerateReport} disabled={loading}>
                        {loading ? <span className="spinner-border spinner-border-sm" style={{ width: 12, height: 12 }} /> : <><FiSearch className="me-1" /> Generate</>}
                    </button>
                    <div className="iw-toolbar-divider" />
                    <div className="iw-field iw-field--search">
                        <label>Search</label>
                        <div className="iw-search-wrap">
                            <input type="text" ref={searchInputRef} placeholder="Name, inv..." value={searchQuery} onChange={handleSearchChange} disabled={data.results.length === 0} autoComplete="off" />
                            {searchQuery && <button className="iw-search-clear" onClick={() => setSearchQuery('')}>×</button>}
                        </div>
                    </div>
                </div>

                {error && <div className="iw-alert"><i className="bi bi-exclamation-circle" />{error}<button type="button" className="btn-close btn-sm ms-auto" onClick={() => setError(null)} /></div>}
                ...
                <div className="iw-main">
                    {data.results.length === 0 && !shouldFetch ? (
                        <div className="iw-state">
                            ...
                            <p>Filter by date and invoice number above.</p>
                        </div>
                    ) : (
                        <>
                            <div className="iw-main__bar">
                                <span><strong>{filteredResults.length}</strong> invoices found</span>
                                <span>{dateRange.fromDate} — {dateRange.toDate}</span>
                            </div>
                            <div className="iw-table-wrap" ref={tableBodyRef}>
                                {loading ? (
                                    <div className="iw-state"><div className="spinner-border text-primary" /><p>Loading data...</p></div>
                                ) : filteredResults.length === 0 ? (
                                    <div className="iw-state"><FiSearch size={32} style={{ opacity: 0.3, marginBottom: '0.5rem' }} /><h3>No invoices found</h3><p>{searchQuery ? 'Try a different search term' : 'No data for the selected criteria'}</p></div>
                                ) : (
                                    <AutoSizer>
                                        {({ height, width }) => {
                                            const totalWidth = Object.values(columnWidths).reduce((a, b) => a + b, 0);
                                            return (
                                                <div style={{ position: 'relative', height: height, width: Math.max(width, totalWidth) }}>
                                                    <TableHeader />
                                                    <List height={height - 28} itemCount={filteredResults.length} itemSize={28} width={Math.max(width, totalWidth)}
                                                        itemData={{ bills: filteredResults, selectedRowIndex, formatCurrency, formatDate, formatPercentage, handleRowClick: setSelectedRowIndex, onViewItems: handleViewItems }}>
                                                        {TableRow}
                                                    </List>
                                                </div>
                                            );
                            ...
                            {filteredResults.length > 0 && (
                                <div className="iw-footer">
                                    <div className="iw-footer-cell" style={{ width: `${columnWidths.nepaliDate + columnWidths.date + columnWidths.invNo + columnWidths.account}px`, flexShrink: 0 }}><strong>Grand Total:</strong></div>
                                    <div className="iw-footer-cell iw-cell--end" style={{ width: `${columnWidths.cost}px` }}><strong>{formatCurrency(totals.totalCost)}</strong></div>
                                    <div className="iw-footer-cell iw-cell--end" style={{ width: `${columnWidths.sales}px` }}><strong>{formatCurrency(totals.totalSales)}</strong></div>
                                    <div className="iw-footer-cell iw-cell--end" style={{ width: `${columnWidths.cpPercentage}px` }}><strong>{formatPercentage(totals.totalProfit, totals.totalCost)}</strong></div>
                                    <div className="iw-footer-cell iw-cell--end" style={{ width: `${columnWidths.spPercentage}px` }}><strong>{formatPercentage(totals.totalProfit, totals.totalSales)}</strong></div>
                                    <div className={`iw-footer-cell iw-cell--end ${totals.totalProfit >= 0 ? 'iw-text-success' : 'iw-text-danger'}`} style={{ width: `${columnWidths.profit}px` }}><strong>{formatCurrency(totals.totalProfit)}</strong></div>
                                    <div className="iw-footer-cell" style={{ width: `${columnWidths.actions}px`, flexShrink: 0 }}></div>
                                </div>
                            )}
                        </>
                    )}
            ...
            {/* Item Details Modal */}
            {showItemModal && selectedBill && (
                <div className="iw-modal-overlay" onClick={() => setShowItemModal(false)}>
                    <div className="iw-modal" onClick={e => e.stopPropagation()}>
                        <div className="iw-modal-header">
                            <h5><i className="fas fa-box me-2" />Item Details - Invoice: {selectedBill.billNumber}</h5>
                        ...
                        <div className="iw-modal-body">
                            <table className="iw-modal-table">
                                <thead>
                                    <tr><th className="text-center">S.N</th><th>Item Name</th><th className="text-end">Qty</th><th className="text-end">Cost Price</th><th className="text-end">Sale Price</th><th className="text-end">Profit/Unit</th><th className="text-end">Total Profit</th></tr>
                                </thead>
                                <tbody>
                                    {selectedBill.items.map((item, idx) => {
                                        const profitPerUnit = item.price - (item.puPrice || 0);
                                        ...
                                        return (
                                            <tr key={idx}>
                                                ...
                                                <td className="text-end">{formatCurrency(item.puPrice)}</td>
                                                <td className="text-end">{formatCurrency(item.price)}</td>
                                                <td className={`text-end ${profitPerUnit >= 0 ? 'iw-text-success' : 'iw-text-danger'}`}>{formatCurrency(profitPerUnit)}</td>
                                                <td className={`text-end fw-bold ${itemProfit >= 0 ? 'iw-text-success' : 'iw-text-danger'}`}>{formatCurrency(itemProfit)}</td>
                                            </tr>
                                        );
                                    })}
                                </tbody>
                                <tfoot>
                                    <tr><td colSpan="5" className="text-end fw-bold">Total Profit:</td><td colSpan="2" className={`text-end fw-bold ${selectedBill.totalProfit >= 0 ? 'iw-text-success' : 'iw-text-danger'}`}>{formatCurrency(selectedBill.totalProfit)}</td></tr>
                                </tfoot>
                            </table>
                        </div>
                        <div className="iw-modal-footer">
                            <button className="iw-btn-secondary" onClick={() => setShowItemModal(false)}><i className="fas fa-times me-1" />Close</button>
                        </div>
                    </div>
                </div>
            )}
