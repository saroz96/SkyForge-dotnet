...
//             try {
//                 setLoading(true);
//                 const response = await api.get(`/api/retailer/journal/${id}/print`);

//                 if (!response.data.success) {
//                     throw new Error(response.data.error || 'Failed to fetch journal data');
//                 }
...
//                 setError(err.response?.data?.error || err.message || 'Failed to fetch journal data');
//                 setLoading(false);
//             }
//         };
...
//     if (!journalData || !journalData.journalVoucher) return (
//         <Container fluid className="mt-4">
...
//     const journal = journalData.journalVoucher;
//     const debitEntries = journalData.debitEntries || [];
//     const creditEntries = journalData.creditEntries || [];
...
//                             <h1 className="compact-text">{journalData.currentCompanyName}</h1>
//                             <h4 className="compact-text">
//                                 {journalData.currentCompany.address}, {journalData.currentCompany.city}
//                                 <br />
//                                 Tel: {journalData.currentCompany.phone} | PAN: {journalData.currentCompany.pan}
//                             </h4>
//                             <h2 className="compact-text">JOURNAL VOUCHER</h2>
//                         </div>
//                         <br />
...
//                             <div><strong>Note:</strong> {journal.description || 'N/A'}</div>
//                         </div>
...
//                                 <div className="compact-text"><strong>{journal.user?.name || 'N/A'}</strong></div>
//                                 Prepared By
//                             </div>
//                             <div className="signature-box">
                ...
                <Container fluid>
                    ...
                    <Card className="p-4">
                        ...
                        <div className="details-container compact-text">
                            ...
                            <div className="right">
                                <div><strong>Date:</strong> {journalData.companyDateFormat === 'nepali' ? (journalData.journalVoucher.nepaliDate) : (journalData.journalVoucher.date)}({new Date(journalData.journalVoucher.date).toLocaleDateString()})</div>
                            </div>
                        </div>
                        ...
                        <Table bordered size="sm">
                            <thead>
                                <tr>
                                    ...
                                    <th>Debit Account</th>
                                    <th className="text-center">Debit(Rs.)</th>
                                    <th>Credit Account</th>
                                    <th className="text-center">Credit(Rs.)</th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <tr key={row.rowNumber}>
                                        ...
                                        <td>
                                            {row.debitEntry ? (
                                                <>
                                                    {!isCanceled ? (
                                                        <>
                                                            {row.debitEntry.accountName}
                                                            {row.debitEntry.referenceNumber && (
                                                                <small className="d-block text-muted">
                                                                    Ref: {row.debitEntry.referenceNumber}
                                                                </small>
                                                            )}
                                                        </>
                                                    ) : (
                                        ...
                                        <td className="text-center">
                                            {row.debitEntry && !isCanceled ? formatTo2Decimal(row.debitEntry.amount) :
                                                row.debitEntry && isCanceled ? <span className="text-danger">0.00</span> : '-'}
                                        </td>
                                        <td>
                                            {row.creditEntry ? (
                                                <>
                                                    {!isCanceled ? (
                                                        <>
                                                            {row.creditEntry.accountName}
                                                            {row.creditEntry.referenceNumber && (
                                                                <small className="d-block text-muted">
                                                                    Ref: {row.creditEntry.referenceNumber}
                                                                </small>
                                                            )}
                                                        </>
                                                    ) : (
                                        ...
                                        <td className="text-center">
                                            {row.creditEntry && !isCanceled ? formatTo2Decimal(row.creditEntry.amount) :
                                                row.creditEntry && isCanceled ? <span className="text-danger">0.00</span> : '-'}
                                        </td>
                                    </tr>
                                ))}
                ...
                <div className="print-voucher-container">
                    ...
                    <table className="print-voucher-table">
                        ...
                        <tbody>
                            {combinedRows.map((row) => (
                                <tr key={row.rowNumber}>
                                    <td className="print-text-center">{row.rowNumber}</td>
                                    <td className="print-text-left">
                                        {row.debitEntry ? (
                                            <>
                                                {!isCanceled ? (
                                                    <>
                        ...
                        <tfoot>
                            <tr>
                                <td colSpan="2" style={{ borderBottom: '1px solid #000' }}>Total</td>
                                <td className="print-text-center" style={{ borderBottom: '1px solid #000', paddingLeft: '50px' }}>
                                    <strong>{!isCanceled ? formatTo2Decimal(totalDebit) : <span className="text-danger">0.00</span>}</strong>
                                </td>
                                <td style={{ borderBottom: '1px solid #000' }}></td>
                                <td className="print-text-center" style={{ borderBottom: '1px solid #000', paddingLeft: '50px' }}>
                                    <strong>{!isCanceled ? formatTo2Decimal(totalCredit) : <span className="text-danger">0.00</span>}</strong>
                                </td>
                            </tr>
                        </tfoot>
                    </table>
