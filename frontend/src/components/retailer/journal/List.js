...
//             tableContent += `
//             <tr>
//                 <td class="nowrap">${voucher.nepaliDate ? new NepaliDate(voucher.nepaliDate).format('YYYY-MM-DD') : ''}</td>
...
//                         {voucher.nepaliDate ? new NepaliDate(voucher.nepaliDate).format('YYYY-MM-DD') : ''}
//                     </span>
//                 </div>
...
const JournalList = () => {
    ...
    // SPLIT STATE: Separate date range from journal vouchers and company info
    const [dateRange, setDateRange] = useState(() => {
        if (draftSave && draftSave.journalData) {
            return {
                fromDate: draftSave.journalData.fromDate || '',
                toDate: draftSave.journalData.toDate || '',
                fromDateAd: draftSave.journalData.fromDateAd || '',
                toDateAd: draftSave.journalData.toDateAd || ''
            };
        }
        return {
            fromDate: '',
    ...
    const [journalVouchers, setJournalVouchers] = useState(() => {
        if (draftSave && draftSave.journalData) {
            return draftSave.journalData.journalVouchers || [];
        }
        return [];
    });
    ...
    const [companyInfo, setCompanyInfo] = useState(() => {
        if (draftSave && draftSave.journalData) {
            return {
                company: draftSave.journalData.company,
                currentFiscalYear: draftSave.journalData.currentFiscalYear,
                currentCompanyName: draftSave.journalData.currentCompanyName || '',
                companyDateFormat: draftSave.journalData.companyDateFormat || 'english',
        ...
        const fetchInitialData = async () => {
            try {
                ...
                if (response.data.success) {
                    ...
                    const hasDraftDates = draftSave?.journalData?.fromDate &&
                        draftSave?.journalData?.toDate;
    ...
    }, [journalVouchers, searchQuery, selectedRowIndex, dateRange.fromDate, dateRange.toDate, dateRange.fromDateAd, dateRange.toDateAd, companyInfo]);
    ...
    // Fetch data when generate report is clicked - ONLY UPDATES JOURNAL VOUCHERS, NOT INPUT FIELDS
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (response.data.success) {
                    // ONLY update journal vouchers - keep everything else unchanged
    ...
    `;
        ...
        rowsToPrint.forEach(voucher => {
            ...
            tableContent += `
            <tr>
                <td class="nowrap">${voucher.nepaliDate || ''}</td>
                <td class="nowrap">${voucher.date ? new Date(voucher.date).toLocaleDateString() : ''}</td>
                <td class="nowrap">${voucher.billNumber || ''}</td>
        ...
        return (
            ...
            >
                ...
                <div className="d-flex align-items-center justify-content-center px-1 border-end" style={{ width: `${columnWidths.bsDate}px`, flexShrink: 0, height: '100%' }}>
                    <span style={{ fontSize: '0.75rem' }}>{voucher.nepaliDate || ''}</span>
                </div>
                ...
                <div className="d-flex align-items-center justify-content-center px-1 border-end" style={{ width: `${columnWidths.adDate}px`, flexShrink: 0, height: '100%' }}>
                    <span style={{ fontSize: '0.75rem' }}>{voucher.date ? new Date(voucher.date).toLocaleDateString() : ''}</span>
                </div>
                ...
                <div className="d-flex align-items-center px-1 border-end" style={{ width: `${columnWidths.voucherNo}px`, flexShrink: 0, height: '100%', overflow: 'hidden' }}>
                    <span style={{ fontSize: '0.75rem' }}>{voucher.billNumber || ''}</span>
                </div>
                ...
                {/* Description */}
                <div className="d-flex align-items-center px-1 border-end" style={{ width: `${columnWidths.description}px`, flexShrink: 0, height: '100%', overflow: 'hidden' }} title={voucher.description || ''}>
                    <span style={{
                        fontSize: '0.75rem',
                        whiteSpace: 'nowrap',
                        overflow: 'hidden',
                    ...
                    <div className="row g-2 mb-3">
                            ...
                            <div className="position-relative">
                                <div className="input-group input-group-sm">
                                    <input
                                        ...
                                        disabled={vouchersArray.length === 0}
                                        autoComplete='off'
                                        style={{ height: '26px', fontSize: '0.875rem', paddingTop: '0.75rem', width: '100%' }}
                                    />
                                </div>
                        ...
                        <div className="col-12 col-md-auto d-flex align-items-end justify-content-end gap-2">
                            ...
                            <button className="btn btn-secondary btn-sm d-flex align-items-center"
                                onClick={() => handlePrint(true)} disabled={vouchersArray.length === 0}
                                style={{ height: '30px', padding: '0 12px', fontSize: '0.8rem', fontWeight: '500', whiteSpace: 'nowrap' }}>
