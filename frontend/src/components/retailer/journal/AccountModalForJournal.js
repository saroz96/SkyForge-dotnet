...
const AccountModalForJournal = ({
    ...
    const [currentFocus, setCurrentFocus] = useState(-1); // -1 = no row focused
    ...
    // Reset focus when accounts change - only if currentFocus is out of bounds
    useEffect(() => {
    ...
    // Scroll to item - only for keyboard navigation
    const scrollToItem = useCallback((index) => {
        if (index < 0 || !listRef.current || !rowRefs.current[index]) return;
    ...
    const handleKeyDown = useCallback((e) => {
        ...
        if (e.target === searchInputRef.current && e.key === 'Enter') {
            e.preventDefault();
            if (currentFocus >= 0 && currentAccounts[currentFocus]) {
                onSelectAccount(currentAccounts[currentFocus]);
            } else if (currentAccounts.length === 1) {
                onSelectAccount(currentAccounts[0]);
            } else {
                handleClose();
            }
            return;
        ...
        if (e.key === 'ArrowDown') {
            e.preventDefault();
            // If no row is focused (-1), start from 0
            const nextFocus = currentFocus === -1 ? 0 : Math.min(currentFocus + 1, currentAccounts.length - 1);
            ...
            e.preventDefault();
            // If no row is focused (-1), start from last
            const nextFocus = currentFocus === -1 ? currentAccounts.length - 1 : Math.max(currentFocus - 1, 0);
            setCurrentFocus(nextFocus);
            setTimeout(() => {
                scrollToItem(nextFocus);
            }, 50);
    ...
    }, [accounts, currentFocus, onSelectAccount, scrollToItem, onCreateAccount]);
    ...
    // OPTIMIZED: Handle scroll for infinite loading - SAME AS PAYMENT RECEIPT
    ...
    useEffect(() => {
        ...
        // Reset focus to -1 when search changes
        setCurrentFocus(-1);
    }, [searchQuery]);
    ...
    // Get balance color based on balance type
                                            ...
                                            {accounts.map((account, index) => {
                                                ...
                                                return (
                                                    <div
                                                        ...
                                                        // REMOVED: onMouseEnter - prevents mouse from changing focus
                                                        style={{
                                                            display: 'grid',
                                                            gridTemplateColumns: '2fr 1.8fr 1.2fr 1.2fr',
                                                            alignItems: 'center',
...
export default AccountModalForJournal;