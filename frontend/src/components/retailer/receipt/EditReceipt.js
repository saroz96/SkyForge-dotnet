...
const EditReceipt = () => {
    ...
    useEffect(() => {
        const fetchReceiptData = async () => {
            try {
                ...
                // Get the nepali date from receipt
                const nepaliDateStr = receipt.nepaliDate || currentNepaliDate;
                const adDateStr = formatDate(receipt.date) || new Date().toISOString().split('T')[0];
                ...
                // Set selected account ID for balance display
                if (creditEntry?.accountId) {
                    setSelectedAccountId(creditEntry.accountId);
                }
    ...
    const selectAccount = (account) => {
        ...
        // Update selected account ID for balance display if it's the party account (index 1)
        if (currentEntryIndexForModal === 1) {
                                        ...
                                        <div className="position-relative">
                                            ...
                                            >
                                                <AccountBalanceDisplay
                                                    accountId={selectedAccountId || creditEntry.accountId}
                                                    api={api}
