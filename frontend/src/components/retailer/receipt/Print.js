...
//             try {
//                 setLoading(true);
//                 const response = await api.get(`/api/retailer/receipts/${id}/print`);

//                 if (!response.data.success) {
//                     throw new Error(response.data.error || 'Failed to fetch receipt data');
//                 }
...
//                 setError(err.response?.data?.error || err.message || 'Failed to fetch receipt data');
//                 setLoading(false);
//             }
//         };
...
//     if (!receiptData || !receiptData.receipt) return (
//         <Container fluid className="mt-4">
...
//                     /* Fixed column widths for print - Receipt specific */
...
//                     /* Compact column widths for screen - Receipt specific */
...
//                             <h1 className="compact-text">{receiptData.currentCompanyName}</h1>
//                             <h4 className="compact-text">
//                                 {receiptData.currentCompany.address}, {receiptData.currentCompany.city}
//                                 <br />
//                                 Tel: {receiptData.currentCompany.phone} | PAN: {receiptData.currentCompany.pan}
//                             </h4>
//                             <h2 className="compact-text">RECEIPT VOUCHER</h2>
//                         </div>
//                         <br />
//                         <div className="details-container compact-text">
//                             <div className="left">
//                                 <div><strong>Vch. No:</strong> {receipt.billNumber}</div>
//                             </div>
//                             <div className="right">
//                                 <div><strong>Date:</strong> {receiptData.companyDateFormat === 'Nepali' ? formatDate(receipt.nepaliDate, 'nepali') : formatDate(receipt.date)}</div>
//                             </div>
//                         </div>
...
//                                         {!isCanceled ?
//                                             formatTo2Decimal(receipt.credit) :
...
//                         <div className="receipt-details compact-text">
//                             <div><strong>Note:</strong> {receipt.description || 'N/A'}</div>
//                             <div className="mt-1">
//                                 <span className="me-3"><strong>Mode of Receipt:</strong> {getInstrumentTypeName(receipt.instType)}</span>
//                                 {receipt.bankAcc && <span className="me-3"><strong>Bank:</strong> {receipt.bankAcc}</span>}
//                                 <span><strong>Inst No:</strong> {receipt.instNo || 'N/A'}</span>
//                             </div>
//                         </div>

//                         <div className="amount-in-words compact-text">
//                             <strong>In Words:</strong> {numberToWordsWithPaisa(receipt.credit)} Only.
//                         </div>

//                         <div className="signature-area">
//                             <div className="signature-box">
//                                 <div className="compact-text"><strong>{receipt.user?.name || 'N/A'}</strong></div>
//                                 Prepared By
//                             </div>
//                             <div className="signature-box">
...
//                                 <td className="print-text-right">
//                                     {!isCanceled ?
//                                         formatTo2Decimal(receipt.debit) :
//                                         <span className="text-danger">0.00</span>}
//                                 </td>
//                                 <td className="print-text-right">0.00</td>
//                             </tr>
...
//                     <div className="print-receipt-details">
//                         <div><strong>Note:</strong> {receipt.description || 'N/A'}</div>
//                         <div style={{ marginTop: '1mm' }}>
//                             <div><strong>Mode of Receipt:</strong> {getInstrumentTypeName(receipt.instType)}</div>
//                             {receipt.bankAcc && <div><strong>Bank:</strong> {receipt.bankAcc}</div>}
//                             <div><strong>Inst No:</strong> {receipt.instNo || 'N/A'}</div>
//                         </div>
//                     </div>
...
//                                 <strong>{receipt.user?.name || 'N/A'}</strong>
//                             </div>
//                             Prepared By
//                         </div>
//                         <div className="print-signature-box">
...
const ReceiptVoucherPrint = () => {
    ...
    const receipt = receiptData.receipt;
    const debitEntries = receiptData.debitEntries || [];
    const creditEntries = receiptData.creditEntries || [];
    const isCanceled = receipt.status === 'Canceled';
                ...
                <Container fluid>
                    ...
                    <Card className="p-4">
                        ...
                        <div className="receipt-details compact-text">
                            <div><strong>Note:</strong> {receipt.description || 'N/A'}</div>
                            <div className="mt-1">
                                {debitEntries[0] && debitEntries[0].instType !== 'N/A' && (
                                    <span className="me-3">
                                        <strong>Mode of Receipt:</strong> {getInstrumentTypeName(debitEntries[0].instType)}
                                    </span>
                                )}
                                {debitEntries[0] && debitEntries[0].bankAcc && (
                                    <span className="me-3">
                                        <strong>Bank:</strong> {debitEntries[0].bankAcc}
                                    </span>
                                )}
                                {debitEntries[0] && debitEntries[0].instNo && (
                                    <span>
                                        <strong>Inst No:</strong> {debitEntries[0].instNo}
                                    </span>
                                )}
                            </div>
                        </div>
            ...
            <div id="printableContent" className="print-version" ref={printableRef}>
                <div className="print-voucher-container">
                    ...
                    <div className="print-receipt-details">
                        ...
                        <div style={{ marginTop: '1mm' }}>
                            {debitEntries[0] && debitEntries[0].instType !== 'N/A' && (
                                <div><strong>Mode of Receipt:</strong> {getInstrumentTypeName(debitEntries[0].instType)}</div>
                            )}
                            {debitEntries[0] && debitEntries[0].bankAcc && (
                                <div><strong>Bank:</strong> {debitEntries[0].bankAcc}</div>
                            )}
                            {debitEntries[0] && debitEntries[0].instNo && (
                                <div><strong>Inst No:</strong> {debitEntries[0].instNo}</div>
                            )}
                        </div>
                    </div>
