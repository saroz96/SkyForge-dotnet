...
const AddReceipt = () => {
    ...
    // Fetch date preference setting from backend for Receipt
    const fetchDatePreference = async () => {
        try {
            console.log('=== fetchDatePreferenceForReceipt CALLED ===');
            const response = await api.get('/api/retailer/date-preference/receipt');
            console.log('Date preference response:', response.data);

            if (response.data.success) {
                const useVoucherDate = response.data.data.useVoucherLastDate;
                console.log('useVoucherLastDateForReceipt value from API:', useVoucherDate);
                setUseVoucherLastDateForReceipt(useVoucherDate);
    ...
    // Fetch last receipt date from backend
    const fetchLastReceiptDate = async () => {
        try {
            console.log('=== fetchLastReceiptDate CALLED ===');

            // Use the endpoint: /api/retailer/last-receipt-date
            const response = await api.get('/api/retailer/last-receipt-date');
            console.log('Last receipt date response:', response.data);
            ...
            if (response.data.success && response.data.data) {
                ...
                if (lastDate) {
                    ...
                    console.log('Formatted last receipt date:', formattedDate);
                    setLastReceiptDate(formattedDate);
                    return formattedDate;
                }
            }

            console.log('No last receipt date found - returning null');
            return null;
        } catch (error) {
            console.error('Error fetching last receipt date:', error);
            return null;
        }
    };
    ...
    useEffect(() => {
        const fetchReceiptFormData = async () => {
            try {
                ...
                // Fetch last receipt date if needed
                let lastDate = null;
                if (useVoucherDate) {
                    lastDate = await fetchLastReceiptDate();
                }
    ...
    const handleAccountSearch = (e) => {
        ...
        const timer = setTimeout(() => {
            fetchAccountsFromBackend(searchText, 1, false);
        }, 300);
            ...
            <div id="printableContent">
                <div class="print-voucher-container">
                    ...
                    <div class="print-voucher-details">
                        ...
                        <div>
                            <div><strong>Date:</strong> ${printData.companyDateFormat === 'nepali' ? formatDateForInput(printData.receipt.nepaliDate) : formatDateForInput(printData.receipt.date)}(${new Date(printData.receipt.date).toLocaleDateString()})</div>
                        </div>
                    </div>
    ...
    return (
        <div className="container-fluid">
            ...
            <div className="container mt-4 wow-form">
                <div className="card mt-2 shadow-lg p-2 animate__animated animate__fadeInUp expanded-card ledger-card compact" style={{ minHeight: '450px' }}>
                                        ...
                                        <div className="position-relative">
                                            ...
                                            >
                                                <AccountBalanceDisplay
                                                    accountId={selectedAccountId || formData.accountId}
                                                    api={api}
            ...
            {/* Account Modal - Replace the existing one with this */}
            {showAccountModal && (
