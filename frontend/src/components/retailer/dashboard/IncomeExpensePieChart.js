...
const IncomeExpensePieChart = ({ companyId, companyName, fiscalYearJson }) => {
  ...
  // ✅ Don't auto-init from here; only update if the chart exists
  const updateChart = (labels, seriesData, colors = null, netTotal = null) => {
          ...
          donut: {
            ...
            labels: {
              ...
              total: {
                ...chartOptions.plotOptions.pie.donut.labels.total,
                label: 'Net Total',
                formatter: () => 'Rs. ' + Number(netTotal).toLocaleString()
              }
            }
          }
        }
  ...
  // ✅ Effect 1 — Fetch data on mount / company change (no auto-refresh)
  useEffect(() => {
    if (!companyId || !currentCompany) return;
    fetchData(!pieChartDraftSave);
  ...
  // ✅ Effect 2 — Init chart AFTER loading resolves and the div exists
  useEffect(() => {
    if (!companyId || !currentCompany) return;
    if (loading) return;
    ...
    const timer = setTimeout(() => {
      ...
      if (pieChartDraftSave && pieChartDraftSave.labels && chartInstance.current) {
        const netTotal =
          (pieChartDraftSave.summary?.totalIncome || 0) -
          (pieChartDraftSave.summary?.totalExpenses || 0);
