...
const SalesChart = ({ companyId, companyName, fiscalYearJson }) => {
  ...
  const chartOptions = {
    ...
    xaxis: {
      categories: [],
      labels: {
        style: { colors: '#6b7280', fontSize: '10px', fontWeight: 400 },
        offsetY: -3
      },
      axisBorder: { show: false },
      axisTicks: { show: false }
    ...
    yaxis: {
      labels: {
        ...
        style: { colors: '#6b7280', fontSize: '9px', fontWeight: 400 },
        offsetX: -5
      },
      axisBorder: { show: false },
      axisTicks: { show: false }
  ...
  const fetchData = useCallback(async (force = false) => {
    ...
    try {
      if (force || !salesChartDraftSave) setLoading(true);
      setError(null);
  ...
  // ✅ Effect 1 — Fetch data on mount / company change
  useEffect(() => {
    if (!companyId || !currentCompany) return;
    fetchData(!salesChartDraftSave);
  ...
  // ✅ Effect 2 — Init chart AFTER the loading state resolves and the div exists
  useEffect(() => {
    if (!companyId || !currentCompany) return;
    if (loading) return;         // wait until real JSX is rendered

    const timer = setTimeout(() => {
      initChart();

      // Replay cached data after the chart is ready
      if (salesChartDraftSave && chartInstance.current) {
        updateChart(salesChartDraftSave.categories, salesChartDraftSave.seriesData);
        setDataStatus('cached');
      }
    }, 0);
  ...
  }, [companyId, currentCompany, loading]);
