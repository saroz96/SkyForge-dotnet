...
const AddCashSalesReturn = () => {
    ...
    const getCurrentBillNumber = async () => {
        try {
            const response = await api.get('/api/retailer/cash/sales-return/current-number');
            return response.data.data.currentCashSalesReturnBillNumber;
        } catch (error) {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                ...
                const numberResponse = await api.get('/api/retailer/cash/sales-return/current-number');
                const currentBillNum = numberResponse.data.data.currentCashSalesReturnBillNumber;
                ...
                setNotification({
                    show: true,
                    message: 'Error loading cash sales return data',
                    type: 'error'
                });
            } finally {
                setIsLoading(false);
    ...
    // Insert selected item from header row
    ...
    const handleSubmit = async (e, print = false) => {
        ...
        if (!formData.cashAccount) {
            setNotification({
                show: true,
                message: 'Please select a cash account first',
                type: 'error'
            });
            return;
        }
        ...
        try {
            ...
            if (response.data.success) {
                setNotification({
                    show: true,
                    message: 'Cash sales return saved successfully!',
                    type: 'success'
                });
                ...
                setNotification({
                    show: true,
                    message: response.data.message || response.data.error || 'Failed to save cash sales return',
                    type: 'error'
                });
                setIsSaving(false);
            }
        } catch (error) {
            console.error('Error saving cash sales return:', error);
            setNotification({
                show: true,
                message: error.response?.data?.message || error.response?.data?.error || 'Failed to save cash sales return',
                type: 'error'
            });
            setIsSaving(false);
        }
    ...
    }, [showTransactionModal, showAccountCreationModal, showItemsModal, showBatchSelectionModal]);
                    ...
                    <div class="print-invoice-container">
                        ...
                        <div class="print-invoice-details">
                            ...
                            <div>
                                ...
                                <div><strong>Trans. Date:</strong> ${printData.companyDateFormat === 'nepali' ? formatDateForInput(printData.transactionDateNepali) : formatDateForInput(printData.bill.transactionDate)}(${new Date(printData.bill.transactionDate).toLocaleDateString()})</div>
                                <div><strong>Invoice Date:</strong> ${printData.companyDateFormat === 'nepali' ? formatDateForInput(printData.nepaliDate) : formatDateForInput(printData.bill.date)}(${new Date(printData.bill.date).toLocaleDateString()})</div>
    ...
    const fetchCashSalesBillDetails = async (e) => {
        ...
        try {
            const response = await api.get(`/api/retailer/cash/sales-bill-by-number/${invoiceNumber}`);
            ...
            if (response.data.success) {
                ...
                if (billData.billType !== 'cash') {
                    ...
                    setNotification({
                        show: true,
                        message: `Bill ${invoiceNumber} is not a cash sales bill. Please check the bill type.`,
                        type: 'warning'
                    });
                ...
                // Update cash account information
                setFormData(prev => ({
                    ...prev,
                    ...
                    paymentMode: billData.billSummary?.paymentMode || 'cash',
                ...
                setNotification({
                    show: true,
                    message: `✅ Cash sales bill ${invoiceNumber} loaded successfully. ${transformedItems.length} items found.`,
                    type: 'success'
                });
                ...
                if (errorData.isCreditSales) {
                    setNotification({
                        show: true,
                        message: `❌ ${invoiceNumber} is a Credit Sales bill!\n\nAccount: ${errorData.account || 'N/A'}\nAddress: ${errorData.accountAddress || 'N/A'}\n\nCredit sales returns should be created from the Sales Return section.`,
                        type: 'error'
                    });
                } else if (errorData.isCreditSales === false) {
                    setNotification({
                        show: true,
                        message: `❌ ${invoiceNumber} is not a valid cash sales bill.\nPlease check the bill type.`,
                        type: 'error'
                    });
                } else {
                    setNotification({
            ...
            console.error('Error fetching cash sales bill:', error);

            if (error.response?.data?.isCreditSales) {
                ...
                setNotification({
                    show: true,
                    message: 'Error fetching cash sales bill details',
                    type: 'error'
                });
            }
            ...
            <div className="card mt-2 shadow-lg p-2 animate__animated animate__fadeInUp expanded-card ledger-card compact">
                <div className="card-header">
                    <div className="d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            {formData.billNumber === '' && (
                                <span className="badge bg-danger me-2">Return number is required!</span>
                            )}
                    ...
                    <form onSubmit={handleSubmit} id="billForm" className="needs-validation" noValidate>
                        ...
                        <div className="row g-1 mb-2">
                            ...
                            {company.dateFormat === 'nepali' || company.dateFormat === 'Nepali' ? (
                                <>
                                    {/* Nepali Transaction Date (Return Transaction Date) */}
                                    ...
                                    {/* AD Return Date (Auto-converted, Read-only) */}
                                    {/* <div className="col-12 col-md-6 col-lg-2">
                                        <div className="position-relative">
                                            <input
                                                type="text"
                                ...
                                <div className="position-relative">
                                    <div className="input-group">
                                        <input
                                            ...
                                            onBlur={fetchCashSalesBillDetails}
                                            onKeyDown={(e) => {
                                                if (e.key === 'Enter') {
                                                    e.preventDefault();
                                                    fetchCashSalesBillDetails(e);
                            ...
                            <div className="col-12 col-md-6">
                                <div className="position-relative">
                                    <input
                                        ...
                                        // onFocus={() => setShowAccountModal(true)} // Remove this line
                                        onChange={(e) => {
                                            const value = e.target.value;
                                            setFormData({
                                                ...formData,
                                            ...
                                            // Also trigger search when typing
                                        ...
                                        onFocus={() => {
                                            setShowAccountModal(true);
                                            if (accountLastSearchQuery && !accountSearchQuery) {
                                                setAccountShouldShowLastSearchResults(true);
                                                if (accountLastSearchQuery.trim() !== '') {
                                                    fetchAccountsFromBackend(accountLastSearchQuery, 1);
                                                }
                                            }
                                        }}
                        ...
                        <div className="modal-content" style={{ height: '400px' }}>
                            ...
                            <div className="p-2 bg-white sticky-top">
                                <input
                                    ...
                                    id="searchAccount"
                                    className="form-control form-control-sm"
                                    placeholder="Search Cash Account... (Press F6 to create new account)"
                                    autoFocus
                                    autoComplete='off'
