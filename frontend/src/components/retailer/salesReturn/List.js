...
//             return {
//                 fromDate: draftSave.salesReturnData.fromDate || '',
//                 toDate: draftSave.salesReturnData.toDate || '',
//                 fromDateAd: draftSave.salesReturnData.fromDateAd || '',
//                 toDateAd: draftSave.salesReturnData.toDateAd || ''
//             };
//         }
//         return {
//             fromDate: '',
...
//         if (draftSave && draftSave.salesReturnData) {
//             return draftSave.salesReturnData.bills || [];
//         }
//         return [];
//     });
...
//         if (draftSave && draftSave.salesReturnData) {
//             return {
//                 company: draftSave.salesReturnData.company,
//                 currentFiscalYear: draftSave.salesReturnData.currentFiscalYear,
//                 currentCompanyName: draftSave.salesReturnData.currentCompanyName || '',
//                 companyDateFormat: draftSave.salesReturnData.companyDateFormat || 'english',
//                 vatEnabled: draftSave.salesReturnData.vatEnabled !== undefined ? draftSave.salesReturnData.vatEnabled : true,
//                 isVatExempt: draftSave.salesReturnData.isVatExempt || false,
//                 isAdminOrSupervisor: draftSave.salesReturnData.isAdminOrSupervisor || false
//             };
//         }
//         return {
//             company: null,
...
//         if (draftSave && draftSave.salesReturnSearch) {
//             return draftSave.salesReturnSearch.searchQuery || '';
//         }
//         return '';
//     });
...
//         if (draftSave && draftSave.salesReturnSearch) {
//             return draftSave.salesReturnSearch.paymentModeFilter || '';
//         }
//         return '';
//     });
...
//         if (draftSave && draftSave.salesReturnSearch) {
//             return draftSave.salesReturnSearch.selectedRowIndex || 0;
//         }
//         return 0;
//     });
...
//             try {
//                 const response = await api.get('/api/retailer/sales-return/register/entry-data');
...
//                     const hasDraftDates = draftSave?.salesReturnData?.fromDate &&
//                         draftSave?.salesReturnData?.toDate;
...
//     useEffect(() => {
//         const savedWidths = localStorage.getItem('salesReturnTableColumnWidths');
//         if (savedWidths) {
//             try {
...
//     useEffect(() => {
//         localStorage.setItem('salesReturnTableColumnWidths', JSON.stringify(columnWidths));
//     }, [columnWidths]);
...
//                     setError(null);
//                 } else {
//                     setError(response.data.error || 'Failed to fetch sales return bills');
//                 }

//                 if (!draftSave?.salesReturnSearch?.selectedRowIndex) {
...
//     </style>
//     ${printHeader}
//     <div class="report-title">Sales Return's Register</div>
//     <table>
//         <thead>
//             <tr>
...
//             // Header information
//             excelData.push(['Sales Return\'s Register']);
...
//             // Generate filename
//             const fileName = `Sales_Returns_${dateRange.fromDate}_to_${dateRange.toDate}_${currentDate}.xlsx`;
...
//             navigate(`/retailer/sales-return/${filteredBills[selectedRowIndex].id}/print`);
//         }
...
//                 {/* Vch. No. */}
//                 <div className="d-flex align-items-center px-1 border-end position-relative" style={{ width: `${columnWidths.returnNo}px`, flexShrink: 0, minWidth: '60px' }}>
...
//             e.stopPropagation();
//             if (bill && bill.id) {
//                 navigate(`/retailer/sales-return/edit/${bill.id}`);
//             }
//         };
...
//                 {/* Vch. No. */}
//                 <div className="d-flex align-items-center px-1 border-end" style={{ width: `${columnWidths.returnNo}px`, flexShrink: 0, height: '100%', overflow: 'hidden' }}>
...
//                     <h1 className="h4 mb-0 text-center text-primary">Sales Return's Register</h1>
//                 </div>
...
//                                 onClick={() => navigate('/retailer/sales-return')}
...
//                                                 columnWidths.returnNo + columnWidths.partyName +
...
//                                     style={{ width: `${columnWidths.bsDate + columnWidths.adDate + columnWidths.returnNo + columnWidths.partyName + columnWidths.payMode}px`, flexShrink: 0, height: '100%' }}
//                                 >
//                                     <strong style={{ fontSize: '0.75rem' }}>Total:</strong>
//                                 </div>
...
const SalesReturnList = () => {
        ...
        let tableContent = `
            <div class="print-header">
                ...
                <div class="statement-info">
                    ...
                    <strong>Total Returns:</strong> ${rowsToPrint.length}
                </div>
            </div>
            <table cellspacing="0">
                <thead>
    ...
    const TableHeader = React.memo(() => {
        ...
        return (
            ...
            >
                {/* BS Date */}
                <div className="sr-header-cell sr-cell--center" style={{ width: `${columnWidths.bsDate}px`, flexShrink: 0, minWidth: '80px' }}>
                    <strong>Miti</strong>
                    <ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.bsDate - 2} columnName="bsDate" />
                </div>

                {/* AD Date */}
                <div className="sr-header-cell sr-cell--center" style={{ width: `${columnWidths.adDate}px`, flexShrink: 0, minWidth: '80px' }}>
                    <strong>Date</strong>
                    <ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.adDate - 2} columnName="adDate" />
                </div>

                {/* Vch. No. */}
                <div className="sr-header-cell" style={{ width: `${columnWidths.returnNo}px`, flexShrink: 0, minWidth: '60px' }}>
                    <strong>Vch. No.</strong>
                    <ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.returnNo - 3} columnName="returnNo" />
                </div>

                {/* Party Name */}
                <div className="sr-header-cell" style={{ width: `${columnWidths.partyName}px`, flexShrink: 0, minWidth: '100px' }}>
                    <strong>Party Name</strong>
                    <ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.partyName - 3} columnName="partyName" />
                </div>

                {/* Pay Mode */}
                <div className="sr-header-cell" style={{ width: `${columnWidths.payMode}px`, flexShrink: 0, minWidth: '60px' }}>
                    <strong>Pay Mode</strong>
                    <ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.payMode - 2} columnName="payMode" />
                </div>

                {/* Sub Total */}
                <div className="sr-header-cell sr-cell--end" style={{ width: `${columnWidths.subTotal}px`, flexShrink: 0, minWidth: '80px' }}>
                    <strong>Sub Total</strong>
                    <ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.subTotal - 2} columnName="subTotal" />
                </div>

                {/* Discount */}
                <div className="sr-header-cell sr-cell--end" style={{ width: `${columnWidths.discount}px`, flexShrink: 0, minWidth: '80px' }}>
                ...
                {showVatColumns && (
                    <>
                        <div className="sr-header-cell sr-cell--end" style={{ width: `${columnWidths.taxable}px`, flexShrink: 0, minWidth: '50px' }}>
                            <strong>Taxable</strong>
                            <ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.taxable - 1} columnName="taxable" />
                        </div>
                        <div className="sr-header-cell sr-cell--end" style={{ width: `${columnWidths.vat}px`, flexShrink: 0, minWidth: '60px' }}>
                            <strong>VAT</strong>
                            <ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.vat - 1} columnName="vat" />
                        </div>
                    </>
                ...
                {/* Round Off */}
                <div className="sr-header-cell sr-cell--end" style={{ width: `${columnWidths.roundOff}px`, flexShrink: 0, minWidth: '80px' }}>
                    <strong>Off(-/+)</strong>
                    <ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.roundOff - 2} columnName="roundOff" />
                </div>

                {/* Total */}
                <div className="sr-header-cell sr-cell--end" style={{ width: `${columnWidths.total}px`, flexShrink: 0, minWidth: '80px' }}>
                    <strong>Total</strong>
                    <ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.total - 2} columnName="total" />
                </div>

                {/* User */}
                <div className="sr-header-cell" style={{ width: `${columnWidths.user}px`, flexShrink: 0, minWidth: '80px' }}>
                    <strong>User</strong>
                    <ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.user - 2} columnName="user" />
                </div>

                {/* Actions */}
                <div className="sr-header-cell" style={{ width: `${columnWidths.actions}px`, flexShrink: 0, minWidth: '100px' }}>
    ...
    const TableRow = React.memo(({ index, style, data: rowData }) => {
        ...
        return (
            ...
            >
                {/* BS Date */}
                <div className="sr-cell sr-cell--center" style={{ width: `${columnWidths.bsDate}px`, flexShrink: 0, height: '100%' }}>
                    <span>{bill.nepaliDate || ''}</span>
                </div>

                {/* AD Date */}
                <div className="sr-cell sr-cell--center" style={{ width: `${columnWidths.adDate}px`, flexShrink: 0, height: '100%' }}>
                    <span>{bill.date ? new Date(bill.date).toLocaleDateString() : ''}</span>
                </div>

                {/* Vch. No. */}
                <div className="sr-cell" style={{ width: `${columnWidths.returnNo}px`, flexShrink: 0, height: '100%', overflow: 'hidden' }}>
                    <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{bill.billNumber || ''}</span>
                </div>

                {/* Party Name */}
                <div className="sr-cell" style={{ width: `${columnWidths.partyName}px`, flexShrink: 0, height: '100%', overflow: 'hidden' }} title={bill.accountName || bill.cashAccount || 'N/A'}>
                    <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{bill.accountName || bill.cashAccount || 'N/A'}</span>
                </div>

                {/* Pay Mode */}
                <div className="sr-cell" style={{ width: `${columnWidths.payMode}px`, flexShrink: 0, height: '100%' }}>
                    <span>{bill.paymentMode || ''}</span>
                </div>

                {/* Sub Total */}
                <div className="sr-cell sr-cell--end" style={{ width: `${columnWidths.subTotal}px`, flexShrink: 0, height: '100%' }}>
                    <span>{formatCurrency(bill.subTotal)}</span>
                </div>

                {/* Discount */}
                <div className="sr-cell sr-cell--end" style={{ width: `${columnWidths.discount}px`, flexShrink: 0, height: '100%' }}>
                    <span>{(bill.discountPercentage || 0).toFixed(2)}% - {formatCurrency(bill.discountAmount)}</span>
                </div>

                {showVatColumns && (
                    <>
                        <div className="sr-cell sr-cell--end" style={{ width: `${columnWidths.taxable}px`, flexShrink: 0, height: '100%' }}>
                            <span>{formatCurrency(bill.taxableAmount)}</span>
                        </div>
                        <div className="sr-cell sr-cell--end" style={{ width: `${columnWidths.vat}px`, flexShrink: 0, height: '100%' }}>
                ...
                {/* Round Off */}
                <div className="sr-cell sr-cell--end" style={{ width: `${columnWidths.roundOff}px`, flexShrink: 0, height: '100%' }}>
                    <span>{formatCurrency(bill.roundOffAmount)}</span>
                </div>

                {/* Total */}
                <div className="sr-cell sr-cell--end" style={{ width: `${columnWidths.total}px`, flexShrink: 0, height: '100%' }}>
                    <span>{formatCurrency(bill.totalAmount)}</span>
                </div>

                {/* User */}
                <div className="sr-cell" style={{ width: `${columnWidths.user}px`, flexShrink: 0, height: '100%', overflow: 'hidden' }} title={bill.userName || 'N/A'}>
                    <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{bill.userName || 'N/A'}</span>
                </div>

                {/* Actions */}
                <div className="sr-cell sr-cell--center" style={{ width: `${columnWidths.actions}px`, flexShrink: 0, height: '100%' }}>
                    <button
                        className="sr-btn-action sr-btn-action--info"
                    ...
                    <button
                        className="sr-btn-action sr-btn-action--warning"
            ...
            <div className="sr-shell">
                ...
                <div className="sr-topbar">
                    <div className="sr-topbar__left">
                        <div className="sr-topbar__icon"><FiFileText /></div>
                        <div><h1>Sales Return's Register</h1></div>
                    </div>
                    <div className="sr-topbar__actions">
                        <button className="sr-btn-icon" onClick={() => navigate('/retailer/sales-return')} title="Add Return"><FiPlus /> Add</button>
                        <button className="sr-btn-icon" onClick={handleExportExcel} disabled={filteredBills.length === 0 || exporting}>
                            <FiDownload /> {exporting ? '…' : 'Excel'}
                        </button>
                        <button className="sr-btn-icon" onClick={() => handlePrint(true)} disabled={billsArray.length === 0}>
                            <FiPrinter /> Print
                        </button>
                        <button className="sr-btn-icon" onClick={resetColumnWidths} title="Reset columns">
                            <FiRefreshCw /> Reset
                        </button>
                    </div>
                </div>
                ...
                <div className="sr-toolbar">
                    <div className="sr-field sr-field--date">
                        ...
                        <input
                            ...
                            autoComplete="off"
                        />
                        {dateErrors.fromDate && <div className="sr-field-error">{dateErrors.fromDate}</div>}
                    </div>
                    ...
                    <div className="sr-field sr-field--date">
                        ...
                        <input
                            ...
                            autoComplete="off"
                        />
                        {dateErrors.toDate && <div className="sr-field-error">{dateErrors.toDate}</div>}
                    </div>
                    ...
                    <button type="button" id="generateReport" ref={generateReportRef} className="sr-btn-gen" onClick={handleGenerateReport} disabled={loading}>
                        {loading ? <span className="spinner-border spinner-border-sm" style={{ width: 12, height: 12 }} /> : <><FiSearch className="me-1" /> Generate</>}
                    </button>

                    <div className="sr-toolbar-divider" />

                    <div className="sr-field sr-field--search">
                        <label>Search</label>
                        <div className="sr-search-wrap">
                            ...
                            <input
                                ...
                                autoComplete="off"
                            />
                            {searchQuery && <button className="sr-search-clear" onClick={() => setSearchQuery('')}>×</button>}
                        </div>
                    </div>

                    <div className="sr-field sr-field--select">
                        <label>Mode</label>
                        <select
                            id="paymentModeFilter"
                            ref={paymentModeFilterRef}
                    ...
                    {billsArray.length === 0 && !loading ? (
                        ...
                        <>
                            ...
                            <div className="sr-footer">
                                <div className="sr-footer-cell" style={{ width: `${columnWidths.bsDate + columnWidths.adDate + columnWidths.returnNo + columnWidths.partyName + columnWidths.payMode}px`, flexShrink: 0 }}>
                                    <strong>Totals:</strong>
                                </div>
                                <div className="sr-footer-cell sr-cell--end" style={{ width: `${columnWidths.subTotal}px`, flexShrink: 0 }}>
                                    <strong>{formatCurrency(totals.subTotal)}</strong>
                                </div>
                                <div className="sr-footer-cell sr-cell--end" style={{ width: `${columnWidths.discount}px`, flexShrink: 0 }}>
                                    <strong>{formatCurrency(totals.discount)}</strong>
                                </div>
                                {companyInfo.vatEnabled && !companyInfo.isVatExempt && (
                                    <>
                                        <div className="sr-footer-cell sr-cell--end" style={{ width: `${columnWidths.taxable}px`, flexShrink: 0 }}>
                                            <strong>{formatCurrency(totals.taxable)}</strong>
                                        </div>
                                        <div className="sr-footer-cell sr-cell--end" style={{ width: `${columnWidths.vat}px`, flexShrink: 0 }}>
                                            <strong>{formatCurrency(totals.vat)}</strong>
                                        </div>
                                    </>
                                )}
                                <div className="sr-footer-cell sr-cell--end" style={{ width: `${columnWidths.roundOff}px`, flexShrink: 0 }}>
                                    <strong>{formatCurrency(totals.roundOff)}</strong>
                                </div>
                                <div className="sr-footer-cell sr-cell--end" style={{ width: `${columnWidths.total}px`, flexShrink: 0 }}>
                                    <strong>{formatCurrency(totals.amount)}</strong>
                                </div>
                                <div className="sr-footer-cell" style={{ width: `${columnWidths.user + columnWidths.actions}px`, flexShrink: 0 }}></div>
                            </div>
                        </>
                    )}
                </div>
