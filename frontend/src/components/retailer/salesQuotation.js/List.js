...
const SalesQuotationList = () => {
    ...
    const [data, setData] = useState(() => {
        ...
        return {
            company: null,
            currentFiscalYear: null,
            bills: [], // Initialize as empty array
            fromDate: '',
            toDate: '',
            currentCompanyName: '',
    ...
    // Fetch company and fiscal year info from sales quotation entry data
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                ...
                if (response.data.success) {
                    ...
                    if (!hasDraftDates && currentFiscalYear) {
                        ...
                        setData(prev => ({
                            ...
                            bills: prev.bills || [] // Ensure bills array exists
                        }));
                    } else {
                        setData(prev => ({
                            ...prev,
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (response.data.success) {
                    setData(prev => ({
                        ...prev,
                        bills: response.data.data.bills || [], // Ensure we have an array
    ...
    useEffect(() => {
        // Ensure data.bills is an array before filtering
        const billsArray = Array.isArray(data.bills) ? data.bills : [];
    ...
    const handlePrint = (filtered = false) => {
        const rowsToPrint = filtered ? filteredBills : (Array.isArray(data.bills) ? data.bills : []);
        ...
        rowsToPrint.forEach(bill => {
            ...
            tableContent += `
            <tr>
                <td class="nowrap">${date.toLocaleDateString()}</td>
    ...
    // Ensure data.bills is always an array
    const billsArray = Array.isArray(data.bills) ? data.bills : [];
