...
const SalesQuotationList = () => {
    ...
    const [dateRange, setDateRange] = useState(() => {
        if (salesQuotationDraftSave && salesQuotationDraftSave.salesQuotationData) {
            return {
                fromDate: salesQuotationDraftSave.salesQuotationData.fromDate || '',
                toDate: salesQuotationDraftSave.salesQuotationData.toDate || '',
                fromDateAd: salesQuotationDraftSave.salesQuotationData.fromDateAd || '',
                toDateAd: salesQuotationDraftSave.salesQuotationData.toDateAd || ''
            };
        }
        return {
            fromDate: '',
    ...
    const [bills, setBills] = useState(() => {
        if (salesQuotationDraftSave && salesQuotationDraftSave.salesQuotationData) {
            return salesQuotationDraftSave.salesQuotationData.bills || [];
        }
        return [];
    });
    ...
    const [companyInfo, setCompanyInfo] = useState(() => {
        if (salesQuotationDraftSave && salesQuotationDraftSave.salesQuotationData) {
            return {
                company: salesQuotationDraftSave.salesQuotationData.company,
                currentFiscalYear: salesQuotationDraftSave.salesQuotationData.currentFiscalYear,
                currentCompanyName: salesQuotationDraftSave.salesQuotationData.currentCompanyName || '',
                companyDateFormat: salesQuotationDraftSave.salesQuotationData.companyDateFormat || 'english',
        ...
        const fetchInitialData = async () => {
            try {
                ...
                if (response.data.success) {
                    ...
                    const hasDraftDates = salesQuotationDraftSave?.salesQuotationData?.fromDate &&
                        salesQuotationDraftSave?.salesQuotationData?.toDate;
                    ...
                    if (!hasDraftDates && currentFiscalYear) {
                        ...
                        // Keep existing dates from draft
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (response.data.success) {
                    ...
                    // Update company info only if needed (e.g., VAT settings changed)
    ...
    const handleDateChange = (e, isFromDate = true) => {
        const { value } = e.target;
        const sanitizedValue = value.replace(/[^0-9/-]/g, '').slice(0, 10);

        // Always treat input as BS date
        ...
        setDateErrors(prev => ({ ...prev, [isFromDate ? 'fromDate' : 'toDate']: '' }));
    };
    ...
    const handlePrint = (filtered = false) => {
        ...
        if (rowsToPrint.length === 0) {
            alert("No sales quotations to print");
            return;
        }
    ...
    const handleExportExcel = async () => {
        ...
        try {
            ...
            // Header information
            excelData.push(['Sales Quotation Register']);
            ...
            excelData.push(['Total Quotations:', filteredBills.length]);
            ...
            // Generate filename
            const fileName = `Sales_Quotations_${dateRange.fromDate}_to_${dateRange.toDate}_${currentDate}.xlsx`;
    ...
    const handleRowDoubleClick = useCallback((billId) => {
        if (filteredBills[selectedRowIndex]) {
            navigate(`/retailer/sales-quotation/${filteredBills[selectedRowIndex].id}/print`);
        }
    ...
    const TableHeader = React.memo(() => {
        ...
        const totalWidth = columnWidths.bsDate + columnWidths.adDate + columnWidths.quotNo +
        ...
        return (
            ...
            >
                ...
                <div className="d-flex align-items-center px-1 border-end position-relative" style={{ width: `${columnWidths.quotNo}px`, flexShrink: 0, minWidth: '60px' }}>
                    <strong style={{ fontSize: '0.75rem' }}>Quot No.</strong>
                    <ResizeHandle onResizeStart={handleResizeStart} left={columnWidths.quotNo - 3} columnName="quotNo" />
                </div>
    ...
    const TableRow = React.memo(({ index, style, data: rowData }) => {
        ...
        const handleDoubleClick = () => {
            if (bill && bill.id) {
                navigate(`/retailer/sales-quotation/${bill.id}/print`);
            }
        };
        ...
        const handleEditClick = (e) => {
            e.stopPropagation();
            if (bill && bill.id) {
                navigate(`/retailer/sales-quotation/edit/${bill.id}`);
            }
        };
        ...
        return (
            ...
            >
                ...
                <div className="d-flex align-items-center px-1 border-end" style={{ width: `${columnWidths.quotNo}px`, flexShrink: 0, height: '100%', overflow: 'hidden' }}>
                    <span style={{ fontSize: '0.75rem' }}>{bill.billNumber || ''}</span>
                </div>
                <div className="d-flex align-items-center px-1 border-end" style={{ width: `${columnWidths.partyName}px`, flexShrink: 0, height: '100%', overflow: 'hidden' }} title={bill.accountName || 'N/A'}>
                        ...
                        <>
                                    ...
                                    <AutoSizer>
                                        {({ height, width }) => {
                                            ...
                                            const totalWidth = columnWidths.bsDate + columnWidths.adDate +
                                                columnWidths.quotNo + columnWidths.partyName +
                            ...
                            >
                                <div
                                    ...
                                    style={{ width: `${columnWidths.bsDate + columnWidths.adDate + columnWidths.quotNo + columnWidths.partyName + columnWidths.payMode}px`, flexShrink: 0, height: '100%' }}
                                >
                                    <strong style={{ fontSize: '0.75rem' }}>Total:</strong>
                                </div>
