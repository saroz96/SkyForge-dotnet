...
//     // Date range state with both BS and AD dates - now includes fromDate and asOnDate
//     const [dateRange, setDateRange] = useState(() => {
//         if (draftSave?.dayWiseAgeingData) {
//             return {
//                 fromDate: draftSave.dayWiseAgeingData.fromDate || '',
//                 toDate: draftSave.dayWiseAgeingData.toDate || '',
//                 fromDateAd: draftSave.dayWiseAgeingData.fromDateAd || '',
//                 toDateAd: draftSave.dayWiseAgeingData.toDateAd || ''
//             };
//         }
//         return {
//             fromDate: '',
...
//                     transactions: [],
//                     summary: {},
//                     openingBalanceBeforeFromDate: 0,
...
//                         } else {
//                             fromDateFormatted = responseData.currentFiscalYear.startDate
//                                 ? new Date(responseData.currentFiscalYear.startDate).toISOString().split('T')[0]
//                                 : currentEnglishDate;
...
//             if (!isValidNepaliDate(dateRange.fromDate)) {
//                 setDateErrors(prev => ({ ...prev, fromDate: 'Invalid date format' }));
//                 fromDateRef.current?.focus();
//                 return;
//             }
//             if (!isValidNepaliDate(dateRange.toDate)) {
...
//                 console.log('Opening balance before from date:', response.data.data.agingData?.openingBalanceBeforeFromDate);
//                 setData(response.data.data);
//                 setHasGenerated(true);
//                 setSelectedRowIndex(0);

//                 if (response.data.data.agingData?.transactions?.length === 0) {
//                     setNotification({ show: true, message: 'No transactions found for this account and date range', type: 'warning', duration: 3000 });
//                 } else {
...
//         // Focus on fromDate input after modal closes
//         setTimeout(() => {
...
//             </div>
//             <div class="info-item">
//                 <strong>From Date (BS):</strong> ${dateRange.fromDate}
//             </div>
//             <div class="info-item">
//                 <strong>To Date (BS):</strong> ${dateRange.toDate}
//             </div>
//             <div class="info-item text-end">
//                 <strong>Opening:</strong> ${formatCurrency(Math.abs(openingBalanceBefore))} ${openingBalanceType}
//             </div>
//         </div>
...
//                         {/* Account Selection */}
//                         <div className="col-12" style={{ flex: '0 0 auto', width: '18%' }}>
...
//                         {/* From Date BS Field */}
//                         <div className="col-12" style={{ flex: '0 0 auto', width: '13%' }}>
//                             <div className="position-relative">
//                                 <input
//                                     type="text"
//                                     id="fromDate"
...
//                         <div className="alert alert-info text-center py-3" style={{ fontSize: '0.8rem' }}>
//                             <i className="fas fa-info-circle me-2"></i>Please select date range and click "Generate" to view the ageing report.
//                         </div>
//                     )}
...
//                                         {formatCurrency(Math.abs(data.agingData?.openingBalanceBeforeFromDate || 0))} {data.agingData?.openingBalanceType || (data.agingData?.openingBalanceBeforeFromDate >= 0 ? 'Dr' : 'Cr')}
//                                     </span>
//                                 </div>
//                             </div>
...
//                                                         <td style={{ padding: '6px 8px' }}>{new Date(transaction.date).toLocaleDateString()}</td>
...
//                                     <i className="fas fa-exclamation-triangle me-2"></i>No transactions found for the selected account and date range. Only opening balance is shown.
//                                 </div>
//                             )}
//                         </>
//                     )}
...
const DayWiseAgeing = () => {
    ...
    const exportToExcel = async () => {
        ...
        } catch (err) { setNotification({ show: true, message: 'Failed to export data', type: 'error' });
