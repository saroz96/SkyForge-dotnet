...
const AddPurcRtn = () => {
    ...
    const [useVoucherLastDateForPurchaseReturn, setUseVoucherLastDateForPurchaseReturn] = useState(false);
    const [lastPurchaseReturnDate, setLastPurchaseReturnDate] = useState(null);
    ...
    // Fetch date preference setting from backend for Purchase Return
    const fetchDatePreference = async () => {
        try {
            console.log('=== fetchDatePreferenceForPurchaseReturn CALLED ===');
            const response = await api.get('/api/retailer/date-preference/purchase-return');
            console.log('Date preference response:', response.data);

            if (response.data.success) {
                const useVoucherDate = response.data.data.useVoucherLastDate;
                console.log('useVoucherLastDateForPurchaseReturn value from API:', useVoucherDate);
                setUseVoucherLastDateForPurchaseReturn(useVoucherDate);
    ...
    const fetchLastPurchaseReturnDate = async () => {
        try {
            console.log('=== fetchLastPurchaseReturnDate CALLED ===');

            const response = await api.get('/api/retailer/last-purchase-return-date');
            console.log('Last purchase return date response:', response.data);
            ...
            if (response.data.success && response.data.data) {
                ...
                if (lastDate) {
                    ...
                    console.log('Formatted last purchase return date:', formattedDate);
                    setLastPurchaseReturnDate(formattedDate);
                    return formattedDate;
                }
            }

            console.log('No last purchase return date found - returning null');
            return null;
        } catch (error) {
            console.error('Error fetching last purchase return date:', error);
            return null;
        }
    };
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                ...
                // Fetch last purchase return date if needed
                let lastDate = null;
                if (useVoucherDate) {
                    lastDate = await fetchLastPurchaseReturnDate();
                }
            ...
            if (e.key === 'ArrowDown') {
                ...
                if (highlightedRowIndex >= 0 && transactions[highlightedRowIndex]) {
                    ...
                    if (transactionType === 'purchase') {
                        ...
                        if (billId) navigate(`/retailer/purchase/${billId?.id || billId}/print`);
                    } else {
                        const billId = transaction.salesBillId || transaction.billId;
                        if (billId) navigate(`/retailer/sales/${billId?.id || billId}/print`);
                    }
                ...
                <div class="print-invoice-container">
                    ...
                    <div class="print-invoice-details">
                        ...
                        <div>
                            ...
                            <div><strong>Vch. Date:</strong> ${printData.companyDateFormat === 'nepali' ? formatDateForInput(printData.nepaliDate, 'Nepali') : formatDateForInput(printData.bill.date)}(${new Date(printData.bill.date).toLocaleDateString()})</div>
                        </div>
                    </div>
