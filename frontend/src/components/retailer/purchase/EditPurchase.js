...
const EditPurchase = () => {
    ...
    const fetchItemsFromBackend = async (searchTerm = '', page = 1, isHeaderModal = true) => {
        try {
            ...
            // Add date filter based on the bill's transaction date (not current date)
    ...
    useEffect(() => {
        const fetchPurchaseData = async () => {
            try {
                ...
                setStoreManagementEnabled(data.storeManagementEnabled || false);
    ...
    const fetchLastTransactions = async (itemId, index) => {
        ...
        try {
            ...
            if (transactionCache.has(cacheKey)) {
                const cachedTransactions = transactionCache.get(cacheKey);
                setTransactions(cachedTransactions);
                setTransactionType('purchase');  // Set to purchase
                setShowTransactionModal(true);
                return;
            }
    ...
    // Function to open sales price modal from header row
    ...
    }, [showTransactionModal, showAccountCreationModal, showAccountModal, showSalesPriceModal]);
    ...
    const handleSubmit = async (e, print = false) => {
        ...
        try {
            ...
            // Prepare data according to your ASP.NET UpdatePurchaseBillDTO
            const billData = {
            ...
            console.log('Submitting purchase data:', billData);
                                    ...
                                    <tr style={{
                                        ...
                                        <td width="8%" style={{ padding: '2px', backgroundColor: '#ffffff' }}>
                                            <input
                                                ...
                                                onChange={(e) => setSelectedItemBonus(parseFloat(e.target.value))}
                                                onFocus={(e) => e.target.select()}
                                                onKeyDown={(e) => {
                                                    if ((e.key === 'Tab' || e.key === 'Enter')) {
                                                        e.preventDefault();
                                        ...
                                        <tr key={index} className={`item ${item.vatStatus === '13' ? 'vatable-item' : 'non-vatable-item'}`} style={{ height: '26px' }}>
                                            ...
                                            <td style={{ padding: '3px' }}>
                                                <input
                                                    ...
                                                    id={`bonus-${index}`}
                                                    value={item.bonus}
                                                    onChange={(e) => updateItemField(index, 'bonus', parseFloat(e.target.value))}
                                                    onFocus={(e) => {
                                                        e.target.select();
                                                    }}
                                                    onKeyDown={(e) => {
                                            ...
                                            {transactions.length > 0 ? (
                                                transactions.map((transaction, index) => (
                                                    <tr
                                                        ...
                                                        onClick={() => {
                                                            if (transaction.purchaseBillId && transaction.purchaseBillId.id) {
                                                ...
                                                <tr style={{ height: '28px' }}>
                                                    <td colSpan="10" className="text-center text-muted align-middle" style={{ padding: '0.15rem 0.3rem' }}>
