...
const convertBsToAd = (bsDate) => {
    ...
    try {
        // Create NepaliDate from BS date string
        const nepaliDate = new NepaliDate(bsDate);

        // Check if the NepaliDate object is valid
...
const convertAdToBs = (adDate) => {
    ...
    try {
        ...
        // Create NepaliDate from JavaScript Date
        const nepaliDate = new NepaliDate(date);

        // Check if NepaliDate object is valid
        ...
        const month = nepaliDate.getMonth(); // Returns 0-11 (0 = Baisakh)
...
const isValidNepaliDate = (dateStr) => {
    ...
    try {
        ...
        // Quick validation for month range
        if (month < 1 || month > 12) return false;

        // Quick validation for day range (max 32 days in Nepali calendar)
        if (day < 1 || day > 32) return false;

        // Create NepaliDate object
        ...
        // Verify by checking if the year matches
...
const getCurrentNepaliDate = () => {
    try {
        ...
        if (!now || typeof now.getYear !== 'function') {
            console.error('Cannot create current Nepali date');
...
const getNepaliMonthDays = (year, month) => {
    ...
    const monthDays = {
        1: 31,  // Baisakh
        2: 31,  // Jestha
        3: 32,  // Ashad (varies, sometimes 31)
        4: 32,  // Shrawan
        5: 31,  // Bhadra
        6: 31,  // Ashwin
        7: 30,  // Kartik
        ...
        9: 30,  // Poush
        10: 30, // Magh
        11: 30, // Falgun (29 in leap years)
        12: 30  // Chaitra
    };

    // Adjust for Ashad (month 3) - some years have 31 days
    if (month === 3) {
        // Years where Ashad has 31 days (based on Nepali calendar pattern)
        const ashad31Years = [2078, 2079, 2082, 2083, 2086, 2087];
        return ashad31Years.includes(year) ? 31 : 32;
    }

    // Adjust for Falgun (month 11) - leap year adjustment
    if (month === 11) {
        // Nepali leap years: years where (year + 1) is divisible by 4
...
const AddPurchase = () => {
    ...
    // Add after your other state declarations (around line 150-200)
    const [useVoucherLastDateForPurchase, setUseVoucherLastDateForPurchase] = useState(false);
    ...
    const fetchItemsFromBackend = async (searchTerm = '', page = 1, isHeaderModal = false) => {
        try {
            ...
            if (formData.transactionDateRoman) {
                ...
                console.log('Sending AD date filter:', formData.transactionDateRoman);
                ...
                if (adDate) {
                    ...
                    console.log('Converted BS to AD for filter:', formData.transactionDateNepali, '->', adDate);
                }
            }
            ...
            if (response.data.success) {
                const itemsWithLatestPuPrice = response.data.items.map(item => {
                    ...
                    // Calculate total stock from filtered stockEntries (already filtered by backend)
                    let totalStock = 0;
        ...
        const fetchInitialData = async () => {
            try {
                ...
                if (response.data.success) {
                    ...
                    // Fetch date preference
                    const useVoucherDate = await fetchDatePreference();

                    // Fetch last purchase date if needed
                    let lastDate = null;
                    if (useVoucherDate) {
                        lastDate = await fetchLastPurchaseDate();
                    }
                    ...
                    if (useVoucherDate && lastDate) {
                        ...
                        if (isNepaliFormat) {
                            // lastDate should already be in YYYY-MM-DD format
    ...
    // Fetch date preference setting from backend
    const fetchDatePreference = async () => {
        try {
            console.log('=== fetchDatePreference CALLED ===');
            const response = await api.get('/api/retailer/date-preference/purchase');
            console.log('Date preference response:', response.data);

            if (response.data.success) {
                const useVoucherDate = response.data.data.useVoucherLastDate;
                console.log('useVoucherLastDateForPurchase value from API:', useVoucherDate);
                setUseVoucherLastDateForPurchase(useVoucherDate);
    ...
    // Fetch last purchase date from backend
    const fetchLastPurchaseDate = async () => {
        try {
            console.log('=== fetchLastPurchaseDate CALLED ===');

            // Use the dedicated endpoint
            const response = await api.get('/api/retailer/last-purchase-date');
            console.log('Last purchase date response:', response.data);
            ...
            if (response.data.success && response.data.data) {
                ...
                if (lastDate) {
                    ...
                    console.log('Formatted last purchase date:', formattedDate);
                    setLastPurchaseDate(formattedDate);
                    return formattedDate;
                }
            }

            console.log('No last purchase date found - returning null');
            return null;
        } catch (error) {
            console.error('Error fetching last purchase date:', error);
            return null;
        }
    };
    ...
    const fetchLastTransactions = async (itemId, index) => {
        ...
        setCurrentViewingItemId(itemId);
        setIsLoadingTransactions(true);
    ...
    const fetchSalesTransactions = async () => {
        ...
        // Use currentViewingItemId instead of items[selectedItemIndex]
        const itemId = currentViewingItemId;

        console.log('currentViewingItemId:', itemId);
        ...
        try {
            setIsLoadingTransactions(true);
            // FIX: Use itemId, not items[selectedItemIndex]?.itemId
    ...
    const handleManualReset = async () => {
        try {
            ...
            let transactionDateBS = '';
            let invoiceDateBS = '';
            let transactionDateAD = '';
            let invoiceDateAD = '';

            // Set dates based on preference
            if (useVoucherDate && lastDate) {
                if (isNepaliFormat) {
                    transactionDateBS = lastDate;
                    invoiceDateBS = lastDate;
                    transactionDateAD = convertBsToAd(lastDate) || '';
                    invoiceDateAD = convertBsToAd(lastDate) || '';
                } else {
                    transactionDateAD = lastDate;
                    invoiceDateAD = lastDate;
                    transactionDateBS = convertAdToBs(lastDate) || '';
                    invoiceDateBS = convertAdToBs(lastDate) || '';
                }
            } else {
                if (isNepaliFormat) {
                    const today = new NepaliDate();
                    transactionDateBS = today.format('YYYY-MM-DD');
                    invoiceDateBS = today.format('YYYY-MM-DD');
                    transactionDateAD = convertBsToAd(transactionDateBS) || '';
                    invoiceDateAD = convertBsToAd(invoiceDateBS) || '';
                } else {
                    const today = new Date();
                    const todayStr = today.toISOString().split('T')[0];
                    transactionDateAD = todayStr;
                    invoiceDateAD = todayStr;
                    transactionDateBS = convertAdToBs(todayStr) || '';
                    invoiceDateBS = convertAdToBs(todayStr) || '';
                }
            }
            ...
            setFormData({
                ...
                accountAddress: '',
                accountPan: '',
                transactionDateNepali: isNepaliFormat ? transactionDateBS : (transactionDateBS || ''),
                transactionDateRoman: isNepaliFormat ? (transactionDateAD || '') : transactionDateAD,
                nepaliDate: isNepaliFormat ? invoiceDateBS : (invoiceDateBS || ''),
                billDate: isNepaliFormat ? (invoiceDateAD || '') : invoiceDateAD,
                billNumber: currentBillNum,
                partyBillNumber: '',
                paymentMode: 'credit',
                isVatExempt: 'all',
    ...
    const resetAfterSave = async () => {
        try {
            ...
            if (useVoucherDate && lastDate) {
                ...
                console.log('resetAfterSave - Using LAST VOUCHER date:', { transactionDateBS, transactionDateAD });
            } else {
                if (isNepaliFormat) {
                    const today = new NepaliDate();
                    transactionDateBS = today.format('YYYY-MM-DD');
                ...
                console.log('resetAfterSave - Using SYSTEM date:', { transactionDateBS, transactionDateAD });
            }
                ...
                <div class="print-invoice-container">
                    ...
                    <div class="print-invoice-details">
                        ...
                        <div>
                            ...
                            <div><strong>Trans Date:</strong> ${printData.companyDateFormat === 'nepali' ? formatDateForInput(printData.bill.transactionDateNepali, 'Nepali') : formatDateForInput(printData.bill.transactionDate)}(${new Date(printData.bill.transactionDate).toLocaleDateString()})</div>
                            <div><strong>Invoice Date:</strong> ${printData.companyDateFormat === 'nepali' ? formatDateForInput(printData.bill.nepaliDate, 'Nepali') : formatDateForInput(printData.bill.date)}(${new Date(printData.bill.date).toLocaleDateString()})</div>
                        </div>
                    </div>
                ...
                <div className="card-header">
                    <div className="d-flex justify-content-between align-items-center">
                        <h2 className="card-title mb-0">
                            ...
                            Purchase Entry - to change date click (Ctrl+D)
                        </h2>
                        <div>
                            {formData.billNumber === '' && (
                ...
                <div className="card-body p-2 p-md-3">
                    <form onSubmit={handleSubmit} id="billForm" className="needs-validation" noValidate>
                                    ...
                                    <div className="col-12 col-md-6 col-lg-2">
                                        <div className="position-relative">
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    if (sanitizedValue.length <= 10) {
                                                        ...
                                                        if (sanitizedValue.length === 10 && /^\d{4}-\d{2}-\d{2}$/.test(sanitizedValue)) {
                                                            console.log('Invoice - Converting BS to AD:', sanitizedValue);
                                                            const adDate = convertBsToAd(sanitizedValue);
                                                            console.log('Invoice - Converted AD date:', adDate);
                                                            if (adDate) {
                                                                setFormData(prev => ({
                                                                    ...prev,
                                                                    billDate: adDate
                                            ...
                                            <label className="position-absolute" style={{
                                                ...
                                                Invoice Date (BS): <span className="text-danger">*</span>
                                            </label>
                                            {dateErrors.nepaliDate && (
                                                <div className="invalid-feedback d-block" style={{ fontSize: '0.7rem' }}>
                                                    {dateErrors.nepaliDate}
                                                ...
                                                onKeyDown={(e) => {
                                                    ...
                                                    if (e.key === 'Enter') {
                                                        ...
                                                        if (!dateStr) {
                                                            ...
                                                            setFormData({
                                                                ...formData,
                                                                billDate: correctedDate
                                                            });
                                                            setDateErrors(prev => ({ ...prev, billDate: '' }));
                        ...
                        <div className="row g-1 mb-2">
                                    ...
                                    {dateFormatToggle === 'nepali' ? (
                                        ...
                                        <input
                                                ...
                                                if (e.key === 'Enter') {
                                                    ...
                                                    if (!dateStr) {
                                                        ...
                                                        setFormData(prev => ({
                                                            ...
                                                            transactionDateRoman: adDate || prev.transactionDateRoman,
                                                            billDate: adDate || prev.billDate,
                                                            nepaliDate: correctedDate  // Also update invoice date BS
                                                        }));
                                                        setDateErrors(prev => ({ ...prev, transactionDateNepali: '' }));
                                            ...
                                            onChange={(e) => {
                                                ...
                                                if (sanitizedValue.length <= 10) {
                                                    ...
                                                    // If complete date, update both AD and invoice dates
                                                    if (sanitizedValue.length === 10 && /^\d{4}-\d{2}-\d{2}$/.test(sanitizedValue)) {
                                                        const adDate = convertBsToAd(sanitizedValue);
                                                        if (adDate) {
                                                            setFormData(prev => ({
                                                                ...
                                                                transactionDateRoman: adDate,
                                                                billDate: adDate,
                                                                nepaliDate: sanitizedValue  // Update invoice date BS
                                                            }));
                                                        }
                                                    }
                                                }
                                                ...
                                                if (isValidNepaliDate(dateStr)) {
                                                    ...
                                                    if (adDate) {
                                                        setFormData(prev => ({
                                                            ...
                                                            transactionDateRoman: adDate,
                                                            billDate: adDate,
                                                            nepaliDate: dateStr  // Update invoice date BS
                                                        }));
                                                    }
                                                    setDateErrors(prev => ({ ...prev, transactionDateNepali: '' }));
                                                } else {
                                                    ...
                                                    setFormData(prev => ({
                                                        ...
                                                        transactionDateRoman: adDate || prev.transactionDateRoman,
                                                        billDate: adDate || prev.billDate,
                                                        nepaliDate: correctedDate  // Update invoice date BS
                                                    }));
                                                    setNotification({
                                                        show: true,
                                                        message: 'Invalid Nepali date. Auto-corrected to current date.',
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => {
                                                ...
                                                console.log('AD to BS conversion:', value, '->', bsDate);
                                                ...
                                                setFormData(prev => ({
                                                    ...
                                                    billDate: value,
                                                    nepaliDate: bsDate || prev.nepaliDate  // Also update invoice date BS
                                                }));
                                            }}
                                            onKeyDown={(e) => {
                                                if (e.key === 'Enter') {
                            ...
                            {/* Invoice Date - Toggle between BS and AD */}
                            <div className="col-12 col-md-6 col-lg-2">
                                <div className="position-relative">
                                    {dateFormatToggle === 'nepali' ? (
                                        // BS Invoice Date
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => {
                                                ...
                                                // Convert AD to BS
                                                const bsDate = convertAdToBs(value);
                                                console.log('Invoice AD to BS conversion:', value, '->', bsDate);

                                                setFormData(prev => ({
                                                    ...prev,
                                                    billDate: value,
                                                    nepaliDate: bsDate || prev.nepaliDate,
                                                    transactionDateNepali: bsDate || prev.transactionDateNepali,  // Also update transaction date BS
                                                    transactionDateRoman: value  // Keep AD in sync
                                                }));
                                            }}
                                            onKeyDown={(e) => {
                                                if (e.key === 'Enter') {
                                    ...
                                    <label className="position-absolute" style={{
                                        ...
                                        Invoice Date {dateFormatToggle === 'nepali' ? '(BS)' : '(AD)'}:
                                        <span className="text-danger">*</span>
                                    </label>
                            ...
                            {/* Hidden fields for both invoice dates */}
                            <input
                                type="hidden"
                            ...
                            {/* Rest of your fields (billNumber, paymentMode, etc.) */}
                            <div className="col-12 col-md-6 col-lg-2">
                                <div className="position-relative">
                                    <input
                                        type="text"
