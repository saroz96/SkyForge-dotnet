...
// Safe lowercase — handles null, undefined, numbers, objects, dates, etc.
const toLower = (val) => (val == null ? '' : String(val).toLowerCase());
...
const DailyInventorySummary = ({ show, onClose, companyId }) => {
    ...
    // -------- Client-side filter over currently loaded items --------
    const visibleItems = useMemo(() => {
        const term = searchTerm.trim().toLowerCase();
        if (!term) return inventoryData.items;

        return inventoryData.items.filter((item) => {
            const name = toLower(item.itemName);
            const code = toLower(item.uniqueNumber);
            const category = toLower(item.categoryName);
            const unit = toLower(item.unitName);
    ...
    }, [searchTerm, inventoryData.items]);

    // -------- Fetch from server --------
    const fetchInventoryData = useCallback(async (searchOverride) => {
        if (!companyId) {
            setNotification({
                show: true,
                message: 'Company not selected',
        ...
        try {
            ...
            params.append('page', currentPage);
            params.append('pageSize', rowsPerPage);
            const termToSend = searchOverride !== undefined ? searchOverride : serverSearchTerm;
            if (termToSend) {
                params.append('searchTerm', termToSend);
            }
    ...
    }, [companyId, currentPage, rowsPerPage, serverSearchTerm]);

    // Initial load / page / rows / serverSearchTerm change → fetch from server
    useEffect(() => {
        if (show && companyId) {
            fetchInventoryData(serverSearchTerm);
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [show, companyId, currentPage, rowsPerPage, serverSearchTerm]);
    ...
    // -------- Debounced decision: filter locally OR hit server --------
    useEffect(() => {
        if (!show) return;
        ...
        // No typing → reset server search term (do not refetch)
        if (term === '') {
            setServerSearchTerm('');
            return;
        }
        ...
        // Check if any currently-loaded item matches
        const localMatches = inventoryData.items.some((item) =>
            toLower(item.itemName).includes(t) ||
            toLower(item.uniqueNumber).includes(t) ||
            toLower(item.categoryName).includes(t) ||
            toLower(item.unitName).includes(t)
        );

        // If local match → do not hit server, do not show loading
        if (localMatches) {
            return;
        }

        // Otherwise debounce, then fetch from server
                                ...
                                <form onSubmit={handleSearch} className="dis-search-form">
                                    <div className="dis-search-input-wrap">
                                        ...
                                        <input
                                            type="text"
                                            className="dis-search-input"
                                            placeholder="Search by Item Name, Code, Category, Unit..."
                                            value={searchTerm}
                                            onChange={(e) => setSearchTerm(e.target.value)}
                                        />
                                    </div>
                            ...
                            <div className="dis-table-card-body">
                                {loading ? (
                                    ...
                                    <>
                                        ...
                                        {/* Pagination (only when not locally filtering) */}
                                        {inventoryData.totalItems > 0 && !searchTerm && (
                                            <div className="dis-pagination">
                                                <div className="dis-pagination-left">
                                                    <label className="dis-pagination-label">Rows:</label>
                                                    <select
