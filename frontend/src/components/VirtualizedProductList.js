...
const compressText = (text, maxLength = 40) => {
  if (!text) return '';
  if (text.length <= maxLength) return text;
  return text.substring(0, maxLength - 3) + '...';
};

const ProductRow = memo(({ product, index, style, onProductClick, searchRef, compressText }) => {
  ...
  // Compress product name for display
  const compressedName = compressText(product.name, 35);
  const compressedCategory = compressText(displayCategory, 20);
  ...
  return (
    <div
      ...
      style={{
        ...style,
        display: 'grid',
        gridTemplateColumns: '0.5fr 3fr 1fr 1fr 1fr 1fr 1fr 0.8fr', // Removed HSN column, matches modal header
        alignItems: 'center',
        padding: '0 8px',
...
const VirtualizedProductList = memo(({
  ...
  page,
  searchQuery = '',
  compressText = (text) => text // Default function if not provided
}) => {
  const itemHeight = 28;
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (!loadingRef.current && hasMore && !searchQuery.trim()) {
  ...
  }, [hasMore, loadMoreItems, searchQuery]);
...
//   // Determine if product is vatable (13% VAT)
//   // Check for string "13" as per your requirement
//   const isVatable = product.vatStatus === '13' || 
//                     product.vatStatus === 'vatable' || 
//                     product.vatStatus === 'false' ||
//                     product.vatStatus === 13; // Also handle number 13 if needed
...
//   // Determine row background color based on VAT status
//   const getRowBackgroundColor = () => {
//     // Light green for 13% VAT (string "13")
//     if (product.vatStatus === '13') {
//       return '#d4edda'; // light green
//     }
//     // Light yellow for vat exempt
//     if (product.vatStatus === 'vatExempt' || product.vatStatus === 'exempt') {
//       return '#fff3cd'; // light yellow
//     }
//     // Default for other vat statuses
//     return '';
//   };

//   // Build CSS class string - keep existing classes for other styling
//   const rowClasses = [
//     'dropdown-item',
//     product.vatStatus === '13' ? 'vat-13' : (isVatable ? 'vatable' : 'vatExempt'),
//     `expiry-${expiryStatus}`
//   ].filter(Boolean).join(' ');
...
//         cursor: 'pointer',
//         fontSize: '12px',
//         backgroundColor: getRowBackgroundColor() // Apply dynamic background color
//       }}
//       onClick={handleClick}
//       tabIndex={0}
//       onKeyDown={handleKeyDown}
