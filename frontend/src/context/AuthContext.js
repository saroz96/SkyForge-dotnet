...
export const AuthProvider = ({ children }) => {
  ...
  const switchCompany = async (companyId) => {
    try {
      ...
      // ✅ Save selected company + fiscal year for future refresh calls
      const company = data.data?.sessionData?.company;
      const fiscalYear = data.data?.sessionData?.fiscalYear;

      if (company?.id) {
        localStorage.setItem('currentCompany', company.id);
      }
      if (fiscalYear?.id) {
        localStorage.setItem('fiscalYearId', fiscalYear.id);
      }

      // ✅ Update Redux state
      dispatch(setCurrentCompany({ company, fiscalYear }));
